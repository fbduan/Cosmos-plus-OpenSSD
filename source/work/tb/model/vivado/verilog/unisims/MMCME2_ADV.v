...
//inputs without timing
  ...
  reg clkvco_lk_osc,  clkvco, clkvco_lk_tmp;
...
// free run vco comp
  ...
  vcoflag = 0;
       ...
       if (clkpll == 1 && m_product > 1 && m_product != divclk_div && vcoflag == 0) begin
          for (ik4=1; ik4 < m_product; ik4=ik4+1) begin
               clkout_en_t <= ik4;
               #(period_vco_half) clkvco_lk_tmp <= 0;
               #(period_vco_half_rm) clkvco_lk_tmp <= 1;
  ...
  always @ (*) begin
    if (clk0_fps_en) clk0in = clkout_ps_mux[clk0_pm_r];
    else             clk0in = clkout_mux[clk0_pm_r];
  end

  always @ (*) begin
    if (clk1_fps_en) clk1in = clkout_ps_mux[clk1_pm];
    else             clk1in = clkout_mux[clk1_pm];
  end

  always @ (*) begin
    if (clk2_fps_en) clk2in = clkout_ps_mux[clk2_pm];
    else             clk2in = clkout_mux[clk2_pm];
  end

  always @ (*) begin
    if (clk3_fps_en) clk3in = clkout_ps_mux[clk3_pm];
    else             clk3in = clkout_mux[clk3_pm];
  end

  always @ (*) begin
    if (clk4_fps_en) clk4in = clkout_ps_mux[clk4_pm];
    else if (clkout4_cascade_int) clk4in = clk6_out;
    else clk4in = clkout_mux[clk4_pm];
  end

  always @ (*) begin
    if (clk5_fps_en) clk5in = clkout_ps_mux[clk5_pm];
    else             clk5in = clkout_mux[clk5_pm];
  end

  always @ (*) begin
    if (clk6_fps_en) clk6in = clkout_ps_mux[clk6_pm];
    else             clk6in = clkout_mux[clk6_pm];
  end

  always @ (*) begin
    if (clkfbout_fps_en) clkfboutin = clkout_ps_mux[clkfbout_pm_r];
    else              clkfboutin = clkout_mux[clkfbout_pm_r];
  end
...
if (nc == 1'b1) begin
  ...
  phase = dt * phase_step;
   ...
   `ifdef DEBUG
    $display("mc_to_attr(%t): - div: %f, phase: %f, duty: %f, ", $time, div, phase, duty);
   `endif
end
endtask 
...
// determine int and frac part
   ...
   `ifdef DEBUG
    $display("attr_to_mc(%t): - div_int: %d, div_frac: %f, frac_en: %b", $time, div_int, div_frac, frac_en);
   `endif
...
function [10:0] mmcm_phase
   ...
   begin
      if ((phase < -360000) || (phase > 360000)) begin
         $display("ERROR: phase of (%d) is not between -360000 and 360000",phase);
         $finish;
      end
