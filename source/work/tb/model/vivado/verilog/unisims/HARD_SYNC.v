...
always @ (posedge CLK_in or posedge glblGSR) begin
   if (glblGSR == 1'b1) begin
      DIN_reg <= #100 {INIT_BIN, INIT_BIN, INIT_BIN};
      end
   else begin
      DIN_reg <= #100 {DIN_reg[1:0], DIN_in};
      end
   end
...
`ifdef XIL_TIMING
  ...
  specify
    ...
    $setuphold (negedge CLK, negedge DIN, 0:0:0, 0:0:0, notifier,clk_en_n,clk_en_n, CLK_dly, DIN_dly);
    $setuphold (negedge CLK, posedge DIN, 0:0:0, 0:0:0, notifier,clk_en_n,clk_en_n, CLK_dly, DIN_dly);
    $setuphold (posedge CLK, negedge DIN, 0:0:0, 0:0:0, notifier,clk_en_p,clk_en_p, CLK_dly, DIN_dly);
    $setuphold (posedge CLK, posedge DIN, 0:0:0, 0:0:0, notifier,clk_en_p,clk_en_p, CLK_dly, DIN_dly);
