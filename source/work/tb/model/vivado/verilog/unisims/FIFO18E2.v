...
reg en_clk_sync = 1'b0;
  ...
  always @ (posedge trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
        ((INIT_REG < 36'h000000000) || (INIT_REG > 36'hFFFFFFFFF))) begin
      $display("Error: [Unisim %s-107] INIT attribute is set to %h.  Legal values for this attribute are 36'h000000000 to 36'hFFFFFFFFF. Instance: %m", MODULE_NAME, INIT_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
        ((SRVAL_REG < 36'h000000000) || (SRVAL_REG > 36'hFFFFFFFFF))) begin
      $display("Error: [Unisim %s-121] SRVAL attribute is set to %h.  Legal values for this attribute are 36'h000000000 to 36'hFFFFFFFFF. Instance: %m", MODULE_NAME, SRVAL_REG);
      attr_err = 1'b1;
    end
  ...
  always @(WRITE_WIDTH_BIN) wr_loops_b <= WRITE_WIDTH_BIN;
   ...
   assign RSTREG_A_int = (REGISTER_MODE_BIN != REGISTER_MODE_DO_PIPELINED) ? RDRST_int :
                         ...
                         (RSTREG_in && REGCE_in);
   assign RDEN_lat = RDEN_int || ((fill_reg || fill_lat) && ~SLEEP_int);
