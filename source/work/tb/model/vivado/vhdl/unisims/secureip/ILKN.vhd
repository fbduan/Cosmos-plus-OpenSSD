...
architecture ILKN_V of ILKN is
  component SIP_ILKN
    ...
    port (
        ...
        CTL_TX_FC_CALLEN     : in std_logic_vector(6 downto 0);
        ...
        RX_SOPOUT2           : out std_ulogic;
        RX_SOPOUT3           : out std_ulogic;
        SCAN_OUT_DRPCTRL     : out std_logic_vector(14 downto 0);
        SCAN_OUT_ILMAC       : out std_logic_vector(249 downto 0);
        STAT_RX_ALIGNED      : out std_ulogic;
        STAT_RX_ALIGNED_ERR  : out std_ulogic;
        STAT_RX_BAD_TYPE_ERR : out std_logic_vector(11 downto 0);
        STAT_RX_BURSTMAX_ERR : out std_ulogic;
        ...
        RX_SERDES_DATA11     : in std_logic_vector(63 downto 0);
        RX_SERDES_RESET      : in std_logic_vector(11 downto 0);
        SCAN_EN              : in std_ulogic;
        SCAN_IN_DRPCTRL      : in std_logic_vector(14 downto 0);
        SCAN_IN_ILMAC        : in std_logic_vector(249 downto 0);
    ...
    constant MODULE_NAME : string := "ILKN";
  ...
  END conv_string_to_slv;
    ...
    constant CTL_TX_FC_CALLEN_BIN : std_logic_vector(6 downto 0) := CTL_TX_FC_CALLEN;
  ...
  -- Parameter encodings and registers
    ...
    signal SCAN_OUT_DRPCTRL_out : std_logic_vector(14 downto 0);
    signal SCAN_OUT_ILMAC_out : std_logic_vector(249 downto 0);
    ...
    signal SCAN_EN_in : std_ulogic;
    signal SCAN_IN_DRPCTRL_in : std_logic_vector(14 downto 0);
    signal SCAN_IN_ILMAC_in : std_logic_vector(249 downto 0);
    signal TEST_MODE_in : std_ulogic;
    ...
    RX_BYPASS_DATAOUT00 <= RX_BYPASS_DATAOUT00_out after OUT_DELAY;
    RX_BYPASS_DATAOUT01 <= RX_BYPASS_DATAOUT01_out after OUT_DELAY;
    RX_BYPASS_DATAOUT02 <= RX_BYPASS_DATAOUT02_out after OUT_DELAY;
    RX_BYPASS_DATAOUT03 <= RX_BYPASS_DATAOUT03_out after OUT_DELAY;
    RX_BYPASS_DATAOUT04 <= RX_BYPASS_DATAOUT04_out after OUT_DELAY;
    RX_BYPASS_DATAOUT05 <= RX_BYPASS_DATAOUT05_out after OUT_DELAY;
    RX_BYPASS_DATAOUT06 <= RX_BYPASS_DATAOUT06_out after OUT_DELAY;
    RX_BYPASS_DATAOUT07 <= RX_BYPASS_DATAOUT07_out after OUT_DELAY;
    RX_BYPASS_DATAOUT08 <= RX_BYPASS_DATAOUT08_out after OUT_DELAY;
    RX_BYPASS_DATAOUT09 <= RX_BYPASS_DATAOUT09_out after OUT_DELAY;
    RX_BYPASS_DATAOUT10 <= RX_BYPASS_DATAOUT10_out after OUT_DELAY;
    RX_BYPASS_DATAOUT11 <= RX_BYPASS_DATAOUT11_out after OUT_DELAY;
    RX_BYPASS_ENAOUT <= RX_BYPASS_ENAOUT_out after OUT_DELAY;
    RX_BYPASS_IS_AVAILOUT <= RX_BYPASS_IS_AVAILOUT_out after OUT_DELAY;
    RX_BYPASS_IS_BADLYFRAMEDOUT <= RX_BYPASS_IS_BADLYFRAMEDOUT_out after OUT_DELAY;
    RX_BYPASS_IS_OVERFLOWOUT <= RX_BYPASS_IS_OVERFLOWOUT_out after OUT_DELAY;
    RX_BYPASS_IS_SYNCEDOUT <= RX_BYPASS_IS_SYNCEDOUT_out after OUT_DELAY;
    RX_BYPASS_IS_SYNCWORDOUT <= RX_BYPASS_IS_SYNCWORDOUT_out after OUT_DELAY;
    RX_CHANOUT0 <= RX_CHANOUT0_out after OUT_DELAY;
    RX_CHANOUT1 <= RX_CHANOUT1_out after OUT_DELAY;
    RX_CHANOUT2 <= RX_CHANOUT2_out after OUT_DELAY;
    RX_CHANOUT3 <= RX_CHANOUT3_out after OUT_DELAY;
    ...
    RX_OVFOUT <= RX_OVFOUT_out after OUT_DELAY;
    ...
    STAT_RX_BAD_TYPE_ERR <= STAT_RX_BAD_TYPE_ERR_out after OUT_DELAY;
    STAT_RX_BURSTMAX_ERR <= STAT_RX_BURSTMAX_ERR_out after OUT_DELAY;
    STAT_RX_BURST_ERR <= STAT_RX_BURST_ERR_out after OUT_DELAY;
    STAT_RX_CRC24_ERR <= STAT_RX_CRC24_ERR_out after OUT_DELAY;
    STAT_RX_CRC32_ERR <= STAT_RX_CRC32_ERR_out after OUT_DELAY;
    STAT_RX_CRC32_VALID <= STAT_RX_CRC32_VALID_out after OUT_DELAY;
    STAT_RX_DESCRAM_ERR <= STAT_RX_DESCRAM_ERR_out after OUT_DELAY;
    STAT_RX_DIAGWORD_INTFSTAT <= STAT_RX_DIAGWORD_INTFSTAT_out after OUT_DELAY;
    STAT_RX_DIAGWORD_LANESTAT <= STAT_RX_DIAGWORD_LANESTAT_out after OUT_DELAY;
    STAT_RX_FC_STAT <= STAT_RX_FC_STAT_out after OUT_DELAY;
    STAT_RX_FRAMING_ERR <= STAT_RX_FRAMING_ERR_out after OUT_DELAY;
    STAT_RX_MEOP_ERR <= STAT_RX_MEOP_ERR_out after OUT_DELAY;
    ...
    STAT_RX_MSOP_ERR <= STAT_RX_MSOP_ERR_out after OUT_DELAY;
    STAT_RX_MUBITS <= STAT_RX_MUBITS_out after OUT_DELAY;
    STAT_RX_MUBITS_UPDATED <= STAT_RX_MUBITS_UPDATED_out after OUT_DELAY;
    STAT_RX_OVERFLOW_ERR <= STAT_RX_OVERFLOW_ERR_out after OUT_DELAY;
    STAT_RX_RETRANS_CRC24_ERR <= STAT_RX_RETRANS_CRC24_ERR_out after OUT_DELAY;
    STAT_RX_RETRANS_DISC <= STAT_RX_RETRANS_DISC_out after OUT_DELAY;
    STAT_RX_RETRANS_LATENCY <= STAT_RX_RETRANS_LATENCY_out after OUT_DELAY;
    STAT_RX_RETRANS_REQ <= STAT_RX_RETRANS_REQ_out after OUT_DELAY;
    STAT_RX_RETRANS_RETRY_ERR <= STAT_RX_RETRANS_RETRY_ERR_out after OUT_DELAY;
    STAT_RX_RETRANS_SEQ <= STAT_RX_RETRANS_SEQ_out after OUT_DELAY;
    STAT_RX_RETRANS_SEQ_UPDATED <= STAT_RX_RETRANS_SEQ_UPDATED_out after OUT_DELAY;
    STAT_RX_RETRANS_STATE <= STAT_RX_RETRANS_STATE_out after OUT_DELAY;
    STAT_RX_RETRANS_SUBSEQ <= STAT_RX_RETRANS_SUBSEQ_out after OUT_DELAY;
    STAT_RX_RETRANS_WDOG_ERR <= STAT_RX_RETRANS_WDOG_ERR_out after OUT_DELAY;
    STAT_RX_RETRANS_WRAP_ERR <= STAT_RX_RETRANS_WRAP_ERR_out after OUT_DELAY;
    STAT_RX_SYNCED <= STAT_RX_SYNCED_out after OUT_DELAY;
    STAT_RX_SYNCED_ERR <= STAT_RX_SYNCED_ERR_out after OUT_DELAY;
    STAT_RX_WORD_SYNC <= STAT_RX_WORD_SYNC_out after OUT_DELAY;
    STAT_TX_BURST_ERR <= STAT_TX_BURST_ERR_out after OUT_DELAY;
    STAT_TX_ERRINJ_BITERR_DONE <= STAT_TX_ERRINJ_BITERR_DONE_out after OUT_DELAY;
    STAT_TX_OVERFLOW_ERR <= STAT_TX_OVERFLOW_ERR_out after OUT_DELAY;
    STAT_TX_RETRANS_BURST_ERR <= STAT_TX_RETRANS_BURST_ERR_out after OUT_DELAY;
    STAT_TX_RETRANS_BUSY <= STAT_TX_RETRANS_BUSY_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_PERROUT <= STAT_TX_RETRANS_RAM_PERROUT_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RADDR <= STAT_TX_RETRANS_RAM_RADDR_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RD_B0 <= STAT_TX_RETRANS_RAM_RD_B0_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RD_B1 <= STAT_TX_RETRANS_RAM_RD_B1_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RD_B2 <= STAT_TX_RETRANS_RAM_RD_B2_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RD_B3 <= STAT_TX_RETRANS_RAM_RD_B3_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_RSEL <= STAT_TX_RETRANS_RAM_RSEL_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WADDR <= STAT_TX_RETRANS_RAM_WADDR_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WDATA <= STAT_TX_RETRANS_RAM_WDATA_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WE_B0 <= STAT_TX_RETRANS_RAM_WE_B0_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WE_B1 <= STAT_TX_RETRANS_RAM_WE_B1_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WE_B2 <= STAT_TX_RETRANS_RAM_WE_B2_out after OUT_DELAY;
    STAT_TX_RETRANS_RAM_WE_B3 <= STAT_TX_RETRANS_RAM_WE_B3_out after OUT_DELAY;
    STAT_TX_UNDERFLOW_ERR <= STAT_TX_UNDERFLOW_ERR_out after OUT_DELAY;
    TX_OVFOUT <= TX_OVFOUT_out after OUT_DELAY;
    TX_RDYOUT <= TX_RDYOUT_out after OUT_DELAY;
    TX_SERDES_DATA00 <= TX_SERDES_DATA00_out after OUT_DELAY;
    TX_SERDES_DATA01 <= TX_SERDES_DATA01_out after OUT_DELAY;
    TX_SERDES_DATA02 <= TX_SERDES_DATA02_out after OUT_DELAY;
    TX_SERDES_DATA03 <= TX_SERDES_DATA03_out after OUT_DELAY;
    TX_SERDES_DATA04 <= TX_SERDES_DATA04_out after OUT_DELAY;
    TX_SERDES_DATA05 <= TX_SERDES_DATA05_out after OUT_DELAY;
    TX_SERDES_DATA06 <= TX_SERDES_DATA06_out after OUT_DELAY;
    TX_SERDES_DATA07 <= TX_SERDES_DATA07_out after OUT_DELAY;
    TX_SERDES_DATA08 <= TX_SERDES_DATA08_out after OUT_DELAY;
    TX_SERDES_DATA09 <= TX_SERDES_DATA09_out after OUT_DELAY;
    TX_SERDES_DATA10 <= TX_SERDES_DATA10_out after OUT_DELAY;
    TX_SERDES_DATA11 <= TX_SERDES_DATA11_out after OUT_DELAY;
    ...
    SCAN_EN_in <= '0'; -- tie off
    SCAN_IN_DRPCTRL_in <= "111111111111111"; -- tie off
    SCAN_IN_ILMAC_in <= "1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"; -- tie off
    TEST_MODE_in <= '0'; -- tie off
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'(ILKN_V'INSTANCE_NAME));
    writeline(output, Message);
    DEALLOCATE (Message);
  end if;
    -------- CTL_RX_PACKET_MODE check
  if((xil_attr_test) or
     ((CTL_RX_PACKET_MODE /= "TRUE") and 
      (CTL_RX_PACKET_MODE /= "FALSE"))) then
    attr_err := true;
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-106] CTL_RX_PACKET_MODE attribute is set to """));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-113] CTL_TEST_MODE_PIN_CHAR attribute is set to """));
  ...
  if((xil_attr_test) or
     ((CTL_TX_DISABLE_SKIPWORD /= "TRUE") and 
      (CTL_TX_DISABLE_SKIPWORD /= "FALSE"))) then
    attr_err := true;
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-117] CTL_TX_DISABLE_SKIPWORD attribute is set to """));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-124] MODE attribute is set to """));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-125] TEST_MODE_PIN_CHAR attribute is set to """));
    ...
    ILKN_INST : SIP_ILKN
      port map (
        ...
        SCAN_OUT_ILMAC       => SCAN_OUT_ILMAC_out,
        ...
        SCAN_IN_ILMAC        => SCAN_IN_ILMAC_in,
