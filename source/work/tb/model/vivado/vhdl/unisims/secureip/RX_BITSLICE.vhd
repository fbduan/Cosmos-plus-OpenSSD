...
architecture RX_BITSLICE_V of RX_BITSLICE is
  component SIP_RX_BITSLICE
    ...
    port (
        ...
        DELAY_VALUE          : in std_logic_vector(10 downto 0);
        DELAY_VALUE_EXT      : in std_logic_vector(10 downto 0);
  ...
  END conv_string_to_slv;
    ...
    constant DELAY_VALUE_BIN : std_logic_vector(10 downto 0) := std_logic_vector(to_unsigned(DELAY_VALUE,11));
    constant DELAY_VALUE_EXT_BIN : std_logic_vector(10 downto 0) := std_logic_vector(to_unsigned(DELAY_VALUE_EXT,11));
  ...
  if((xil_attr_test) or
    ...
    if ((DELAY_VALUE_EXT < 0) or (DELAY_VALUE_EXT > 1250)) then
      attr_err := true;
      Write ( Message, string'("Error : [Unisim "));
      Write ( Message, string'(MODULE_NAME));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-122] SIM_VERSION attribute is set to "));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-126] UPDATE_MODE attribute is set to """));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-127] UPDATE_MODE_EXT attribute is set to """));
    ...
    if  (attr_err) then
      ...
      Write ( Message, string'(MODULE_NAME));
      Write ( Message, string'("-118] Attribute Error(s) encountered. "));
      Write ( Message, string'("Instance "));
      Write ( Message, string'(RX_BITSLICE_V'INSTANCE_NAME));
      assert FALSE
      report Message.all
...
K2_RX_inst : if (SIM_VERSION = 2.0) generate
    ...
    V3T_RX_inst : if (SIM_VERSION = 1.0) generate
    RX_BITSLICE_INST : SIP_RX_BITSLICE
      port map (
        CASCADE              => CASCADE_BIN,
        DATA_TYPE            => DATA_TYPE_BIN,
        DATA_WIDTH           => DATA_WIDTH_BIN,
