...
--  end if;
       ...
       if (dwe_in = '1')  then
          if (rst_input = '1') then
             ...
             if (daddr_in = "0001000") then
                daddr_str := "0001000";
                 clkout_hl_para_drp (clk_lt, clk_ht, clkpm_sel, di_in, daddr_in, di_str, daddr_str, clk0_frac_en);
                 clk0_lt <= clk_lt;
                 clk0_ht <= clk_ht;
                 clk0_lti := clk_lt;
                 clk0_hti := clk_ht;
             ...
             if (daddr_in = "0001010") then
                 daddr_str := "0001010";
                 clkout_hl_para_drp (clk_lt, clk_ht, clkpm_sel, di_in, daddr_in, di_str, daddr_str, 0);
                 clk1_lt <= clk_lt;
                 clk1_ht <= clk_ht;
                 clk1_lti := clk_lt;
                 clk1_hti := clk_ht;
             ...
             if (daddr_in = "0010100") then
               daddr_str := "0010100";
               clkout_hl_para_drp (clk_lt, clk_ht, clkpm_sel, di_in, daddr_in, di_str, daddr_str, clkfb_frac_en);
               clkfbm1_lt <= clk_lt;
               clkfbm1_lti := clk_lt;
               clkfbm1_ht <= clk_ht;
               clkfbm1_hti := clk_ht;
  ...
  clkfb_frac_para_p : process (clkfbm1_ht, clkfbm1_lt, clkfb_frac, clkfb_frac_en, clkfbm1pmf_sel, clkfbm1_div)
...
--     clk0_div_frac <= clk0_div_frac_v;
  ...
  clkfb_frac_ht_lt_p : process (period_vco_frl, clkfbm1_fht, clkfbm1_flt, clkfbm1r_sel, clkfbm1pm_sel_int) begin
    clkfb_frac_ht <= (period_vco_frl * real(clkfbm1_fht) + (period_vco_frl * real(clkfbm1pm_sel_int)) / 8.0) * 1 ps;
    clkfb_frac_lt <= (period_vco_frl * real(clkfbm1_flt) + (period_vco_frl * real(clkfbm1r_sel)) / 8.0) * 1 ps;
  end process;

  clk0_frac_ht_lt_p : process(period_vco_frl, clk0_fht, clk0_flt, clk0pm_sel_int, clk0pm_rsel) begin
    clk0_frac_ht <= (period_vco_frl * real(clk0_fht) + (period_vco_frl * real(clk0pm_sel_int)) / 8.0) * 1 ps;
    clk0_frac_lt <= (period_vco_frl * real(clk0_flt) + (period_vco_frl * real(clk0pm_rsel)) / 8.0) * 1 ps;
  end process;
   ...
   if (rst_in = '1') then
    ...
    begin
      if (init_done = '1' and fb_delay_found = '1') then
       ...
       md_product <= real2int(clkfbm1_div_t * clkind_div_rl + 0.5);
