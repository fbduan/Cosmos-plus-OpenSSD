...
--    CASNXTRDEN_in <= CASNXTRDEN_delay;
    ...
    DINP_in <= CASDINP_delay when ((CASCADE_ORDER_BIN = CASCADE_ORDER_LAST) or
                                   (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) else
               DINP_delay;
    DIN_in <=  CASDIN_delay  when ((CASCADE_ORDER_BIN = CASCADE_ORDER_LAST) or
                                   (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) else
               DIN_delay;
    ...
    RDEN_in  <= CASNXTRDEN_delay and not SLEEP_int when
    ...
    WREN_in <= not (CASPRVEMPTY_delay or FULL_out or SLEEP_int) when
  ...
  mem_rd_en_a  <= (RDEN_in or 
                  ((fill_lat or fill_reg or fill_ecc) and not SLEEP_int))
  ...
  if((xil_attr_test) or
    ...
    case READ_WIDTH is
      when  4   =>
        if (PROG_EMPTY_THRESH >= 1) and (PROG_EMPTY_THRESH <= mem_depth_f) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-125]: PROG_EMPTY_THRESH is not in range 1 .. "));
      ...
      when  9   => 
        if (PROG_EMPTY_THRESH >= 1) and (PROG_EMPTY_THRESH <= mem_depth_f/2) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-126]: PROG_EMPTY_THRESH is not in range 1 .. "));
      ...
      when  18   =>
        if (PROG_EMPTY_THRESH >= 1) and (PROG_EMPTY_THRESH <= mem_depth_f/4) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-127]: PROG_EMPTY_THRESH is not in range 1 .. "));
      ...
      when  36   =>
        if (PROG_EMPTY_THRESH >= 1) and (PROG_EMPTY_THRESH <= mem_depth_f/8) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-128]: PROG_EMPTY_THRESH is not in range 1 .. "));
      ...
      when  72   =>
        if (PROG_EMPTY_THRESH >= 1) and (PROG_EMPTY_THRESH <= mem_depth_f/16) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-129]: PROG_EMPTY_THRESH is not in range 1 .. "));
    ...
    case WRITE_WIDTH is
      when  4   =>
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-131]: PROG_FULL_THRESH is not in range 1 .. "));
          Write ( Message, mem_depth_f);
          Write ( Message, string'("."));
          assert FALSE report Message.all severity error;
          DEALLOCATE(Message);
      ...
      when  9   => 
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f/2) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-132]: PROG_FULL_THRESH is not in range 1 .. "));
          Write ( Message, mem_depth_f/2);
          Write ( Message, string'("."));
          assert FALSE report Message.all severity error;
          DEALLOCATE(Message);
      ...
      when  18   =>
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f/4) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-133]: PROG_FULL_THRESH is not in range 1 .. "));
          Write ( Message, mem_depth_f/4);
          Write ( Message, string'("."));
          assert FALSE report Message.all severity error;
          DEALLOCATE(Message);
      ...
      when  36   =>
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f/8) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-134]: PROG_FULL_THRESH is not in range 1 .. "));
          Write ( Message, mem_depth_f/8);
          Write ( Message, string'("."));
          assert FALSE report Message.all severity error;
          DEALLOCATE(Message);
      ...
      when  72   =>
        if (PROG_FULL_THRESH >= 1) and (PROG_FULL_THRESH <= mem_depth_f/16) then 
          ...
          Write ( Message, string'("Error [Unisim FIFO36E2-135]: PROG_FULL_THRESH is not in range 1 .. "));
          Write ( Message, mem_depth_f/16);
          Write ( Message, string'("."));
          assert FALSE report Message.all severity error;
          DEALLOCATE(Message);
   ...
   SLR : process (mem_rd_clk_a) begin
     if (rising_edge(mem_rd_clk_a)) then
       if ((glblGSR = '1') or (SLEEP_ASYNC_BIN = SLEEP_ASYNC_TRUE)) then
          SLEEP_reg1 <= '0';
          SLEEP_reg <= '0';
       else
          SLEEP_reg1 <= SLEEP_in;
          SLEEP_reg <= SLEEP_reg1;
       end if;
     end if;
     end process SLR;

   SLEEP_int <= SLEEP_reg1 or SLEEP_in;
...
-- cascade out
   ...
   CASDOUTP_out <= DOUTP_out when ((CASCADE_ORDER_BIN = CASCADE_ORDER_FIRST) or
                            (CASCADE_ORDER_BIN = CASCADE_ORDER_PARALLEL) or
                            (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) else (others => '0');
   CASNXTEMPTY_out <= EMPTY_out or SLEEP_int when ((CASCADE_ORDER_BIN = CASCADE_ORDER_FIRST) or
                            (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) else '0';
   CASPRVRDEN_out  <= not (CASPRVEMPTY_in or FULL_out or SLEEP_int) when 
                          ((CASCADE_ORDER_BIN = CASCADE_ORDER_LAST ) or
                           (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) else '0';
...
-- output register
   mem_a_reg_mux  <= CASDINA_in when
                     (((CASCADE_ORDER_BIN = CASCADE_ORDER_LAST) or
                      ...
                      CASOREGIMUXA_reg = '1') else mem_a_ecc;
   memp_a_reg_mux  <= CASDINPA_in when
                      (((CASCADE_ORDER_BIN = CASCADE_ORDER_LAST) or
                        (CASCADE_ORDER_BIN = CASCADE_ORDER_PARALLEL) or
                        (CASCADE_ORDER_BIN = CASCADE_ORDER_MIDDLE)) and
                       CASOREGIMUXA_reg = '1') else memp_a_ecc;
...
-- when ram not empty and o_latch empty
   ...
   OLF  : process (mem_rd_clk_a) begin
     if (falling_edge(mem_rd_clk_a)) then
       if ((glblGSR = '1') or (RDRST_int = '1') or (SLEEP_int = '1')) then
          fill_lat <= '0';
   ...
   MRA  : process
     ...
     if (rising_edge(mem_rd_clk_a) or INIT_MEM='1') then
       if (glblGSR = '1' or INIT_MEM='1') then
         ...
         first_read <= '0';
       elsif ((SLEEP_int = '1') and mem_rd_en_a = '1') then
         Write ( Message, string'("Error : [Unisim "));
         Write ( Message, string'(MODULE_NAME));
