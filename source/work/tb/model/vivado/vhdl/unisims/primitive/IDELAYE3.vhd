...
architecture IDELAYE3_V of IDELAYE3 is
  ...
  constant INTRINSIC_FINE_DELAY	: time := 75 ps;
  constant INTRINSIC_MEDIUM_DELAY	: time := 40 ps;
  constant IDATAIN_INTRINSIC_DELAY : time := 20 ps;
  constant DATAIN_INTRINSIC_DELAY : time := 60 ps;
  constant CASC_IN_INTRINSIC_DELAY : time := 60 ps;
  constant CASC_RET_INTRINSIC_DELAY  : time := 60 ps;
  constant DATA_OUT_INTRINSIC_DELAY : time := 25 ps;
  constant CASC_OUT_INTRINSIC_DELAY : time := 80 ps;
...
--  DATAOUT <= DATAOUT_out after OUT_DELAY;
  ...
  if((xil_attr_test) or
    ...
    -------- DELAY_VALUE check
    if ((DELAY_VALUE < 0) or (DELAY_VALUE > 1250)) then
      attr_err := true;
      Write ( Message, string'("Error : [Unisim "));
      Write ( Message, string'(MODULE_NAME));
  ...
  if((xil_attr_test) or
    ...
    if ((REFCLK_FREQUENCY < 200.0) or (REFCLK_FREQUENCY > 2400.0)) then
      attr_err := true;
      Write ( Message, string'("Error : [Unisim "));
      Write ( Message, string'(MODULE_NAME));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-110] SIM_VERSION attribute is set to "));
  ...
  if((xil_attr_test) or
    ...
    Write ( Message, string'("Error : [Unisim "));
    Write ( Message, string'(MODULE_NAME));
    Write ( Message, string'("-111] UPDATE_MODE attribute is set to """));
       ...
       if((glblGSR = '1') or (FIRST_TIME) or (RST_in = '1'))then
          ...
          FIRST_TIME   := false;
		     ...
		     case DELAY_TYPE_BIN is
				     ...
				     case bcat is
				       ...
				       when "110" =>
						 ...
						 DEALLOCATE (Message);
					         Write ( Message, string'("[Unisim "));
      						 Write ( Message, string'(MODULE_NAME));
      						 Write ( Message, string'("-1] Invalid Scenario.LOAD = 1, CE = 1 INC = 0 is valid only for UPDATE_MODE = MANUAL and DELAY_TYPE = VAR_LOAD"));
      						 Write ( Message, string'("Instance "));
      						 Write ( Message, string'(IDELAYE3_V'INSTANCE_NAME));
         				         assert false
				                 report Message.all
						 ...
						 DEALLOCATE (Message);
					       ...
					       if (UPDATE_MODE /= "MANUAL") then
					       	 Write ( Message, string'("[Unisim "));
      						 Write ( Message, string'(MODULE_NAME));
      						 Write ( Message, string'("-2] Invalid Scenario.LOAD = 1, CE = 1 INC = 1 is valid only for UPDATE_MODE = MANUAL and DELAY_TYPE = VAR_LOAD"));
      						 Write ( Message, string'("Instance "));
      						 Write ( Message, string'(IDELAYE3_V'INSTANCE_NAME));
         				         assert false
				                 report Message.all
				       ...
				       when others =>  Write ( Message, string'("[Unisim "));
      						       Write ( Message, string'(MODULE_NAME));
						       Write ( Message, string'("-3] Invalid Scenario LOAD  = "));
			     ...
			     when others => Write ( Message, string'("[Unisim "));
      					    Write ( Message, string'(MODULE_NAME));
					    Write ( Message, string'("-5] Attribute Syntax Error : Legal values for DELAY_TYPE on IDELAYE3 instance are FIXED or VAR_LOAD or VARIABLE."));
			     		    assert false
					    report Message.all
         				    severity Failure;
					    DEALLOCATE (Message);
...
--####################################################################
  ...
  begin
	  ...
	  cntvalue_upper_nibble := SLV_TO_INT(cntvalue(8 downto 3));
            delay_value_calc_format := ((cntvalue_updated * 5.0)* 1 ps) + INTRINSIC_FINE_DELAY + INTRINSIC_MEDIUM_DELAY;
