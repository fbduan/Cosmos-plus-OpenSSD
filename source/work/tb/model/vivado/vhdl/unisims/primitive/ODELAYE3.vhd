...
architecture ODELAYE3_V of ODELAYE3 is
  ...
  constant ODATAIN_INTRINSIC_DELAY : time := 60 ps;
...
--####################################################################
  ...
  variable idelay_count_var : integer := ODELAYE3_InitDelayCount(DELAY_FORMAT,DELAY_VALUE);
       ...
       if((glblGSR = '1') or (FIRST_TIME) or (RST_in = '1'))then
          ...
          FIRST_TIME   := false;
		     ...
		     case DELAY_TYPE_BIN is
			     ...
			     when others => Write ( Message, string'("[Unisim "));
      					    Write ( Message, string'(MODULE_NAME));
      					    Write ( Message, string'("-5] Attribute Syntax Error : Legal values for DELAY_TYPE on ODELAYE3 instance are FIXED or VAR_LOAD or VARIABLE."));
      					    Write ( Message, string'("Instance "));
      				  	    Write ( Message, string'(ODELAYE3_V'INSTANCE_NAME));
      					    assert FALSE
      					    report Message.all
