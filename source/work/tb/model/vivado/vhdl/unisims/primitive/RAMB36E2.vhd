...
-- Parameter encodings and registers
    ...
    signal t_coll_max : time := 2951 ps;
...
-- input output assignments
    ...
    DINPADINP_in <= CASDINPA_delay when (((CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_LAST) or
                    (CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_MIDDLE)) and CASDIMUXA_in = '1') else
                    DINPADINP_delay;
    DINADIN_in  <=  CASDINA_delay  when (((CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_LAST) or
                    (CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_MIDDLE)) and CASDIMUXA_in = '1') else
                    DINADIN_delay;
    DINPBDINP_in <= CASDINPB_delay when (((CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_LAST) or
                    (CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_MIDDLE)) and CASDIMUXB_in = '1') else
                    DINPBDINP_delay;
    DINBDIN_in  <=  CASDINB_delay  when (((CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_LAST) or
                    (CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_MIDDLE)) and CASDIMUXB_in = '1') else
                    DINBDIN_delay;
...
-- determine clk period for collision window.
  ...
  ECCPARITY_out <= eccparity_reg;
   ...
   dbit_ecc    <= CASINDBITERR_in when (((CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_FIRST) or
   ...
   sbit_ecc    <= CASINSBITERR_in when (((CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_FIRST) or
                                         (CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_MIDDLE)) and
                                        (CASOREGIMUXA_reg = '1')) else
...
-- output register
   mem_a_reg_mux  <= CASDINB_in & CASDINA_in when
                     (((CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_LAST) or
                       (CASCADE_ORDER_A_BIN = CASCADE_ORDER_A_MIDDLE)) and
   ...
   mem_b_reg_mux  <= CASDINB_in when (((CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_LAST) or
                                       (CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_MIDDLE)) and
                                      CASOREGIMUXB_reg = '1') else mem_b_out;
   memp_b_reg_mux  <= CASDINPB_in when (((CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_LAST) or
                                       (CASCADE_ORDER_B_BIN = CASCADE_ORDER_B_MIDDLE)) and
...
-- bit err reg
   ...
   MRAWF : process (wr_a_event, INIT_MEM) begin
     if (ADDR_IS_VALID(rd_addr_a_slv) and 
         (coll_rd_a_wr_b = '0' and coll_wr_b_rd_a = '0' and coll_wr_b_rd_a_sim = '0')) then
...
--         (coll_rd_b_wr_a = '0' and coll_rd_b_wr_a_sim = '0' and coll_wr_a_rd_b = '0'
         (coll_rd_b_wr_a = '0' and coll_wr_a_rd_b = '0' and coll_wr_a_rd_b_sim = '0')) then
   ...
   MRB  : process
     ...
     if (rising_edge(mem_rd_clk_b) or INIT_MEM='1') then
       ...
       elsif ((SLEEP_int = '1') and mem_rd_en_b = '1') then
         Write ( Message, string'("Error : [Unisim "));
         Write ( Message, string'(MODULE_NAME));
