...
begin 
   ...
   begin
        ...
        if ((init41h_tmp(15 downto 12)="0011") and (init40h_tmp(8)='1') and (init40h_tmp_chan /= 3 ) and (init40h_tmp_chan < 16)) then
          assert false report " Warning: [Unisim MODULE_NAME-1] The attribute INIT_40 bit[8] must be set to 0. Long acquistion mode is only allowed for external channels."
          severity warning;
        end if;

        if ((init41h_tmp(15 downto 12) /="0011") and (init4eh_tmp(10 downto 0) /= "00000000000") and (init4eh_tmp(15 downto 12) /= "0000")) then
           assert false report " Warning: [Unisim MODULE_NAME-2] The attribute INIT_4E Bit[15:12] and bit[10:0] must be set to 0. Long acquistion mode is only allowed for external channels."
          severity warning;
        end if;

        if ((init41h_tmp(15 downto 12)="0011") and (init40h_tmp(13 downto 12) /= "00") and (INIT_48 /=X"0000") and (INIT_49 /= X"0000")) then
           assert false report " Warning: [Unisim MODULE_NAME-3] The attribute INIT_48 and INIT_49 must be set to 0000h in single channel mode and averaging enabled."
          severity warning;
        end if;

        if (init42h_tmp(1 downto 0) /= "00") then
             assert false report
             " Error: [Unisim MODULE_NAME-4] The attribute INIT_42 Bit[1:0] must be set to 00."
              severity Error;
        end if;

        if (INIT_45 /= "0000000000000000") then
             assert false report
             " Warning: [Unisim MODULE_NAME-5] The attribute INIT_45 must   be set to 0000."
             severity warning;
        end if;

        if (not(IS_DCLK_INVERTED = '0' or IS_DCLK_INVERTED = '1')) then
          assert false report " Error: [Unisim MODULE_NAME-6] The attribute IS_DCLK_INVERTED must be set to 0 or 1."
            severity failure;
        end if;

        
        if (not(IS_CONVSTCLK_INVERTED = '0' or IS_CONVSTCLK_INVERTED = '1')) then
          assert false report " Error: [Unisim MODULE_NAME-7] The attribute IS_CONVSTCLK_INVERTED must be set to 0 or 1."
            severity failure;
        end if;
       ...
       if (den_in = '1') then
         if (drdy_out_tmp1 = '0') then
            ...
            if (daddr_in /= daddr_in_lat) then
              Write ( Message, string'(" Warning: [Unisim MODULE_NAME-10] Input pin DEN on SYSMONE1 can not continue set to high. Need wait DRDY high and then set DEN high again."));
              assert false report Message.all  severity warning;
              DEALLOCATE(Message);
            end if;
          end if;
        ...
        if (den_in = '1') then
           ...
           if (valid_daddr) then
               ...
               if (address > 173 or (address > 165 and address < 168) or (address > 133 and address < 160) or (address > 121 and address < 128) or (address > 117 and address < 120) or (address > 109 and address < 115) or (address > 101 and address < 104) or (address > 46 and address < 60) or (address > 50 and address < 56) or address = 43 or address = 47) then
                 Write ( Message, string'("Warning : [Unisim "));
                 Write ( Message, string'(MODULE_NAME));
                 Write ( Message, string'("-11] The DADDR "));
        ...
        if (dwe_in = '1' and en_data_flag = 1) then  
           if (valid_daddr and address >= 64 and address <= 95) then
            ...
            if ( address = 66 and  di_in( 2 downto 0) /= "000") then
              ...
              Write ( Message, string'(MODULE_NAME));
              Write ( Message, string'("-12] The DI bit[2:0] "));
           ...
           if ( (address >= 115 and address <= 117) and di_in(15 downto 0) /= "0000000000000000") then
             Write ( Message, string'("Error : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-13] The DI value "));
          ...
          if (address = 64) then
           ...
           if ((( tmp_value = 7) or ((tmp_value >= 10) and (tmp_value <= 15)))) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-14] The DI bit[5:0] at DADDR "));
             Write ( Message, bit_vector'(TO_BITVECTOR(daddr_in)));
           ...
           if ((cfg_reg1(15 downto 12)="0011") and (di_in(8)='1') and (tmp_value /= 3) and (tmp_value < 16)) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-15] The DI value is "));
             Write ( Message, bit_vector'(TO_BITVECTOR(di_in)));
             Write ( Message, string'(" at DADDR "));
             Write ( Message, bit_vector'(TO_BITVECTOR(daddr_in)));
           ...
           if ((cfg_reg1(15 downto 12)="0011") and (di_in(13 downto 12)/="00") and (seq_chan_reg1 /= X"0000") and (seq_chan_reg2 /= X"0000") and (seq_chan_reg3 /= X"0000")) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-16] The Control Regiter 46h, 48h and 49h are "));
          ...
          if (address = 65 and en_data_flag = 1) then
           ...
           if ((di_in(15 downto 12)="0011") and (cfg_reg0(8)='1') and (tmp_value /= 3) and (tmp_value < 16)) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-17] The Control Regiter 40h value is "));
           ...
           if ((di_in(15 downto 12) /= "0011") and (seq_acq_reg1(10 downto 0) /= "00000000000") and (seq_acq_reg1(15 downto 12) /= "0000")) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-18] The Control Regiter 4Eh is "));
           ...
           if ((di_in(15 downto 12) = "0011") and (cfg_reg0(13 downto 12) /= "00") and (seq_chan_reg1 /= X"0000") and (seq_chan_reg2 /= X"0000") and (seq_chan_reg3 /= X"0000")) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-19] The Control Regiter 46h, 48h and 49h are "));
             Write ( Message, bit_vector'(TO_BITVECTOR(seq_chan_reg3)));
             Write ( Message, string'(",  "));
             Write ( Message, bit_vector'(TO_BITVECTOR(seq_chan_reg1)));
             Write ( Message, string'(" and  "));
   ...
   begin
     if (rst_in = '1') then
        ...
        if (acq_chan_index = 7 or (acq_chan_index >= 10 and acq_chan_index <= 12)) then
             Write ( Message, string'("Warning : [Unisim "));
             Write ( Message, string'(MODULE_NAME));
             Write ( Message, string'("-20] The mn channel "));
             Write ( Message, acq_chan_index);
     ...
     if (rst_in = '1' or rst_lock = '1' ) then 
       ...
       if ( curr_seq1_0_lat = "0011" ) then
          seq_count_a <= 1;
           ...
           if (seq_count_a >= 37 or seq_count_a >= seq_num) then
                 seq_count_a <= 1;
           else
        ...
        if (rst_in = '1' or rst_lock = '1') then
            ...
            if (adc_state = S2_ST and rst_lock = '0' and acq_e_c = '0') then 
                ...
                if (next_state = S3_ST) then
                    ...
                    assert false report "Warning: [Unisim MODULE_NAME-21] Acquisition time not enough."
                    severity warning;
                    end if;
                end if;
            else
...
-- end alarm
  ...
  READFILE_P : process
      ...
      type int_a is array (0 to 71) of integer;
  ...
  begin
    ...
    if (open_status /= open_ok) then
         assert false report
         "Warning: [Unisim MODULE_NAME-22] The analog data file for SYSMONE1 was not found. Use the SIM_MONITOR_FILE generic to specify the input analog data file name or use default name: design.txt. "
         severity warning; 
         sim_file_flag1 := '1';
         sim_file_flag <= '1';
    end if;
               ...
               while((in_buf'LENGTH > 0 ) and (not commentline)) loop
                   ...
                   if (HeaderCount=1) then
                      if (str_token(1 to token_len) /= "TIME") then
                         infile_format;
                         assert false report
                           " Error: [Unisim MODULE_NAME-23] No TIME label is found in the input file."
                         severity failure;
                      end if;
                      ...
                      for i in 0 to 35 loop
                       ...
                       if (inchannel = -1 and token_len >0) then
                           infile_format;
                           assert false report
                             "Error: [Unisim MODULE_NAME-24] No valid channel name in the input file for SYSMONE1. Valid names: TEMP VCCINT VCCAUX VCCBRAM VCCPINT VCCPAUX VCCDDRO VP VN VAUXP[1] VAUXN[1] ....."
                           severity failure;
                       end if;
                  else
                       infile_format;
                       assert false report
                         "Error: [Unisim MODULE_NAME-25] Header not found in the input file for SYSMONE1. The header is: TIME TEMP VCCINT VCCAUX VCCBRAM VCCPINT VCCPAUX VCCDDRO VP VN VAUXP[1] VAUXN[1] ..."
                           severity failure;
                  end if;
            ...
            for i IN 1 to HeaderCount Loop
              if ( i=1) then
                ...
                if (not read_ok) then
                  infile_format;
                  assert false report
                   " Error: [Unisim MODULE_NAME-26] The time value should be integer in ns scale and the last time value needs bigger than simulation time."
                  severity failure;
                 end if;
               ...
               if (not read_ok) then
                  assert false report
                    "Error: [Unisim MODULE_NAME-27] The data type should be REAL, e.g. 3.0  0.0  -0.5 "
                  severity failure;
               end if;
