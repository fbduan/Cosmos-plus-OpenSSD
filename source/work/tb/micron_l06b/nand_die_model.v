...
* 7.23 ew          	9/21/09     - Fix: Corrected bug where polling status during tDBSY of a muliplane read command would reset the selected planes
                                    ...
                                    - MU_ONLY: Added On-the-fly SLC functionality to switch between SLC/MLC modes
...
* 7.26_rel ew          11/17/10     - Fix: Pgm Clear now clears only the selected plane of the selected die
                                    ...
                                    - MU_ONLY: Added MLBi Get/Set trim functionality
                                    - MU_ONLY: Added Set Feature 85h: User Selectable Trim Profile- Reload Default Trim Set functionality
                                    - MU_ONLY: Added RB Pulse functionality (Set Feature 94h)
...
* 7.27 ew, rv            2/7/11     - Fix: Giving Reset during Erase operation did not disable scheduled event where RB# returns ready after tBERS time
                                    ...
                                    - Added ONFI 3.0 Synchronous Enhanced = NV-DDR2 Interface
                                    - Added Complementary DQS set feature option
                                    - Testbench: Modified tasks such as latch_command/address/data for Sync Enhanced Mode
                                    - For this release, Timing Checks are not written for Sync Enhanced. 
                                    - Timing Checks for Sync Enhanced Mode will be added in Next Release
                                    - MU_ONLY: Added Erase Suspend and Resume commands
* 7.28 ew               9/19/11     - Added tWB and tWRCK check; fixed sync interface tRHW check
                                    ...
                                    - Added ONFI 3.0 Synchronous Enhanced = NV-DDR2 Interface
                                    - Added Complementary DQS set feature option
                                    - Added ONFI 3.0 Volume Select function
                                    - Added ONFI 3.0 Warmup cycles set feature option
                                    - Added Timing Checks for ONFI 3.0 Sync Enhanced = NV-DDR2 Interface
                                    - Testbench: Modified tasks such as latch_command/address/data for Sync Enhanced Mode
                                    - Testbench: added daisy chain example under `DAISY_CHAIN
                                    - MU_ONLY: Updated On-the-fly SLC functionality for 3BPC B-series chips
                                    - MU_ONLY Fix: Erase or multidie status command following set trim incorrectly enabled all dies if row addr bit 15 was high
* 7.29 ew               6/30/12     - Fix: Reading multiple status bytes in edo mode yielded xx after first byte
				    - Testbench: switching async timing mode will correctly update tb clock period
                                    ...
                                    - MU_ONLY: Added Read Page with Soft Information command
                                    - MU_ONLY: Added SDPC Calibration Table commands
* 7.30 ew                8/7/12     - Added previous owner's (jjc) sv updates for dynamic memory solution
                                    - MU_ONLY fix: D2h command did not reset WP status bit[7] when previous program/erase operation was locked
                                    - MU_ONLY: Added Factory Bad Block Tagging information read on OTP block
* 7.30 karthik                      - fixed the tCAD voilation during data pause during input and output
                                    - fix the tWHR and tRHW voilation reported with l74a model when executing multiple die and rolled over 
                                    - fix for tWB error issue when status or status enhanced is issued.
				    - fix to MLBI read during Sync mode
				    - fix the check for the Parameter to not make use of the address cycle for 78h command during status check
				    - fix the change row address (85h) to not update the active plane if the previous command was 80h
				    - implemented Read Retry
				    - implemeneted feature 30h
				    
      Jay               06/22/2013  - implemented the fix to model which fix the issue of passing 85h command after set feature.
                                    - fixed the issues with respect to tCKL and tcK voilations during data pause         
      karthik		07/16/2013  - fixed issue of status getting cleared on volume select ('hE1) command
      Jay               03/21/2014  - Added ONFI 4.0 = NV-DDR3 Interface 
                                    - Added Timing Checks for ONFI 4.0 = NV-DDR3 Interface 
				    - Updated feature address 01h nv_ddr3 timing mode 
				    - Testbench: Modified tasks such as latch_command/address/data for NV_DDR3 Mode      	    
--------------------------------------------------------------------------- */
`timescale 1ns / 1ps
...
//
parameter CMD_ONFI4	=   4'h7; 
parameter CMD_ONE_PASS	=   4'h8; 
//----------------------------------
parameter mds = 4'b0000;
//------------------------------------
...
reg [(BPC_MAX*DQ_BITS) - 1 : 0] data_reg_packed0_lower[0 : NUM_COL - 1]; //data register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] data_reg_packed1_lower[0 : NUM_COL - 1]; //data register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] data_reg_packed2_lower[0 : NUM_COL - 1]; //data register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] data_reg_packed3_lower[0 : NUM_COL - 1]; //data register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] cache_reg_packed0_lower[0 : NUM_COL - 1]; //cache register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] cache_reg_packed1_lower[0 : NUM_COL - 1]; //cache register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] cache_reg_packed2_lower[0 : NUM_COL - 1]; //cache register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] cache_reg_packed3_lower[0 : NUM_COL - 1]; //cache register for each plane
reg [(BPC_MAX*DQ_BITS) - 1 : 0] mem_array_packed [0 : NUM_ROW - 1] [0 : NUM_COL - 1]; //Main flash memory array
`endif 
reg [PAGE_SIZE - 1 : 0]         data_reg[0:NUM_PLANES -1]; //data register to store SLC or upper page for each plane
reg [PAGE_SIZE - 1 : 0]         cache_reg[0:NUM_PLANES -1]; //cache register to store SLC or upper page for each plane
reg [PAGE_SIZE - 1 : 0]         data_reg_lower[0:NUM_PLANES -1]; //data register to store low page for each plane
reg [PAGE_SIZE - 1 : 0]         cache_reg_lower[0:NUM_PLANES -1]; //cache register to store upper page for each plane
...
reg [DQ_BITS - 1 : 0]		assign_volume_address; // this is the volume address assigned to this volume
reg				volume_assignment_done; // flag tell us if vol select feature is enabled
reg				volume_selected; // flag tells us if this volume is selected
reg				cmnd_E1h; // flag tells us if last command is E1h
reg				first_cmd_after_ce; // used in E1h cmd timing check
wire				Ce_n_vol; // Ce_n qualified with volume selected  
reg				compl_dqs_en;
reg [7:0]			num_warmup_din;
reg [7:0]			num_warmup_dout;
reg [7:0]			num_warmup_remain;
reg				din_ODT_en;
reg				dout_ODT_en;
reg				first_tDSC; // used in SYNC_ENH timing checks
reg				first_tRC; // used in SYNC_ENH timing checks
reg				any_din_sync_enh; // used in SYNC_ENH timing checks, similar to datain_sync_enh, but it looks at other dies as well
reg				any_dout_sync_enh; // used in SYNC_ENH timing checks and datain_sync_enh, similar to data_out_enable_sync_enh, but it looks at other dies as well
reg				any_din_nv_ddr3; // used in NV_DDR3 timing checks, similar to datain_sync_enh, but it looks at other dies as well
reg				any_dout_nv_ddr3; // used in NV_DDR3 timing checks and datain_sync_enh, similar to data_out_enable_sync_enh, but it looks at other dies as well
...
reg [ROW_BITS -1 : 0]           row_addr_lower [0:NUM_PLANES -1]; //decoded row address
reg [ROW_BITS -1 : 0]           pgm_row_addr [0:NUM_PLANES -1];//save row address for program 
reg [ROW_BITS -1 : 0]           pgm_sus_addr [0:NUM_PLANES -1];
reg [ROW_BITS -1 : 0]           pgm_res_addr [0:NUM_PLANES -1];
reg [ROW_BITS -1 : 0]           pgm_row_addr_lower [0:NUM_PLANES -1];//save row address for program 
reg [ROW_BITS -1 : 0]           pgm_sus_addr_lower [0:NUM_PLANES -1];
reg [ROW_BITS -1 : 0]           pgm_res_addr_lower [0:NUM_PLANES -1];
...
reg [PAGE_BITS -1 : 0]          page_addr_bk [0:NUM_PLANES -1];
reg   load_cache_en;
reg   load_cache_en_r;
reg   ld_reg_en;
integer ldthisPlane;
...
reg [BLCK_BITS -1 : 0] ld_load_block_addr;
reg [1:0] first_mp_cmd;
reg [ROW_BITS -1 : 0]array_cache_addr;
...
reg erase_blk_pls ;
integer erase_data;
reg [BLCK_BITS -1 : 0] erase_susp_blk_addr [0:NUM_PLANES -1]; // blk addr of erase suspended blk
reg erase_susp_flag;
...
//-----------------------------------------------
reg             cmnd_ignore;         // for L06 Chip
reg             cmnd_31h_first;
reg             set_get_feature_lun_1addr;
reg             set_get_feature_lun_addr_ok; 

reg [31:0]      set_ftr_data;
reg [7:0]       lastpgmcmd; // added for tracking change row cmd within pgm cmds
reg [7:0]       lastpgmcmd_85h; // added for tracking change row cmd within copyback pgm cmds
reg             cache_check; // this is to make sure only the least queued plane outputs the data in the cache operation
...
reg [NUM_PLANES >> 2:0] pgm_plane;
reg [NUM_PLANES >> 2:0] pgm_sus_plane;
reg [NUM_PLANES >> 2:0] pgm_res_plane;
reg [NUM_PLANES >> 2:0] cache_rd_active_plane; // used to designate which plane should be outputting data in read cache
reg [NUM_PLANES >> 2:0] least_active_mp_plane; // used to designate which plane should be outputting data on data read in multiplane
reg [NUM_PLANES >> 2:0] prev_least_active_mp_plane;
reg queued_plane [0: NUM_PLANES-1];
reg queued_pgm_plane [0: NUM_PLANES-1];
reg queued_sus_plane [0: NUM_PLANES-1];
reg queued_res_plane [0: NUM_PLANES-1];
reg current_plane [0: NUM_PLANES-1];
...
wire            dqs_out_enable;
reg		datain_sync_enh;
reg		datain_nv_ddr3; //ONFI4
reg		data_out_enable_sync_enh;
reg		data_out_enable_nv_ddr3; //ONFI4
...
// program suspend
//----------------------------------------
reg                     op_pgm;//go high when program
reg                     op_pgm_sus;//go high when successfully suspend
reg                     flag_pgm_sus;//go high when suspended, go low when resumed or reset
reg                     op_pgm_res;
reg                     op_pgm_cache;
reg                     sus_pgm_cache;
...
//-----------------------------------------
// partial page read and express read
//-----------------------------------------
reg                     express_rd_en;
reg                     partial_pg_rd_en;
reg    [15:0]           partial_pg_rd_len;
...
reg                     mirror_mode;
reg                     first_cmd;
wire  [DQ_BITS - 1 : 0] Io_mirror;

`ifdef x8
assign   Io_mirror  = (mirror_mode== 1'b1) ?  {Io[0],Io[1],Io[2],Io[3],Io[4],Io[5],Io[6],Io[7]} : Io;
`else
assign   Io_mirror  = (mirror_mode== 1'b1) ? {Io[0],Io[1],Io[2],Io[3],Io[4],Io[5],Io[6],Io[7],Io[8],Io[9],Io[10],Io[11],Io[12],Io[13],Io[14],Io[15]} : Io;
`endif					   
...
realtime    tm_vol_sel	=   0;       //ONFI3 only : volume selected timestamp
realtime    tm_eni_r	=   0;       //ONFI3 only : ENi rise timestamp
real	    tDSC_sync_enh=  0;       //ONFI3 only : calculate DQS clock period
real	    tRC_sync_enh=   0;       //ONFI3 only : calculate RE clock period

real	    tDSC_nv_ddr3=  0;       //ONFI4 only : calculate DQS clock period
real	    tRC_nv_ddr3=   0;       //ONFI4 only : calculate RE clock period

//Continuous Assignments
assign Io = Io_wire;
assign Dqs = Dqs_wire;  //sync mode only
assign Dqs_c = ((Dqs_buf === 1'bz) || (~compl_dqs_en)) ? 1'bz : ~Dqs_buf;
assign Dqs_wire = (dqs_en) ? Dqs_buf : 1'bz;   //sync mode only
`ifdef x8
assign Io_wire = rd_out ? (mirror_mode == 1'b1) ? {Io_buf[0],Io_buf[1],Io_buf[2],Io_buf[3],Io_buf[4],Io_buf[5],Io_buf[6],Io_buf[7]} : Io_buf : IoX ? {DQ_BITS{1'bx}} : {DQ_BITS{1'bz}};
`else
assign Io_wire = rd_out ? (mirror_mode == 1'b1) ? {Io_buf[0],Io_buf[1],Io_buf[2],Io_buf[3],Io_buf[4],Io_buf[5],Io_buf[6],Io_buf[7],Io_buf[8],Io_buf[9],Io_buf[10],Io_buf[11],Io_buf[12],Io_buf[13],Io_buf[14],Io_buf[15]}
                                                : Io_buf : IoX ? {DQ_BITS{1'bx}} : {DQ_BITS{1'bz}};
`endif					      
...
initial begin
    ...
    datain_nv_ddr3      =   1'b0; //ONFI4
    data_out_enable_sync_enh = 1'b0;
    data_out_enable_nv_ddr3  = 1'b0; //ONFI4
    clr_que_en_rd       =   1'b0;
    clr_que_en_wr       =   1'b0;
    ...
    cache_check         =   1'b0;
    express_rd_en       =   1'b0;
    partial_pg_rd_en    =   1'b0;
    partial_pg_rd_len   =   16'h0;
    mirror_mode         =   1'b0;
    first_cmd           =   1'b1;
    vpp_en              =   1'b0;
    sleeplite_en        =   1'b0;
	...
	pgm_res_addr[i] = 0;
        row_addr_lower[i] = 0;
	pgm_row_addr_lower[i] = 0;
	pgm_sus_addr_lower[i] = 0;
	pgm_res_addr_lower[i] = 0;
    end
    clear_queued_planes;
    ...
    least_active_mp_plane = 2'b0;
    prev_least_active_mp_plane = 2'b0;  
    lastpgmcmd          =   8'h0;
    lastpgmcmd_85h      =   8'h0;
    ...
    ENo			=   1'b0;
    volume_assignment_done =1'b0;
    volume_selected	=   1'b0;
    cmnd_E1h		=   1'b0;
    first_cmd_after_ce	=   1'b0;
    compl_dqs_en        =   1'b0;
    num_warmup_din	=   8'h0;
    num_warmup_dout	=   8'h0;
    num_warmup_remain	=   8'h0;
    din_ODT_en		=   1'b0;
    dout_ODT_en		=   1'b0;
    first_tDSC		=   1'b1;
    first_tRC		=   1'b1;
    any_din_sync_enh	=   1'b0;
    any_dout_sync_enh	=   1'b0;

    any_din_nv_ddr3	=   1'b0; //ONFI4
    any_dout_nv_ddr3	=   1'b0; //ONFI4
    set_get_feature_lun_1addr = 1'b0;
    set_get_feature_lun_addr_ok =1'b0;
//added for program suspend
    op_pgm              =   1'b0;
    op_pgm_sus          =   1'b0;
    op_pgm_res          =   1'b0;
    flag_pgm_sus        =   1'b0;
...
`endif
    ...
    config_6add = (LUN_BITS==3);
    //Determine how many address cycles we need
    if (ROW_BITS > 24) ROW_BYTES = 4;
    else if (ROW_BITS > 16) ROW_BYTES = 3;
...
begin
  ...
  $display("%m at time %t: %0s", $time, msg);//xxzxxz
end
endtask
...
task corrupt_block;
    ...
    integer i;
begin
    if (DEBUG[0]) begin $sformat(msg, "Corrupting block addr=%0h due to reset, erase suspend, or illegal on-the-fly slc programming", tsk_block_addr); INFO(msg); end
`ifdef MODEL_SV   
        page = 0;
...
begin
    page_type  = 2'b00;
    // get page type
    if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin    
      for (num_plane=0; num_plane < NUM_PLANES; num_plane=num_plane+1) begin
     	   if (queued_plane[num_plane]) begin 
	      page_type =  page_map_checker(row_addr[num_plane][PAGE_BITS-1:0]);
	      break; // only need get the first page type of queued planes because page address must be same for multi-plane operation	   
   	   end     
      end
    end
    for (num_plane = 0; num_plane < NUM_PLANES; num_plane = num_plane +1) begin
	    ...
	    if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin
	     	if(page_type == 2'b00) begin
             	   data_reg[num_plane] = cache_reg[num_plane];
	     	end
	     	else if (page_type == 2'b01) begin
             	   cache_reg_lower[num_plane] = cache_reg[num_plane];  	
	     	end
	     	else if (page_type == 2'b10) begin
             	   data_reg[num_plane] = cache_reg[num_plane];
             	   data_reg_lower[num_plane] = cache_reg_lower[num_plane];
	     	end
             	`ifdef PACK
	     	   ...
	     	   if(page_type == 2'b00) begin
             		 ...
             		 case (num_plane)
             		     0 : cache_reg_packed0_lower[i] = cache_reg_packed0[i];
             		     1 : cache_reg_packed1_lower[i] = cache_reg_packed1[i];
             		     2 : cache_reg_packed2_lower[i] = cache_reg_packed2[i];
             		     3 : cache_reg_packed3_lower[i] = cache_reg_packed3[i];
             		 endcase
	     	   end
	     	   else if (page_type == 2'b10) begin
             		 case (num_plane)
             		     0 : begin 
	     			   data_reg_packed0[i] = cache_reg_packed0[i];
	     			   data_reg_packed0_lower[i] = cache_reg_packed0_lower[i];
	     			 end
             		     1 : begin
	     			   data_reg_packed1[i] = cache_reg_packed1[i];
	     			   data_reg_packed1_lower[i] = cache_reg_packed1_lower[i];
	     			 end
             		     2 : begin 
	     			   data_reg_packed2[i] = cache_reg_packed2[i];
	     			   data_reg_packed2_lower[i] = cache_reg_packed2_lower[i];
	     			 end
             		     3 : begin 
	     			   data_reg_packed3[i] = cache_reg_packed3[i];
	     			   data_reg_packed3_lower[i] = cache_reg_packed3_lower[i];
	     			 end
             		 endcase	   
	     	   end
             	end
...
//    #tWB_delay;
    ...
    //-----------------------------------------------------------------
    if(!cmnd_31h_first) begin //first cache read do not copy data from data_reg to cache_reg
    for (num_plane = 0; num_plane < NUM_PLANES; num_plane = num_plane +1) begin
...
task load_cache_register;
    ...
    integer delay;
    integer temp_delay;
    reg [BLCK_BITS-1 : 0] load_block_addr;
...
begin
    ...
    for (thisPlane =0; thisPlane < NUM_PLANES; thisPlane=thisPlane+1) begin : plane_loop
	    ...
	    if (partial_pg_rd_en == 1'b1) begin
	       for (int i = 0; i < col_addr; i++) begin
	    	  cache_reg[active_plane][i*8+:8] <= #(temp_delay+1) 8'hFF;
	       end
	       for (int i =(col_addr+partial_pg_rd_len); i < NUM_COL; i++) begin
	    	  cache_reg[active_plane][i*8+:8] <= #(temp_delay+1) 8'hFF;
	       end
	    end
	    if (row_addr[active_plane][BLCK_BITS+PAGE_BITS-1:0] >= OTP_ADDR_MAX) begin
            	$sformat(msg, "Error: OTP Read Address overflow.  Block must be 0 and page < OTP_ADDR_MAX.  Block=%0h Page=%0h  OTP_ADDR_MAX=%0h", row_addr[active_plane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS], row_addr[active_plane][PAGE_BITS-1:0], OTP_ADDR_MAX); ERROR(ERR_OTP, msg); 
            end 
            ...
            if (~do_read_id_2 && ~do_read_unique && queued_plane[thisPlane]) begin : no_id_2
                ...
                load_block_addr = load_row_addr[BLCK_BITS+PAGE_BITS-1:PAGE_BITS];
		    ...
		    if (!multiplane_op_rd) // For Multiplane operation read,make use of the least active plane	
		    ...
		    if (partial_pg_rd_en == 1'b1) begin
		      for (int i = 0; i < col_addr; i++) begin
		     	 cache_reg[thisPlane][i*8+:8] <= #(temp_delay+1) 8'hFF;
		      end
		      for (int i =(col_addr+partial_pg_rd_len); i < NUM_COL; i++) begin
		     	 cache_reg[thisPlane][i*8+:8] <= #(temp_delay+1) 8'hFF;
		      end
    ...
    // -------------------------------------------------------------------------
	    ...
	    tRER = tRER_typ;
	    tRER_vpp = tRER_vpp_typ;
	    tR_typ = tR_max_typ;
	    tR_vpp_typ = tR_vpp_max_typ;
	end
	else begin
	    tRER = tRER_SP_typ;
	    tRER_vpp = tRER_SP_vpp_typ;
	    tR_typ = tR_SP_max_typ;
	    tR_vpp_typ = tR_SP_vpp_max_typ;		
	end
	if (ONFI_read_param == 1'b1 || do_read_unique == 1'b1) begin // only support express read
	   if (express_rd_en == 1'b1 ) begin
	       if (vpp_en == 1'b0)
	       tR_max  = tRER;
	       else
	       tR_max  = tRER_vpp;
        ...
        else if (OTP_read == 1'b1 || (~cache_op)) begin
	   if (express_rd_en == 1'b1 && partial_pg_rd_en == 1'b0) begin
	       if (vpp_en == 1'b0)
	       tR_max  = tRER;
	       else
	       tR_max  = tRER_vpp;
	   end	        
	   else if (express_rd_en == 1'b0 && partial_pg_rd_en == 1'b1) begin
	       if (vpp_en == 1'b0) 
	       tR_max  = tRPP_typ;
	       else
	       tR_max  = tRPP_vpp_typ;
	   end
	   else if (express_rd_en == 1'b1 && partial_pg_rd_en == 1'b1) begin
               if (vpp_en == 1'b0)
	       tR_max  = tRPPER_typ;
	       else
	       tR_max  = tRPPER_vpp_typ;
	   end
	   else begin
	       if (vpp_en == 1'b0) 
	       tR_max  = tR_typ;
	...
	if (read_retry_flag && tr_increased) begin
	   tR_max = tR_max_retry;	
	end
    ...
    if (multiplane_op_rd & ~FEATURE_SET2[CMD_MP_OUTPUT] & (NUM_PLANES > 1)) begin
	  if (lastCmd == 'h30 || lastCmd == 'h35)
	     active_plane = least_active_mp_plane;
	  else 
	     active_plane = prev_least_active_mp_plane;
	  
	  prev_least_active_mp_plane = least_active_mp_plane;
	  cache_rd_active_plane      = prev_least_active_mp_plane;
...
begin
	...
	status_register[7] = 1'b1;
        ...
        for (ldthisPlane =0; ldthisPlane < NUM_PLANES; ldthisPlane=ldthisPlane+1) begin : plane_loop
            if (OTP_read && (ldthisPlane == 0)) begin : otp_read
 		    ...
 		    if (partial_pg_rd_en == 1'b1) begin
		      for (int i = 0; i < col_addr; i++) begin
		    	 cache_reg[active_plane][i*8+:8] = 8'hFF;
		      end
		      for (int i =(col_addr+partial_pg_rd_len); i < NUM_COL; i++) begin
		    	 cache_reg[active_plane][i*8+:8] = 8'hFF;
		      end
	...
	ld_delay = tR_max;
        ...
        if (multiplane_op_rd & ~FEATURE_SET2[CMD_MP_OUTPUT] & (NUM_PLANES > 1)) begin
	   active_plane <= #(ld_delay) least_active_mp_plane;
...
begin
    if (sync_mode && (Clk || (~Clk && saw_posedge_dqs))) begin
        ...
        `endif         
    end else if (sync_enh_mode) begin
        //data transitions from undriven to known value
        Io_buf <= #(tDQSRE_sync_enh_max) {DQ_BITS{1'bx}};
        IoX    <= #(tDQSRE_sync_enh_max) 1'b1;
        //data becomes valid
        rd_out <= #(tDQSRE_sync_enh_max + tDQSQ_sync_enh_max) 1'b1;
        Io_buf <= #(tDQSRE_sync_enh_max + tDQSQ_sync_enh_max) dataOut;

        //data goes back to X as we transition to the next value
        Io_buf <= #(tDQSRE_sync_enh_max + tDQSQ_sync_enh_max + tDVW_sync_enh_min) {DQ_BITS{1'bx}};
        rd_out <= #(tDQSRE_sync_enh_max + tDQSQ_sync_enh_max + tDVW_sync_enh_min) 1'b0;
        t_readtox = ($realtime + tDQSRE_sync_enh_max + tQH_sync_enh_min); 
        // IoX_enable <= #(t_readtox) 1'b1; // only used during async data output
    end else if (nv_ddr3) begin
        //data transitions from undriven to known value
        Io_buf <= #(tDQSRE_nv_ddr3_max) {DQ_BITS{1'bx}};
        IoX    <= #(tDQSRE_nv_ddr3_max) 1'b1;
        //data becomes valid
        rd_out <= #(tDQSRE_nv_ddr3_max + tDQSQ_nv_ddr3_max) 1'b1;
        Io_buf <= #(tDQSRE_nv_ddr3_max + tDQSQ_nv_ddr3_max) dataOut;

        //data goes back to X as we transition to the next value
        Io_buf <= #(tDQSRE_nv_ddr3_max + tDQSQ_nv_ddr3_max + tDVW_nv_ddr3_min) {DQ_BITS{1'bx}};
        rd_out <= #(tDQSRE_nv_ddr3_max + tDQSQ_nv_ddr3_max + tDVW_nv_ddr3_min) 1'b0;
        t_readtox = ($realtime + tDQSRE_nv_ddr3_max + tQH_nv_ddr3_min); 
        // IoX_enable <= #(t_readtox) 1'b1; // only used during async data output
    end
end
endtask
...
task output_status;
begin
    if (((sync_mode && ~Wr_n) || sync_enh_mode || nv_ddr3) && ~Ce_n && die_select && status_cmnd) begin    
    ...
    if (~sync_mode && ~sync_enh_mode && ~nv_ddr3 && ~Re_n && ~Ce_n && We_n && die_select && status_cmnd) begin    
        if (cmnd_78h && disable_md_stat) begin
            $sformat(msg, "MULTI-DIE STATUS READ (78h) IS PROHIBITED DURING AND AFTER POWER-UP RESET, OTP OPERATIONS, READ PARAMETERS, READ ID, READ UNIQUE ID, and GET/SET FEATURES.");
            INFO(msg);
        end else begin
    ...
    if (eb_unlocked_erase) begin : unlocked_erase_block
        ...
        for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin : plane_loop
            if (queued_plane[eb_thisPlane]) begin //only proceed if this plane is queued to be erase
                ...
                if (1) begin $sformat(msg, "ERASE %0s: interleave/plane=%h, Block=%h", (lastCmd == 8'hD2 ? "RESUME" : ""), eb_thisPlane, erase_block_addr); INFO(msg);  end
...
//-----------------------------------------------------------------
// TASK : clear_erase_suspend()
//      clears all regs assiciated with erase suspend
//-----------------------------------------------------------------
task clear_erase_suspend;
begin
    status_register [2] <= 1'b0; // reset erase suspend status bit
    erase_susp_flag <= 1'b0;
    for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
	erase_susp_blk_addr[eb_thisPlane] = {BLCK_BITS{1'bx}};
    end
end
endtask
...
begin
    ...
    if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin    
      for (thisPlane=0; thisPlane < NUM_PLANES; thisPlane=thisPlane+1) begin
	      ...
	      page_type =  page_map_checker(row_addr[thisPlane][PAGE_BITS-1:0]);
	      break; // only need get the first page type of queued planes because page address must be same for multi-plane operation	   
   	   end     
      end
    end
    ...
    pgm_row_addr = row_addr;
    if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1)
    pgm_row_addr_lower = row_addr_lower;
    pgm_plane = active_plane;
    ...
    if (prog_cache_op === 1 && lastCmd != 8'hFF && lastCmd != 8'hFA && lastCmd != 8'hFC && !(FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01)) begin
        // this is for cache mode program ops
        // now wait for delay to transfer cache_reg -> data_reg
        go_busy(tCBSY_min-tWC_min - 100); 
	    ...
	    op_pgm_cache  = 1'b0;
            ...
            Rb_n_int <= # (tWC_min+100) 1'b1;
            //only cache bit in status register changes here
            status_register [6] <= #(tWC_min+100) 1'b1;
        end
    end
    array_prog_done = 1'b0;
    status_register[5] = 1'b0;
    queued_pgm_plane = queued_plane;
    
    if (~copyback2 && lastCmd != 8'hFF && lastCmd != 8'hFA && lastCmd != 8'hFC && op_pgm && !flag_pgm_sus) begin 
        program_page_from_datareg(multiplane); 
    end

   if(lastCmd != 8'h15 && lastCmd != 8'hFF && !( lastCmd == 8'hFA && die_select == 1'b1) && lastCmd != 8'hFC )begin	

	 array_prog_done <= #10 1'b1;
	 otp_prog_done   <= #10 1'b1;
    end

    if(lastCmd != 8'hFF && !( lastCmd == 8'hFA && die_select == 1'b1) && lastCmd != 8'hFC)begin	
          op_pgm <= # 10 1'b0;
	if (force_sts_fail)
	    status_register[0] <= #10 1'b1;
	if (otp_prog_fail)
	    status_register[7] <= #10 1'b0;
...
task program_cache_resume_case2;//program cache resume is just for the page suspended when Rb_n_init is high. 
   integer delay;
begin
   if (vpp_en == 1'b0)
      delay = tPROG_typ;
...
task program_resume_page;
    ...
    integer delay;
    reg [1:0] page_type; //00: SLC page,01: Lower MLC page, 10: Upper MLC page 
begin   
    ...
    if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin    
      for (thisPlane=0; thisPlane < NUM_PLANES; thisPlane=thisPlane+1) begin
     	   if (queued_plane[thisPlane]) begin 
	      page_type =  page_map_checker(pgm_res_addr[thisPlane][PAGE_BITS-1:0]);
	      break; // only need get the first page type of queued planes because page address must be same for multi-plane operation	   
   	   end     
      end
    end
...
for (thisPlane = 0; thisPlane < NUM_PLANES; thisPlane = thisPlane + 1) begin : plane_loop 
            //set up the address to load
                if (queued_res_plane[thisPlane]) begin : queued_plane_section //only proceed if this plane is queued to program & same with previous program 
                    if (NUM_PLANES > 1) begin
                            array_prog_addr = {pgm_res_addr[thisPlane][ROW_BITS-1:(PAGE_BITS+(NUM_PLANES >> 2) +1)], //upper row address bits
                                  thisPlane[(NUM_PLANES >> 2) : 0], //plane address bits
                                  pgm_res_addr[pgm_res_plane][PAGE_BITS-1:0]};  //page address bits defined by last address plane
                    end else begin
                            array_prog_addr = {pgm_res_addr[thisPlane][ROW_BITS-1:PAGE_BITS], //upper row address bits
                                  pgm_res_addr[pgm_res_plane][PAGE_BITS-1:0]};  //page address bits defined by last address plane
                    end
                    if (FEATURE_SET2[CMD_ONE_PASS] == 1'b0 || (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b00)) begin
		    ...
		    else if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b10) begin // MLC upper page
		        if (shared_wordline(pgm_res_addr_lower[thisPlane],pgm_res_addr[thisPlane]) == 1'b0) begin
                            $sformat (msg, "The lower page %h isn't shared the same wordline with the upper page %h",pgm_res_addr_lower[thisPlane], pgm_res_addr[thisPlane]);
                            WARN(msg);
			end
		        for (int j = 1; j >= 0; j--) begin // check lower and upper page that shared a word line
                    	     temp_mem_exist = memory_addr_exists(array_prog_addr -j);
                    	     if (temp_mem_exist == 5)
                    	     	 mem_mp_index = array_prog_addr -j;
                    	     else if (temp_mem_exist == 4) begin
                    	     	 pp_counter[array_prog_addr -j ]  = {4{1'b0}}; //initialize partial page counter
                    	     	 mem_array [array_prog_addr -j ]  = {PAGE_SIZE{erase_data[0]}}; // initialize array row data
                    	     	 `ifdef PACK
                    	     	 for (i = 0; i <= NUM_COL - 1; i = i + 1) begin
                    	     	     mem_array_packed [array_prog_addr- j][i]  = {(DQ_BITS*BPC_MAX){erase_data[0]}}; // initialize array row data
                    	     	 end 
                    	     	 `endif 
                    	     	 mem_mp_index = array_prog_addr- j;
                    	     end else if (temp_mem_exist == 1)
                    	     	 ...
                    	     	 `else
                    	     	 pp_counter[memory_index]  = {4{1'b0}}; //initialize partial page counter
                    	     	 memory_addr[memory_index] = array_prog_addr -j;
                    	     	 mem_array [memory_index]  = {PAGE_SIZE{erase_data[0]}}; // initialize array row data
                    	     	 `ifdef PACK
                    	     	 for (i = 0; i <= NUM_COL - 1; i = i + 1) begin
                    	     	     mem_array_packed [memory_index][i]  = {(DQ_BITS*BPC_MAX){erase_data[0]}}; // initialize array row data
                    	     ...
                    	     //program the array
                    	 `ifdef MODEL_SV
			       if (j == 0)   mem_array[array_prog_addr] =  data_reg[thisPlane] & mem_array [array_prog_addr];
                     	       else  mem_array[array_prog_addr-1] =  data_reg_lower[thisPlane] & mem_array [array_prog_addr-1];
                   	     `ifdef PACK
                    	     for (i = 0; i <= NUM_COL - 1; i = i + 1) begin
                    		 case (thisPlane)
                    		 0 : begin 
	             			if (j == 0) mem_array_packed[mem_mp_index][i] = data_reg_packed0[i] & mem_array_packed [mem_mp_index][i];
	             			else mem_array_packed[mem_mp_index][i] = data_reg_packed0_lower[i] & mem_array_packed [mem_mp_index][i];
	             		     end
                    		 1 : begin 
	             			if (j == 0) mem_array_packed[mem_mp_index][i] = data_reg_packed1[i] & mem_array_packed [mem_mp_index][i];
	             			else mem_array_packed[mem_mp_index][i] = data_reg_packed1_lower[i] & mem_array_packed [mem_mp_index][i];
	             		     end
                    		 2 : begin 
	             			if (j == 0) mem_array_packed[mem_mp_index][i] = data_reg_packed2[i] & mem_array_packed [mem_mp_index][i];
	             			else mem_array_packed[mem_mp_index][i] = data_reg_packed2_lower[i] & mem_array_packed [mem_mp_index][i];
	             		     end
                    		 3 : begin
	             			if (j == 0) mem_array_packed[mem_mp_index][i] = data_reg_packed3[i] & mem_array_packed [mem_mp_index][i];
	             			else mem_array_packed[mem_mp_index][i] = data_reg_packed3_lower[i] & mem_array_packed [mem_mp_index][i];
	             		     end
                    		 endcase
                    	     end
                    	     `endif 
                    	 `else
                    		 ...
                    		 if (j == 0) mem_array[mem_mp_index] =  data_reg[thisPlane] & mem_array [mem_mp_index];
                    		 else mem_array[mem_mp_index] =  data_reg_lower[thisPlane] & mem_array [mem_mp_index];
                    		 `ifdef PACK
                    		 for (i = 0; i <= NUM_COL - 1; i = i + 1) begin
                    		     case (thisPlane)
                    		     0 : begin 
                    	     ...
                    	     `endif
                    	 `endif 		 
                             inc_pp(array_prog_addr -j);
                             check_block(array_prog_addr -j);
		        end		  
		    end // MLC upper page
...
begin
    ...
    if (multiplane) begin
        ...
        if (FEATURE_SET[CMD_2PLANE] && copyback) begin
    ...
    if (erase_susp_flag) begin
	for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
	    if (queued_plane[eb_thisPlane] && erase_susp_blk_addr[eb_thisPlane] == row_addr[eb_thisPlane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS]) begin
                // if this plane has a queued pgm op, check that it is not on ers suspended blk
		unlocked_write = 0;
	    end
	end
    end
    ...
    if (unlocked_write) begin : unlocked_write_command
        ...
        case (lastCmd) 
            8'h10 : begin
                ...
                //else if(cache_prog_last) begin 
		//  delay = tLPROG_cache_typ;
                //end
                else begin
		  if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01) 
		  delay = tPBSY_min-tWC_min;
                  else begin		  
		     if (vpp_en == 1'b0)
		       delay = tPROG_typ - tWC_min;
		     else
		       delay = tPROG_vpp_typ - tWC_min;
		  end		  
		end
            end
            8'h15 : if (vpp_en == 1'b0)
	              delay = tPROG_typ - (tCBSY_min-tWC_min-100);
		    else
		      delay = tPROG_vpp_typ - (tCBSY_min-tWC_min-100);
        ...
        endcase 
     	...
     	if(lastCmd == 8'h15) begin
     	   array_prog_done <= #(delay) 1'b1;
     	end else begin
     	   go_busy(delay-10);//if program suspend is issued, flag_pgm_sus will go high, op_pgm will still keep high
     	end
    ...
    // ----------------------------------------------------------------------------------------------------
		        ...
		        if (shared_wordline(row_addr_lower[thisPlane],row_addr[thisPlane]) == 1'b0) begin
                            $sformat (msg, "The lower page %h isn't shared the same wordline with the upper page %h",row_addr_lower[thisPlane], row_addr[thisPlane]);
                            WARN(msg);
			end
		        for (int j = 1; j >= 0; j--) begin // check lower and upper page that shared a word line
                    	     temp_mem_exist = memory_addr_exists(array_prog_addr -j);
		    ...
		    else if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01) begin // MLC Lower page
                         row_addr_lower[thisPlane]  = row_addr[thisPlane];
		    end
                end // queued_plane_section
            end //page_write
        end //plane_loop
        if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01) begin // MLC Lower page
	    if (cache_op) begin
               Rb_n_int <= 1'b1;   // not busy anymore
               status_register [6:5] <= 2'b11; 
	    end   
	...
	// need active_plane ? 1 : 0 format instead of doing row_addr[active_plane] b/c doing the latter gave us plane1 data when active_plane was 0, race condtion?
        ...
        end else if (erase_susp_flag) begin
            $sformat(msg, "LOCKED: Not Programing Block 0x%0h, page 0x%0h, Erase is Suspended on Block 0x%0h", 
	    	active_plane ? row_addr[1][BLCK_BITS+PAGE_BITS-1:PAGE_BITS] : row_addr[0][BLCK_BITS+PAGE_BITS-1:PAGE_BITS],
		active_plane ? row_addr[1][PAGE_BITS-1:0] : row_addr[0][PAGE_BITS-1:0], active_plane ? erase_susp_blk_addr[1] : erase_susp_blk_addr[0]);
        end else begin
	...
	if (erase_susp_flag)
        ...
        Rb_n_int <= 1'b1;   // not busy anymore
        status_register [6:5] = 2'b11;
        if (LOCK_DEVICE || erase_susp_flag) begin
            status_register [7] = 1'b0;
        end else begin
            status_register [7] = 1'b1;
        end
...
begin
    ...
    if ((~cache_op && ~(queued_copyback2 && ~copyback2))&&(!sus_pgm_cache)) begin
        Rb_n_int <= 1'b1; // Device Ready
        status_register [6] = 1;           
        ...
        if (~nand_mode[0]) status_register[5] <= #(tCBSY_min) 1;
	cache_op <= #(tCBSY_min)1'b0;
	if(op_pgm_cache) op_pgm_cache <= #(tCBSY_min) 1'b0;
        //output_status;
    end else begin
        cache_prog_last = 1'b0;
        //output_status;
...
begin
    ...
    if ((lastCmd !== 8'h3F) && (lastCmd !== 8'h31)) begin
	   ...
	   if (express_rd_en && ! (read_retry_flag && tr_increased)) begin
	      if (vpp_en == 1'b0) begin
	         if( partial_pg_rd_en) status_register[5] <= #(tRPP_typ - tRPPER_typ) 1;
	         else status_register[5] <= #(tR_typ - tRER) 1;
	      end
	      else begin
	         if( partial_pg_rd_en) status_register[5] <= #(tRPP_vpp_typ - tRPPER_vpp_typ) 1;
	         else status_register[5] <= #(tR_vpp_typ - tRER_vpp) 1;	      
	      end	   
	   end
	   else begin
	      status_register[5] = 1;
...
begin
    ...
    while (delay > 0) begin  : delay_loop
        ...
        last_we_n = We_n;
        if (Cle && We_n && ~Ale && Re_n && ~Ce_n_vol && ~sleeplite_en && saw_edge_we_n) begin
            if ((first_cmd == 1'b1) && (Io_mirror[7:0] == 8'h0E)) begin
                mirror_mode = 1'b1;
		#1; // added 1ns delay to get 70h in this always blocks
            end
            if (Io_mirror[7:0] != 8'hFF) begin
               first_cmd = 1'b0;       
            end
	    if ((Io_mirror [7 : 0] === 8'h70) && die_select && (cmnd_70h === 1'b0)) begin
                if (DEBUG[3]) begin $sformat(msg, "STATUS READ WHILE BUSY : MODE = STATUS"); INFO(msg); end
                cmnd_70h = 1'b1;
                cmnd_78h = 1'b0;
            end else if ((Io_mirror [7 : 0] === 8'h78) && ~disable_md_stat) begin
		...
		if (lastCmd == 8'hFA && FEATURE_SET2[CMD_RESETLUN]) begin
            ...
            end else if ((Io_mirror [7 : 0] === 8'h61) & FEATURE_SET2[CMD_JEDEC]) begin // use CMD_JEDEC b/c JEDEC was implemented at same time, and we did not want to use another FEATURE_SET2 bit
		// @(posedge row_valid) will check if cmd is valid and then disable this task
		lastCmd = 8'h61;
		row_valid = 1'b0;
		addr_start = COL_BYTES +1;
		addr_stop = ADDR_BYTES;
            end else if ((Io_mirror [7 : 0] === 8'hFF) | (Io_mirror [7 : 0] === 8'hFD) | ((Io_mirror [7 : 0] === 8'hFC) & (sync_mode || sync_enh_mode || nv_ddr3))) begin	    
                $sformat(msg, "RESET WHILE BUSY - ABORT");
                INFO(msg);
		disable_md_stat = 1'b0;
		if (Io_mirror [7 : 0] === 8'hFD)
                lastCmd = 8'hFF;
		else 
                lastCmd = (Io_mirror [7 : 0]);
                nand_reset(1);
                disable delay_loop; //exit out of this loop
            end else if ((Io_mirror [7 : 0] === 8'hFA) & FEATURE_SET2[CMD_RESETLUN]) begin
                lastCmd = 8'hFA;
                row_valid = 1'b0;
		disable_md_stat = 1'b0;
                addr_start = COL_BYTES +1;
	    ...
	    end else if(Io_mirror [7:0] === 8'h84) begin
		lastCmd = 8'h84;
		col_valid = 1'b0;
		row_valid = 1'b0;
		addr_start = 1'b1;
	        addr_stop = ADDR_BYTES;
		col_counter = 0;
		sub_col_cnt = sub_col_cnt_init;
		if(!mode_6add) wait(addr_start == 'h6);	
		else           wait(addr_start == 'h7);	

		#tWB_delay;
		if(LA == thisDieNumber) 
		begin
		  if (OTP_mode) begin
		     $sformat(msg, "Program suspend operations are only supported in the default mode of operation."); ERROR(ERR_CMD, msg);				  
		  end else begin
		     if(op_pgm_cache && !array_prog_done && !flag_pgm_sus && die_select) begin //suspend when program cache
                     Rb_n_int = 1'b0;
		     op_pgm = 1'b0;
		     op_pgm_sus = 1'b1;
		     flag_pgm_sus = 1'b1;
		     status_register [2] = 1'b1;
		     wait(array_prog_done);
		     pgm_sus_plane = pgm_plane;
		     pgm_sus_addr = pgm_row_addr;
		     if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1) 
		     pgm_sus_addr_lower = pgm_row_addr_lower;
		     queued_sus_plane = queued_pgm_plane;
		     Rb_n_int <=1;	     
		     op_pgm_sus <= 1'b0;
		     status_register [6] <= 1'b1;
		     status_register [5] <= 1'b1;
		     ...
		     end else if(op_pgm && !flag_pgm_sus && die_select) begin //suspend when program
                     Rb_n_int = 1'b0;
		     op_pgm = 1'b0;
		     op_pgm_sus = 1'b1;
		     flag_pgm_sus = 1'b1;
		     ...
		     status_register [5] <= 1'b1;
		     status_register [2] <= 1'b1;	     
		     end else if(op_pgm_res && !sus_pgm_cache && !flag_pgm_sus && die_select) begin //suspend when program resume
                     Rb_n_int = 1'b0;
		     flag_pgm_sus = 1'b1;
		     op_pgm_sus = 1'b1; 	     
		     op_pgm_res = 1'b0;
		     status_register [2] <= 1'b1;
		     go_busy(tPSPD_max);
		     pgm_sus_plane = pgm_res_plane;
                     pgm_sus_addr  = pgm_res_addr;// for all plane
		     if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1) 
		     pgm_sus_addr_lower = pgm_res_addr_lower;
		     queued_sus_plane = queued_res_plane;
		     Rb_n_int <=1;
		     op_pgm_sus <= 1'b0;
		     status_register [6] <= 1'b1;
		     status_register [5] <= 1'b1;
		     status_register [2] <= 1'b1;
		     end else if(op_pgm_res && sus_pgm_cache && !flag_pgm_sus && die_select) begin
                     Rb_n_int = 1'b0;
		     flag_pgm_sus = 1'b1;
		     op_pgm_sus = 1'b1; 	     
		     op_pgm_res = 1'b0;
		     ...
		     status_register [5] <= 1'b1;
		     status_register [2] <= 1'b1;	     
		     end else if(flag_pgm_sus && die_select) begin // suspend to a LUN already in suspend state ??? need to consider suspend for different LUN ???
		     $sformat(msg, "Program suspend is not valid when program is already suspended for same LUN");
                     WARN(msg);
		     Rb_n_int = 1'b0;		     
		     go_busy(tPSPDN_max);
		     Rb_n_int = 1'b1;
		     ...
		     disable program_resume_page;
		     disable program_cache_resume_case2;
		     delay=0;
		  end
		end
		else begin
...
//	         $display ("busy during task go_busy at time %t\n", $time);
		...
		//  to look at the upcoming address cycles to de-select the die if needed
                if (die_select) begin
		    if ((Io_mirror[7:0] == 8'h30 || Io_mirror[7:0] == 8'h32 || Io_mirror[7:0] == 8'h35 || Io_mirror[7:0] == 8'h10 || Io_mirror[7:0] == 8'h11 || Io_mirror[7:0] == 8'h13 || Io_mirror[7:0] == 8'h15 ||
			Io_mirror[7:0] == 8'hD0 || Io_mirror[7:0] == 8'hD1)&& (Rb_n_int == 1'b0)) begin
			$sformat(msg, "LUN is busy, and has received new %0hh command. New command will be ignored.", Io_mirror); ERROR(ERR_CMD, msg);
		    end
		    if ({Io_mirror[7:1], 1'b0} !== 8'h60 && {Io_mirror[7:1], 1'b0} !== 8'hD0)
			col_valid = 1'b0; // Do not reset for case of interleaved read and erase, read did not output data
		    row_valid = 1'b0;
		    if ({Io_mirror[7:1], 1'b0} === 8'h60 )
			addr_start = COL_BYTES +1;
		    else
			addr_start = 1;
		    addr_stop = ADDR_BYTES;
...
//-----------------------------------------------------------------
// function : page_map_checker          
//-----------------------------------------------------------------
function     bit      [1 : 0]  page_map_checker    ;
    input   [PAGE_BITS-1 : 0]  adrs                ;
begin
    if (adrs <=31 || adrs >= 992 || (OTP_mode == 1'b1))
       page_map_checker = 2'b00; // SLC page
    else if ( adrs%2 == 0) 
       page_map_checker = 2'b01; // lower page
    else
       page_map_checker = 2'b10; // upper page
end
endfunction


//-----------------------------------------------------------------
// function : shared_wdline          
//-----------------------------------------------------------------
function     bit               shared_wordline         ;
    input   [BLCK_BITS-1 : 0]  adrs_lower	   ;
    input   [BLCK_BITS-1 : 0]  adrs_upper	   ;
begin
    shared_wordline = 1'b0;
    if ((adrs_lower[BLCK_BITS-1: PAGE_BITS] == adrs_upper[BLCK_BITS-1: PAGE_BITS] ) && (adrs_lower[PAGE_BITS-1]/2 == adrs_upper[PAGE_BITS-1]/2))
     shared_wordline = 1'b1;
end
endfunction
...
begin
  if (!ResetComplete & InitReset_Complete) begin
	...
	if (lastCmd === 8'hFF && sync_enh_mode) sync_enh_mode <= #tITC_max 1'b0;
	if (lastCmd === 8'hFF && nv_ddr3) nv_ddr3 <= #tITC_max 1'b0;
    `endif
    tWB_check_en = 1'b1;
    #tWB_delay;
    ...
    if (soft_reset) begin
        ...
        if (dev_was_busy) begin : busy_interrupt
            ...
            if (~array_load_done) begin
        	   ...
        	   if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin	
        	     for (i=0; i < NUM_PLANES; i=i+1) begin
        	          if (queued_plane[i]) begin 
        	             page_type =  page_map_checker(row_addr[i][PAGE_BITS-1:0]);
        	             break; // only need get the first page type of queued planes because page address must be same for multi-plane operation	  
        	          end	  
        	     end
        	   end
                   for (i=0;i<NUM_PLANES;i=i+1) begin
                       if (queued_plane[i]) begin
		          if (FEATURE_SET2[CMD_ONE_PASS] == 1'b0 || (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b00)) begin 
		             if(memory_addr_exists(row_addr[i]) == 4)
			    	pp_counter[row_addr[i]] = 0;
			     else if (memory_addr_exists(row_addr[i]) == 0) begin
                              `ifdef MODEL_SV
                              `elsif FullMem
                              `else
			    	pp_counter[memory_index] = 0;				
                            	memory_addr[memory_index] = row_addr[i];
                            	memory_used  = memory_used + 1'b1;
                            	memory_index = memory_index + 1'b1;
			      `endif
			     end
		             corrupt_page(row_addr[i]); //regular array program interrupted
		             inc_pp(row_addr[i]);
			     check_block(row_addr[i]);
			  end
			  else if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b10) begin// MLC Upper page
			    for (int j = 1; j >= 0; j--) begin // should check both lower and upper pages in the wordline
		               if(memory_addr_exists(row_addr[i] -j) == 4)
			          pp_counter[row_addr[i] -j] = 0;
			       else if (memory_addr_exists(row_addr[i]-j) == 0) begin
                                `ifdef MODEL_SV
                                `elsif FullMem
                                `else
			          pp_counter[memory_index ] = 0; 			  
                                  memory_addr[memory_index] = row_addr[i] -j;
                                  memory_used  = memory_used + 1'b1;
                                  memory_index = memory_index + 1'b1;
			        `endif
			       end
		               corrupt_page(row_addr[i]-j); //regular array program interrupted
		               inc_pp(row_addr[i]-j);
			       check_block(row_addr[i]-j);
			    end
			  end
	    ...
	    end else if (op_pgm_res) begin
	        delay = tRST_prog;
        	   page_type  = 2'b00;
        	   // get page type
        	   if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin	
        	     for (i=0; i < NUM_PLANES; i=i+1) begin
        	          if (queued_res_plane[i]) begin 
        	             page_type =  page_map_checker(pgm_res_addr[i][PAGE_BITS-1:0]);
        	             break; // only need get the first page type of queued planes because page address must be same for multi-plane operation	  
        	          end	  
        	     end
        	   end
                   for (i=0;i<NUM_PLANES;i=i+1) begin
                       if (queued_res_plane[i]) begin
		          if (FEATURE_SET2[CMD_ONE_PASS] == 1'b0 || (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b00)) begin 
		             if(memory_addr_exists(pgm_res_addr[i]) == 4)
			    	pp_counter[row_addr[i]] = 0;
			     else if (memory_addr_exists(pgm_res_addr[i]) == 0) begin
                              `ifdef MODEL_SV
                              `elsif FullMem
                              `else
			    	pp_counter[memory_index] = 0;				
                            	memory_addr[memory_index] = pgm_res_addr[i];
                            	memory_used  = memory_used + 1'b1;
                            	memory_index = memory_index + 1'b1;
			      `endif
			     end
		             corrupt_page(pgm_res_addr[i]); //regular array program interrupted
		             inc_pp(pgm_res_addr[i]);
			     check_block(pgm_res_addr[i]);
			  end
			  else if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b10) begin// MLC Upper page
			    for (int j = 1; j >= 0; j--) begin // should check both lower and upper pages in the wordline
		               if(memory_addr_exists(pgm_res_addr[i] -j) == 4)
			          pp_counter[row_addr[i] -j] = 0;
			       else if (memory_addr_exists(pgm_res_addr[i]-j) == 0) begin
                                `ifdef MODEL_SV
                                `elsif FullMem
                                `else
			          pp_counter[memory_index ] = 0; 			  
                                  memory_addr[memory_index] = pgm_res_addr[i] -j;
                                  memory_used  = memory_used + 1'b1;
                                  memory_index = memory_index + 1'b1;
			        `endif
			       end
		               corrupt_page(pgm_res_addr[i]-j); //regular array program interrupted
		               inc_pp(pgm_res_addr[i]-j);
			       check_block(pgm_res_addr[i]-j);
			    end
			  end
    ...
    if ((lastCmd === 8'hFF || lastCmd === 8'hFD) && (sync_mode || sync_enh_mode)) begin
        `ifdef SO
	sync_mode <= #tITC_max 1'b1;
        `else
	sync_mode <= #tITC_max 1'b0;
        `endif             
	sync_enh_mode <= #tITC_max 1'b0;
    ...
    else if ((lastCmd === 8'hFF || lastCmd === 8'hFD) && nv_ddr3) begin
    // issue FFh during nv_ddr3 mode
    // won't switch to async mode 
    // only switch to nv_ddr3 timing mode 0
        //nv_ddr3 <= #tITC_max 1'b0;
	nv_ddr3 <= #tITC_max 1'b1;
	onfi_features[8'h01] = 8'h0; 
	switch_timing_mode_ONFI4(onfi_features[8'h01]);
	update_tWB;	
    end
    else if ( lastCmd === 8'hFD) begin // async mode
	onfi_features[8'h01] = 8'h00;
	switch_timing_mode(onfi_features[8'h01]);
	update_tWB;
    end
	...
	if (lastCmd !== 8'hFD) begin
        ...
        onfi_features[8'hE7] = 8'h34; // temperature sensor 25 degree
	...
	partial_pg_rd_len    = 16'h0;
        ...
        disable_md_stat = 1'b1;		// 78h status DISABLED after Trim Load
`ifdef SHORT_RESET
        go_busy(tRST_ready);
`else
...
`endif
    ...
    status_register [6 : 5] = 2'b11;
    least_active_mp_plane = 2'b00;
    prev_least_active_mp_plane = 2'b00;
...
begin
    ...
    saw_cmnd_81h_jedec = 1'b0;
    if (Wp_n && (OTP_mode || erase_susp_flag))
         status_register[7] = Wp_n; // reset this value when doing a new otp operation after the previous otp program failed or a new operation after pgm op failed while erase was suspended
end
endtask
    ...
    begin
        case (featAddr)
            8'h01 : begin //Timing mode
                        if ((onfi_features[featAddr][5:4] === 2'b01) && (nv_ddr3 == 1'b0) ) begin
                            $display("-----------------------------------------------------------");
			    ...
			    nv_ddr3 <= 1'b0;
                        ...
                        if ((onfi_features[featAddr][5:4] === 2'b10) && (nv_ddr3 == 1'b0)) begin
                            $display("-----------------------------------------------------------");
                            $sformat(msg, "Switching to SYNC Enhanced timing mode %0d ...", onfi_features[featAddr][3:0]);
                            INFO(msg);
                            $display("-----------------------------------------------------------");
			    // if we're already in sync enh mode, need to wait for cen to go low before switching
			    //  (that's why the wait_for_cen flag is set)
                            if (~sync_enh_mode) begin
                                switch_timing_mode(onfi_features[featAddr]);
				tRPST_sync_enh_min = tDQSRE_sync_enh_max + 0.5 * tRC_sync_enh_min; // we only get tRC when re_ is toggled twice, so we get false error when reading 1 byte
                            end
                            sync_mode     <= 1'b0;
                            sync_enh_mode <= 1'b1;
			    nv_ddr3 <= 1'b0;
                            if (~async_only_n) begin
                                $sformat(msg, "This configuration can not be run in sync enhanced mode. %h", async_only_n);  ERROR(ERR_MISC, msg);
                            end 
                        end else
			if ((onfi_features[featAddr][5:4] === 2'b00) && (nv_ddr3 == 1'b0)) begin                        
                            $display("-----------------------------------------------------------");
			...
			if ( nv_ddr3 == 1'b1 ) begin                        
                            $display("-----------------------------------------------------------");
                            $sformat(msg, "Switching to NV DDR3 timing mode %0d ...", onfi_features[featAddr][3:0] );
                            INFO(msg);
                            $display("-----------------------------------------------------------");
                            switch_timing_mode_ONFI4(onfi_features[featAddr]);
                            update_tWB; // needed when going mode 0 to other modes
                            sync_mode <= 1'b0;
                            sync_enh_mode <= 1'b0;
			    nv_ddr3 <= 1'b1;
            ...
            8'h02 : begin //  Enabling Complementary Dqs, Warmup cycles, and ODT
                        $sformat(msg, "%0sabling External Vrefq", onfi_features[featAddr][0] ?  "En" : " Dis");
                        INFO(msg);
                        compl_dqs_en = onfi_features[featAddr][1]; 
                        $sformat(msg, "%0sabling Complementary DQS_c pin", onfi_features[featAddr][1] ?  "En" : " Dis");
                        INFO(msg);
                        $sformat(msg, "%0sabling Complementary Re_c pin", onfi_features[featAddr][2] ?  "En" : " Dis");
                        INFO(msg);
			num_warmup_dout = onfi_features[featAddr][11:8];
			num_warmup_din = onfi_features[featAddr][15:12];
                        $sformat(msg, "Updating number of data input warmup cycles to %0d, data output warmup cycles to %0d", num_warmup_din, num_warmup_dout);
                        INFO(msg);
			din_ODT_en = (onfi_features[featAddr][7:4] == 4'h0) ? 0 : 1;
			dout_ODT_en = (onfi_features[featAddr][7:4] == 4'h0) ? 0 : 1;
			$sformat(msg, "%0sabling ODT during data input and output via set feature", din_ODT_en ?  "En" : " Dis");
			INFO(msg);
                    end
	    ...
	    8'h30 : begin //  Vpp configuration
		        $display("-----------------------------------------------------------");
			$sformat(msg, "%0sabling External Vpp Configuration ...", (onfi_features[featAddr][0]==1 ? " En" : "Dis")); 
                        INFO(msg);
                        $display("-----------------------------------------------------------");
			vpp_en = onfi_features[featAddr][0];
			if (vpp_en == 1'b0) 
			   tR_max       = tR_max_typ;
			else
			   tR_max       = tR_vpp_max_typ;
			
                    end
	    
	     	    
            8'h58 : begin //used for daisy chain
			if (!volume_assignment_done && ENi == 1'b1) begin
			    assign_volume_address = onfi_features[featAddr] [7:0];
			    volume_assignment_done = 1'b1;
			    ENo  = 1'bz; // the pullup on ENi of next target will pull this up to 1
			end else if (volume_assignment_done) begin
			    $sformat(msg, "Volume address has been already assigned during this power cycle"); ERROR(ERR_CMD, msg);
			end
	            end
            ...
            8'h89 : begin 
                        if (DEBUG[2]) $display("Read Retry option enabled.,tR time would be greater based upon option enabled"); 
		        if (onfi_features[featAddr][3:0]  > 4'h0) begin 
			    $sformat(msg, "Enabling Read retry option : %d", onfi_features[featAddr][3:0]); INFO(msg);  
			     if ( onfi_features[featAddr][3:0]  >= 4'h7) begin
			        tr_increased = 1;
			     end
			     else begin
			        tr_increased = 0;
			     ...
			     $sformat(msg, "Disabling Read retry option");  INFO(msg); 
			     tr_increased = 0;
			     read_retry_flag = 0;
			end    
		    end            
	    8'hF5 : begin
	               // express read and partial page read
		        $display("-----------------------------------------------------------");
			$sformat(msg, "%0sabling Express Read ...", (onfi_features[featAddr][1]==1 ? " En" : "Dis")); 
                        INFO(msg);
                        $display("-----------------------------------------------------------");
			express_rd_en =  onfi_features[featAddr][1];
		        $display("-----------------------------------------------------------");
			$sformat(msg, "%0sabling Partial Page Read ...", (onfi_features[featAddr][2]==1 ? " En" : "Dis")); 
                        INFO(msg);
                        $display("-----------------------------------------------------------");
			partial_pg_rd_en = onfi_features[featAddr][2];
			partial_pg_rd_len = onfi_features[featAddr][23:8];
			if (partial_pg_rd_en == 1'b1 && partial_pg_rd_len == 0) begin
			    $sformat(msg, "Partial page read length can't be set to 0"); ERROR(ERR_CMD, msg);
			    partial_pg_rd_len = NUM_COL;			
			end
		    end  	     
	    8'hF6 : begin
	               // Sleep Lite enable
                       $display("-----------------------------------------------------------");
		       $sformat(msg, "%0sabling Sleep Lite ...", (onfi_features[featAddr][0]==1 ? " En" : "Dis")); 
                       INFO(msg);
                       $display("-----------------------------------------------------------");
                       if (((onfi_features[featAddr][1:0] === 2'b01) && die_select ) || ((onfi_features[featAddr][1:0] === 2'b11) && !die_select)) begin
			   sleeplite_en =  1'b1;
                       end
		       else begin
			   sleeplite_en =  1'b0;		       
    ...
    for (thisPlane=0; thisPlane <NUM_PLANES; thisPlane=thisPlane+1) begin
        if (queued_plane[thisPlane]) begin
		    ...
		    // only make this check if there are LUN bits in row address
		        if ( NUM_DIE/NUM_CE == 2 && current_addr[ROW_BITS-1] != first_addr[ROW_BITS-1] )
			begin
				$sformat(msg, "Multi-plane address error. -> LUN addr must be identical for multi plane op  LUN1=%h  LUN2=%h ", current_addr[ROW_BITS-1], first_addr[ROW_BITS-1]); 
                        	ERROR(ERR_ADDR, msg);  // ??? issue with multiple reads to same page separated by another page read. 
			end
	
			if ( NUM_DIE/NUM_CE == 4 && current_addr[ROW_BITS-1 : ROW_BITS-2] != first_addr[ROW_BITS-1 : ROW_BITS-2] )
			begin
				$sformat(msg, "Multi-plane address error. -> LUN addr must be identical for multi plane op  LUN1=%h  LUN2=%h ", current_addr[ROW_BITS-1 : ROW_BITS-2], first_addr[ROW_BITS-1 : ROW_BITS-2]); 
                        	ERROR(ERR_ADDR, msg);  // ??? issue with multiple reads to same page separated by another page read. 
			end

			if ( NUM_DIE/NUM_CE == 8 && current_addr[ROW_BITS-1 : ROW_BITS-3] != first_addr[ROW_BITS-1 : ROW_BITS-3] )
			begin
				$sformat(msg, "Multi-plane address error. -> LUN addr must be identical for multi plane op  LUN1=%h  LUN2=%h ", current_addr[ROW_BITS-1 : ROW_BITS-3], first_addr[ROW_BITS-1 : ROW_BITS-3]); 
                        	ERROR(ERR_ADDR, msg);  // ??? issue with multiple reads to same page separated by another page read. 
		        end
...
endtask
    ...
    task update_tWB;
	    ...
	    tWB_delay = tWB_sync_enh_max;
        end else if (nv_ddr3) begin
	    tWB_delay = tWB_nv_ddr3_max;
        end else begin
            tWB_delay = tWB_max;
        end
    endtask
...
// don't need to modify this for onfi4 
// onfi4 mode is controlled by voltage 1.2 only
...
// Ignore original Ce_n if ENi is low (except init reset) or volume is not selected.  Note: we will use original Ce_n during the time between E1h command and select volume address 
assign Ce_n_vol = ((!ENi && InitReset_Complete) || (volume_assignment_done && !volume_selected)) ?  1'b1 : Ce_n;
...
always @ (posedge multiplane_op_rd) begin
  if (first_mp_cmd == 2'b01) begin
     least_active_mp_plane  = active_plane;
     first_mp_cmd = 2'b10;
  end
end
...
always @ (posedge row_valid) begin
    if (FEATURE_SET[CMD_2PLANE] | FEATURE_SET[CMD_MP]) begin
        ...
        if (~cmnd_78h & (lastCmd !== 8'hFA) & (lastCmd !== 8'h61)) begin
          ...
          //  active_plane = new_addr[PAGE_BITS + (NUM_PLANES >> 2) : PAGE_BITS];
	    if ((active_plane  < least_active_mp_plane) && multiplane_op_rd) 
	       least_active_mp_plane = active_plane;
	    
	    if (lastCmd == 8'h30 &&  multiplane_op_rd) 
	        prev_least_active_mp_plane =  least_active_mp_plane;
		...
		prev_least_active_mp_plane =  least_active_mp_plane;
	        ...
	        page_addr_bk[active_plane] = row_addr[active_plane][PAGE_BITS-1:0];
                row_addr[active_plane] = new_addr[ROW_BITS -1 : 0];
            end
        end
    end else begin
        //single plane address assignment
	if (~cmnd_78h & (lastCmd !== 8'hFA) & (lastCmd !== 8'h61)) row_addr[active_plane] = new_addr[ROW_BITS -1 : 0];
    end
    ...
    if (lastCmd === 8'h61) begin : erase_suspend
	if (erase_done == 1) begin
	    $sformat(msg, "No erase operation is in progress on this LUN. Erase Suspend command will be ignored");
	    INFO(msg);
	end else begin // erase op in progress, 61h cmd is valid
	    disable unlocked_erase_block; // interrupt erase block
	    $sformat(msg, "ERASE SUSPEND DURING ERASE OPERATION - SUSPEND");
	    INFO(msg);
	    for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
		if (queued_plane[eb_thisPlane]) begin // only proceed if this plane is executing erase
		    erase_susp_blk_addr[eb_thisPlane] = row_addr[eb_thisPlane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS];
		    corrupt_block(erase_susp_blk_addr[eb_thisPlane]);
		end else begin
		    erase_susp_blk_addr[eb_thisPlane] = {BLCK_BITS{1'bx}};
		end
	    end
	    // RB# and status reg are already ready
	    erase_susp_flag <= 1'b1;
	    go_busy(tESPD_max);
	    Rb_n_int <= 1'b1;   // not busy anymore
	    status_register [6 : 5] <= 2'b11;
	    if (erase_susp_flag)
		status_register [2] <= 1'b1; // suspend bit set, must qualify with erase_susp_flag b/c of possible reset in middle of ers suspend
	    erase_done <= 1'b1;
	    multiplane_op_erase <= 1'b0;
	    multiplane_op_rd    <= 1'b0;
	    multiplane_op_wr    <= 1'b0;
    ...
    if (lastCmd === 8'hD9 && die_select) begin // ZQ Calibration Short
	    tWB_check_en = 1'b1;
	    go_busy(tWB_delay);
	    Rb_n_int <= 1'b0;
	    status_register[6:5]=2'b00;
    ...
    if (lastCmd === 8'hF9 && die_select) begin // ZQ Calibration Long
	    tWB_check_en = 1'b1;
	    go_busy(tWB_delay);
	    Rb_n_int <= 1'b0;
	    status_register[6:5]=2'b00;
...
assign address_enable = (~Cle && Ale && ~Ce_n_vol && Re_n);
// Address Latch
always @ (posedge We_n) begin
    if (address_enable) begin : latch_address
        ...
        if ((addr_start === 1'b1) && (addr_stop === 1'b0)) begin : special_address
            id_reg_addr [7:0] = Io_mirror [7 : 0];
	    col_counter = 0;
	    if ((lastCmd == 8'h90) && (id_reg_addr === 8'h00 || id_reg_addr === 8'h20 || id_reg_addr === 8'h40)) begin
		   col_valid  = 1'b1;
		   col_addr = 0;
                   new_addr = 0;
             //special case for read ONFI params (ECh with 00h address cycle)
	     end else if ((ONFI_read_param === 1'b1) && (id_reg_addr === 8'h00 || (id_reg_addr === 8'h40 && FEATURE_SET2[CMD_JEDEC]))) begin
		...
		load_cache_register(0,0);
            ...
            // check for get_features by LUN; D4h + addr(LUN) + addr(Feature Address)
            end else if (lastCmd === 8'hD4) begin
	        if(set_get_feature_lun_1addr) begin    // the first address is LUN address
		    set_get_feature_lun_1addr = 1'b0;  		    
		    addr_start =  0;                // To receive the second address 
		    LA[2:0] = Io_mirror [2 : 0];
		    //here we determine if this die model is the active device based on the LUN address
                    if ((NUM_DIE / NUM_CE) == 4) begin
			if ( LA[2:0] == thisDieNumber[2:0]) begin
			    die_select = 1'b1;
                        end else begin
                            die_select = 1'b0;
		...
		end else if(die_select && (addr_start == 1)) begin     // the second address is feature address.
                    set_get_feature_lun_addr_ok = 1'b1;
		    if (check_feat_addr(id_reg_addr,nand_mode[0]) == 0)
                    begin
                    ...
                    status_register[6:5]<=2'b11;
                    Rb_n_int <=1'b1;
		    disable_md_stat = 1'b0; // 78h allowed after tFEAT
	       end 	            //now check for set_features by LUN; D5h + addr(LUN) + addr(Feature Address)
            end else if (lastCmd === 8'hD5) begin
	        if(set_get_feature_lun_1addr) begin   // the first address is LUN address
		    set_get_feature_lun_1addr = 1'b0;		    
		    addr_start =  0;                // To receive the second address  
		    LA[2:0] = Io_mirror [2 : 0];
		...
		end else if(die_select && (addr_start == 1)) begin
		    set_get_feature_lun_addr_ok = 1'b1;
		    if (check_feat_addr(id_reg_addr,nand_mode[0]) == 0 || id_reg_addr == 8'hE7)
                    begin
        ...
        // Latch Column
          if ((addr_start <= COL_BYTES ) && (addr_start <= addr_stop) && ~col_valid  && ~col_addr_dis && ~nand_mode[0]) begin : latch_col_addr
            ...
            case (addr_start)
                1 : begin
                        temp_col_addr [7 : 0] = Io_mirror [7 : 0];
			if ((sync_mode || sync_enh_mode || nv_ddr3) && (temp_col_addr[0] !== 1'b0) && ((lastCmd != 8'hEE && lastCmd != 8'hEF  ) && die_select && Rb_n_int)) begin			
                            $sformat(msg, "LSB of column address must be 0 in sync mode.  lastCmd=%2h", lastCmd);
                            ERROR(ERR_ADDR, msg);
			    temp_col_addr[0] = 1'b0; 
			end    			
                    end                
                2 : begin 
                        temp_col_addr [COL_BITS - 1 : 8] = Io_mirror [(COL_BITS -8 - 1) : 0];
			...
			// this should take of commands with 2 address cycle
        ...
        // Latch Row
        if ((addr_start >= (COL_BYTES +1)) && (addr_start <= addr_stop) && (addr_stop > COL_BYTES +1) && ~nand_mode[0]) begin : latch_row_addr
             case (addr_start)
                3 : begin
                    if(lastCmd ==8'h85 | cmnd_85h) begin		       
			...
			if (!die_select) begin
			    temp_col_addr = col_addr_dup; // in "pgm pause" case, restore temp_col_addr to col addr that was latched. col_addr_dup is helper var since we overwrote temp_col_addr
                    ...
                    new_addr [ 7 : 0] = Io_mirror [7 : 0];
		   
                end
                4 : begin
                    new_addr [15 : 8] = Io_mirror [7 : 0];
                ...
                5 : begin
		    if(!mode_6add) begin
                    new_addr [(ROW_BITS -1):16] = Io_mirror [(ROW_BITS -1 -16):0];
                    if (~row_valid) begin
                        case (LUN_BITS)
                        2       : begin
			            LA[2] = 1'b0;
			            LA[1] = Io_mirror [(ROW_BITS -1) -16];
                                    LA[0] = Io_mirror [(ROW_BITS -2) -16];
                                  end 
                        1       : begin 
			            LA[2] = 1'b0;
			            LA[1] = 1'b0;
                                    LA[0] = Io_mirror [(ROW_BITS -1) -16]; //need to modify
                                  end
                        default : begin 
			            LA[2] = 1'b0;
                                    LA[1] = 1'b0;
                        ...
                        endcase 
			if (DEBUG[1]) begin $sformat(msg, "Lun Addr0 %d  : Lun Addr1 %d : Lun Addr2 %d", LA[0], LA[1], LA[2]); INFO(msg); end
			if ( lastCmd === 8'hD9 || lastCmd === 8'hF9) begin
			    LA[2:0] = Io_mirror[2:0];    
			    if ( cmnd_78h) begin
			      LA[0] = Io_mirror [(ROW_BITS -1) -16];
			    end						    
			end
		    ...
		    if ((NUM_DIE / NUM_CE) == 8) begin
			    ...
			    die_is_selected;
                    ...
                    end else if ((NUM_DIE / NUM_CE) == 4) begin
		    ...
		    end else begin//----------------latch address when 6 addres cycle enabled--------------------
		    
	                    new_addr [23:16] = Io_mirror [7:0];
                    if (~row_valid) begin
                        case (LUN_BITS)
			3       : begin
			            LA[1] = Io_mirror [7];
		...
		6:   begin
		    ...
		    if ( lastCmd === 8'hD9 || lastCmd === 8'hF9) begin
                    ...
                    if(config_6add)new_addr [(32 -1):24] = Io_mirror [(32 -1 -24):0];
                    if (~row_valid) begin
                        case (LUN_BITS)
			3       : begin
			            LA[2] = Io_mirror [0];
            ...
            endcase
            if (DEBUG[0]) begin 
                $sformat (msg, "Latch Address (%0h) = %0h", addr_start, Io_mirror [7 : 0]);
                INFO(msg);
            end

	    if (((addr_start >= ADDR_BYTES) & die_select ) || (cmnd_78h && die_select && (addr_start >= ADDR_BYTES))) begin 	    
                row_valid = 1'b1;
            end
...
// need seperate block to detect E1h command and address because we must look at raw Ce_n instead of qualified Ce_n_vol
always @ (posedge We_n) begin
    // ***************************************
    // Command (E1h) : VOLUME SELECT
    // ***************************************
    if (Cle & ~Ale & ~Ce_n & Re_n & ~sleeplite_en && Io_mirror[7:0] === 8'hE1) begin // command
	cmnd_85h = 1'b0;
	saw_cmnd_00h = 1'b0;
	saw_cmnd_00h_stat = 1'b0;
	saw_cmnd_60h = 1'b0;
	...
	addr_start = 1;
	addr_stop = 0;
	if (first_cmd_after_ce) begin
	    first_cmd_after_ce <= 0;
	end else begin
	    $sformat(msg,"VOLUME SELECT (E1h) command may only be issued as the first command after CE# is pulled LOW"); ERROR(ERR_CMD, msg);
	end
	if (tm_ce_n_f - tm_ce_n_r < tCEH_min) begin $sformat(msg,"tCEH violation by %t", tCEH_min - (tm_ce_n_f - tm_ce_n_r)); ERROR(ERR_TIM, msg); end
    end if (Cle & ~Ale & ~Ce_n & Re_n & ~sleeplite_en) begin // non-E1h command
	first_cmd_after_ce <= 0;  
    end else if (~Cle && Ale && ~Ce_n && Re_n && cmnd_E1h) begin // address
	volume_selected = (Io_mirror[7:0] == assign_volume_address) ? 1 : 0;
	cmnd_E1h = 0;
	tm_vol_sel <= $realtime;
    end
end
...
assign command_enable = (Cle & ~Ale & ~Ce_n_vol & Re_n & ~sleeplite_en);
...
always @ (posedge We_n) begin : cLatch
    ...
    if (command_enable) begin : Cle_enable
       ...
       cmnd_ignore = 1'b0;
        ...
        `ifdef SO
        if (~InitReset_Complete& FEATURE_SET[CMD_ONFI] & ((Io_mirror[7:0] != 8'hFF) & (Io_mirror[7:0] != 8'hFC) & (Io_mirror[7:0] != 8'h70)) ) begin : reset_check
        `else
        if (~InitReset_Complete& FEATURE_SET[CMD_ONFI] & (Io_mirror[7:0] != 8'hFF) & (Io_mirror[7:0] != 8'h70) ) begin : reset_check
        `endif
	    //if (id_reg_addr == 8'h85 && (Io_mirror[7:0] == 8'h70 || Io_mirror[7:0] == 8'hFC || Io_mirror[7:0] == 8'hFA || Io_mirror[7:0] == 8'hFF)) begin
	    if (id_reg_addr == 8'h85 || (Io_mirror[7:0] == 8'h70 || Io_mirror[7:0] == 8'hFC || Io_mirror[7:0] == 8'hFA || Io_mirror[7:0] == 8'hFF)) begin
	  	// We reach here if host wrote to ftr addr 85h: User Selectable Trim Profile, which requires host to give reset before any operations.
	  	// Host can only give 70h sts or any reset command. So, if host gives an invalid command, print the error.
	    end else begin
          	if (nand_mode[0]) begin
	    ...
	    end // ends id_reg_addr == 8'h85 ...
        end 
        if (Rb_n_int === 1'b1) begin : cLatch_unbusy
            if (DEBUG[3]) begin $sformat(msg, "Command Latched = %2Hh", Io_mirror[7:0]); INFO(msg); end
            // *******************************
            // Command (00h) : PAGE READ START
            // *******************************
            if (Io_mirror [7 : 0] === 8'h00) begin : cmnd_00h
                abort_en = 1'b0;
                cmnd_85h = 1'b0;
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
            ...
            // ***************************************
            else if (Io_mirror [7 : 0] === 8'h05) begin 
                cmnd_85h = 1'b0;
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
                // Some devices disable random read during cache mode
                if (die_select) begin
		    if (saw_cmnd_00h_stat == 1'b1 && lastCmd == 8'h00) begin
		      row_addr[active_plane] = {row_addr[active_plane][ROW_BITS-1:(PAGE_BITS+(NUM_PLANES >> 2)+1)],active_plane[(NUM_PLANES >> 2) : 0],page_addr_bk[active_plane]};
		      prev_least_active_mp_plane =  least_active_mp_plane;		    
		    end
                    saw_cmnd_00h_stat = 1'b0;
                    lastCmd = 8'h05;
            ...
            // ***********************************************
            else if ((Io_mirror [7 : 0] === 8'h06) & (FEATURE_SET[CMD_2PLANE] | FEATURE_SET[CMD_MP])) begin
                cmnd_85h = 1'b0;
            ...
            // ***************************************
            else if ((Io_mirror [7 : 0] === 8'h10) && die_select) begin
                cmnd_85h = 1'b0;
                datain_index = 0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
		lastpgmcmd = 8'h0;     // reset to zero once the program operation completed
		lastpgmcmd_85h = 8'h0;     // reset to zero once the program operation completed
		if((flag_pgm_sus ===1'b1) && (lastCmd ===8'h80 || lastCmd ===8'h85)) begin
		    lastCmd = 8'h10;
	      	    Rb_n_int = 1'b0;
		    status_register[7]=1'b0;
	            go_busy(tPSPDN_max);
     	            Rb_n_int <=1'b1;
		    status_register[7:4] <= 4'b0110;			       
		end else begin	
		...
		if (saw_cmnd_81h_jedec && ~multiplane_op_wr) begin
		    ...
		    saw_cmnd_81h_jedec = 1'b0;
		end else if ((row_valid && ~nand_mode[0] && Wp_n)) begin
        	    ...
        	    if( FEATURE_SET2[CMD_ONE_PASS] == 1'b1) begin    
        	      for (thisPlane=0; thisPlane < NUM_PLANES; thisPlane=thisPlane+1) begin
                    ...
                    if ((cache_op === 1) && ~nand_mode[0] && ~ (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01)) begin
                    ...
                    //program_page
                    if (((lastCmd === 8'hA5) && ~nand_mode[0])) begin
			...
			tWB_check_en = 1'b1;
                ...
                multiplane_op_rd    = 1'b0;
                multiplane_op_erase = 1'b0;
		if (~ (FEATURE_SET2[CMD_ONE_PASS] == 1'b1 && page_type == 2'b01)) begin
                cache_op <= 1'b0;
		op_pgm_cache <= 1'b0;
		end
		end
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h11) & (FEATURE_SET[CMD_2PLANE] | FEATURE_SET[CMD_MP])) begin 
		...
		lastpgmcmd_85h = 8'h0;     // reset to zero once the program operation completed
		    ...
		    for (thisPlane=0; thisPlane <NUM_PLANES; thisPlane=thisPlane+1) begin
                          if (queued_plane[thisPlane]) begin
                              array_addr = {row_addr[thisPlane][ROW_BITS-1:PAGE_BITS], 
                                  row_addr[active_plane][PAGE_BITS-1:0]};         
                      //  $display("Executing check block for plane = %d" ,thisPlane);
                       // check_block(array_addr);
                          end
                    end
		    go_busy(tWB_delay);
                    Rb_n_int = 1'b0;
            ...
            // ********************************************************
            // Command (13h) : PROGRAM RESUME PAGE
            // ********************************************************
    	        else if (Io_mirror [7:0] === 8'h13) begin
		if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		if(sync_enh_mode)              #(tWH_sync_enh_min+tWB_delay-1);
		else if(nv_ddr3)                #(tWH_nv_ddr3_min+tWB_delay-1);
		else                                    #(tWH_min+tWB_delay-1);
		if(LA === thisDieNumber) begin			    
		if(OTP_mode) begin
		$sformat(msg, "Program suspend operations are only supported in the default mode of operation."); ERROR(ERR_CMD, msg);							
		end else begin	
		if(flag_pgm_sus && !sus_pgm_cache && (LA === thisDieNumber)) begin // resume successfully
		    Rb_n_int = 1'b0;
		    flag_pgm_sus = 1'b0;
		    op_pgm_res = 1'b1;
		    status_register [7] <= 1'b1;
		    ...
		    status_register [5] <= 1'b0;
		    status_register [2] <= 1'b0;	
		    pgm_res_plane = pgm_sus_plane;
		    pgm_res_addr = pgm_sus_addr;//all plane
		    if (FEATURE_SET2[CMD_ONE_PASS] == 1'b1)
		    pgm_res_addr_lower = pgm_sus_addr_lower;
	            queued_res_plane = queued_sus_plane;
	            program_resume_page(multiplane_op_wr);
		end else if (flag_pgm_sus && sus_pgm_cache && (LA === thisDieNumber)) begin
		    Rb_n_int = 1'b0;
		    flag_pgm_sus = 1'b0;
		    op_pgm_res = 1'b1;			    	    
		    status_register [7] <= 1'b1;
		    ...
		    status_register [5] <= 1'b0;
		    status_register [2] <= 1'b0;
		    program_cache_resume_case2;//sus_pgm_cache changes to 0 at the end of the tPROG, no data transfer!!!		    		
		end else begin
		    Rb_n_int = 1'b0;		
		    status_register [6] <= 1'b0;
	            status_register [5] <= 1'b0;
		    ...
		    status_register [2] <= 1'b0;
		    Rb_n_int = 1'b1;
	            $sformat(msg, "Resume will be ignored when no program is suspended");
                    WARN(msg);			
		    end
		end			
		end
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h15) && die_select) begin
		...
		lastpgmcmd_85h = 8'h0;     // reset to zero once the program operation completed
                ...
                if (boot_block_lock_mode) begin
                    $sformat(msg, "Error: Program Page Cache Mode Command %0h not allowed in boot block lock mode.", Io_mirror[7:0]); ERROR(ERR_CMD, msg); 
                end
		...
		if (saw_cmnd_81h_jedec && ~multiplane_op_wr) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h23) && Wp_n && FEATURE_SET[CMD_LOCK]) begin
                cmnd_85h = 1'b0;
                abort_en = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h24) && FEATURE_SET[CMD_LOCK]) begin
                cmnd_85h = 1'b0;
                abort_en = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h2A) && FEATURE_SET[CMD_LOCK]) begin
                cmnd_85h = 1'b0;
                abort_en = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h2C) && FEATURE_SET[CMD_LOCK]) begin
                cmnd_85h = 1'b0;
                abort_en = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h30) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                cmnd_85h = 1'b0;
		...
		first_mp_cmd = 2'b00;
		       ...
		       if (read_retry_flag) begin
		         $sformat(msg, "READ RETRY OPTION ENABLED,READ OTP PAGE NOT VALID!!!!!!!!!!!!!"); ERROR(ERR_CACHE, msg);
		       end	    
                        OTP_read = 1'b1;
                    ...
                    else if ((lastCmd === 8'h00 || lastCmd === 8'h33) && row_valid && ~saw_cmnd_65h) begin //page_read
                        lastCmd = 8'h30;
                        col_counter = 0;
                        sub_col_cnt = sub_col_cnt_init;  // reset sub col count
                        if (saw_cmnd_00h_stat) begin 
...
`endif
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h31) begin
	        if(op_pgm_cache) begin
	            $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                cmnd_85h = 1'b0;
		if((lastCmd ===8'h30||lastCmd ===8'h00 ||lastCmd === 8'hE0)&&(cache_op==0)&&(flag_pgm_sus===1'b0))
		   cmnd_31h_first = 1'b1;
		else
		   cmnd_31h_first = 1'b0;
                if(flag_pgm_sus) begin
		    $sformat(msg, "CACHE READ IS NOT VALID DURING PROGRAM SUSPEND !!!!!!!!!!!!!"); ERROR(ERR_CACHE, msg);
		end else begin
		...
		if (read_retry_flag) begin
		    $sformat(msg, "READ RETRY OPTION ENABLED,CACHE OPERATION NOT VALID !!!!!!!!!!!!!"); ERROR(ERR_CACHE, msg);
		end
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
		first_mp_cmd = 2'b00;
		    ...
		    // !cmnd_78h is for case of die1 78h-00h, die0 78h-31h; die 0 should auto-increment row addr
		    	...
		    	// otherwise we have sequential read cache (30h->31h->3Fh) command, need to auto-increment row addr
                        for (plane_addr = 0; plane_addr < NUM_PLANES; plane_addr = plane_addr +1) begin
                            if (queued_plane[plane_addr] && !cache_check) begin
				...
				if(!multiplane_op_rd_cache) begin cache_check = 1'b1; end
                            end
                        end
		    ...
		    cache_check = 1'b0;
	               ...
	               if  (rd_pg_cache_seqtl & multiplane_op_rd_cache)  begin
                          cache_rd_active_plane = #1 least_active_mp_plane;
			  active_plane          = #1 cache_rd_active_plane;
		       end else if (~rd_pg_cache_seqtl & multiplane_op_rd) begin
		          active_plane <= #1 least_active_mp_plane; 	  
		       end 	  
		...
		saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h32) && FEATURE_SET[CMD_ONFI] && die_select) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
               cmnd_85h = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h35) && die_select) begin
	        if(op_pgm_cache) begin
	             $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);			
		end else begin
                cmnd_85h = 1'b0;
		...
		saw_cmnd_00h_stat = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h3A) && FEATURE_SET[CMD_ONFI]) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h3F) && FEATURE_SET[CMD_NEW]) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);						
		end else begin
		cmnd_31h_first = 1'b0;
		    ...
		    if (NUM_PLANES > 1) begin
                       ...
                       if  (rd_pg_cache_seqtl & multiplane_op_rd_cache)  begin
                       // this will override the blocking assignment made in load_cache_register task
                           cache_rd_active_plane = #1 least_active_mp_plane;
                           active_plane <= #1 cache_rd_active_plane; 
		       end else if (~rd_pg_cache_seqtl & multiplane_op_rd)
		           active_plane <= #1 least_active_mp_plane; 	  
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h60) begin
	        if(op_pgm_cache) begin
	            $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);			
		end else begin
                cmnd_85h = 1'b0;
	    ...
	    // ********************************************************
            // Command (61h) : BLOCK SUSPEND START (only for cov model)
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h61) & FEATURE_SET2[CMD_JEDEC]) begin // special code for cov model b/c model's ers finishes much faster than chip
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
		row_valid = 1'b0;
		...
		addr_stop = ADDR_BYTES;	
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h65) && (FEATURE_SET[CMD_ID2] || FEATURE_SET[CMD_UNIQUE])) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h70) && die_select) begin
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h78) begin
		...
		if (lastCmd == 8'hFA && FEATURE_SET2[CMD_RESETLUN]) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'h7A) && (die_select) && FEATURE_SET[CMD_LOCK]) begin
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h80) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		    ...
		    disable_md_stat = 1'b0;
		        ...
		        OTP_write = 1'b0;//xxz case for 85???
		    end
                end //program
            end
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h81 && FEATURE_SET2[CMD_JEDEC]) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
            ...
            // ************************************************************
            // Command (84h) : PROGRAM SUSPEND WHILE NOT BUSY
            // ************************************************************
	    else if (Io_mirror [7 : 0] === 8'h84 && command_enable) begin
	        if(!op_pgm_cache) begin//suspend while no program is performing
                    saw_cmnd_00h = 1'b0;
                    saw_cmnd_00h_stat = 1'b0;
                    saw_cmnd_60h = 1'b0;
                    saw_cmnd_60h_clear = 1'b0;
		    ...
		    if(LA == thisDieNumber) begin	
		       ...
		       $sformat(msg, "Program suspend operations are only supported in the default mode of operation."); ERROR(ERR_CMD, msg);					
		           ...
		           $sformat(msg, "Suspend will be ignored when no program operation is performming");
                           WARN(msg);
  		       end
		    end
	            #100;
		end else begin//suspend while cache program is performing		
		      lastCmd = 8'h84;
		      col_valid = 1'b0;
		      row_valid = 1'b0;
		      addr_start = 1'b1;
			...
			if((op_pgm_cache|| sus_pgm_cache) && !op_pgm && !flag_pgm_sus && die_select) begin //suspend when cache program
			Rb_n_int = 1'b0;
			op_pgm_cache = 1'b0;
			cache_op = 1'b0;
			sus_pgm_cache = 1'b1;//added for program cache
			op_pgm = 1'b0;
			op_pgm_sus = 1'b1;
			flag_pgm_sus = 1'b1;
			status_register [6] = 1'b0;
			status_register [2] = 1'b1;
			go_busy(tPSPD_max);
			//data is already stored into mem_array in model, so no need to transfer address
			Rb_n_int <= 1;
			op_pgm_sus <= 1'b0;
			status_register [6] <= 1'b1;
		        status_register [5] <= 1'b1;
            ...
            // ************************************************************
            else if (Io_mirror [7 : 0] === 8'h85) begin
			...
			if (lastCmd !=8'h85) begin
			   lastpgmcmd = lastCmd; // added for checking if last command is 'h80 
                        end
            ...
            // ***********************************************************
            else if ((Io_mirror [7 : 0] === 8'h8C) && FEATURE_SET[CMD_ONFI]) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_60h = 1'b0;
                saw_cmnd_60h_clear = 1'b0;
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'h90 & id_cmd_lun) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
		...
		do_read_unique = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hA0) && FEATURE_SET[CMD_OTP]) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hA5) && FEATURE_SET[CMD_OTP]) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hAF) && FEATURE_SET[CMD_OTP]) begin
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hB8) & FEATURE_SET[CMD_DRVSTR]) begin
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hD0) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                cmnd_85h = 1'b0;
                ...
                saw_cmnd_60h_clear = 1'b0;
                abort_en = 1'b0;
		if((lastCmd === 8'h60) && (flag_pgm_sus === 1'b1)) begin
		   lastCmd = 8'hD0;
		   Rb_n_int = 1'b0;
		   status_register[6:5] = 2'b00;
		   go_busy(tPSPDN_max);
		   ...
		   Rb_n_int = 1'b1;
                ...
                if ((lastCmd === 8'h60) && row_valid && Wp_n) begin
                    ...
                    if (erase_susp_flag) begin // we are giving erase op when a previous erase has been suspended in this LUN
			for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
			if(row_addr[active_plane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS] == erase_susp_blk_addr[active_plane]) begin //resume if the block address is same with that was suspended
			    if (erase_susp_blk_addr[eb_thisPlane] !== {BLCK_BITS{1'bx}} && queued_plane[eb_thisPlane] == 0) begin // restore this plane's suspended erase if plane is not queued with an erase
				queued_plane[eb_thisPlane] = 1;
				row_addr[eb_thisPlane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS] = erase_susp_blk_addr[eb_thisPlane]; 
				$display("--------------Resuming Block Erase to plane=%h, Block=%h at time %t", eb_thisPlane, erase_susp_blk_addr[eb_thisPlane], $time); 
				if (DEBUG[4]) begin $sformat(msg, "INFO: Resuming Block Erase to plane=%h, Block=%h", eb_thisPlane, erase_susp_blk_addr[eb_thisPlane]); INFO(msg); end 
			    end
			end
			end
			if(!OTP_mode)
            ...
            // ********************************************************
            else if ((Io_mirror[7:0] === 8'hD1) & FEATURE_SET[CMD_ONFI] & (FEATURE_SET[CMD_2PLANE] | FEATURE_SET[CMD_MP])) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);			
		end else begin
                cmnd_85h = 1'b0;
            ...
            // ********************************************************
            // Command (D2h) : BLOCK ERASE RESUME
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hD2) & FEATURE_SET2[CMD_JEDEC]) begin // use CMD_JEDEC b/c JEDEC was implemented at same time, and we did not want to use another FEATURE_SET2 bit
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                cmnd_85h = 1'b0;
                ...
                abort_en = 1'b0;
	      if (OTP_mode) begin
                    $sformat(msg, "Device in OTP mode.  ERASE Resume operation disabled.");
                    WARN(msg);
              end else begin
		lastCmd = 8'hD2;
                clear_queued_planes;
		...
		multiplane_op_rd    = 1'b0; 
                ...
                if (erase_susp_flag) begin
		    for (eb_thisPlane=0; eb_thisPlane < NUM_PLANES; eb_thisPlane=eb_thisPlane+1) begin
			if (erase_susp_blk_addr[eb_thisPlane] !== {BLCK_BITS{1'bx}}) begin // only proceed if this plane has a suspended erase
			    queued_plane[eb_thisPlane] = 1;
			    row_addr[eb_thisPlane][BLCK_BITS+PAGE_BITS-1:PAGE_BITS] = erase_susp_blk_addr[eb_thisPlane];
			    if (DEBUG[4]) begin $sformat(msg, "INFO: Resuming Block Erase to plane=%h, Block=%h", eb_thisPlane, erase_susp_blk_addr[eb_thisPlane]); INFO(msg); end 
			end
		    ...
		    clear_erase_suspend;
		    wait(erase_done);
                end else begin // no erase operation is suspended on this LUN, execute dummy wait
		    tWB_check_en = 1'b1;
		    go_busy(tWB_delay);
		    if (!OTP_mode) begin // Suppose the Erase operation comes in mid of OTP operation
		...
		eb_boot_fail = 0; // reset to default value
	       end
	       end	
            end
            ...
            // ********************************************************
            // Command (D4h) : GET FEATURES by LUN for L06 Chip; 
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hD4) & FEATURE_SET[CMD_FEATURES]) begin
	  	if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                ...
                abort_en = 1'b0;
                clear_queued_planes;
                disable_md_stat = 1'b0;   // 78h ENABLED during and after D4h
                lastCmd = 8'hD4;
		die_select = 1'b0;
                set_get_feature_lun_1addr = 1'b1; 
		set_get_feature_lun_addr_ok = 1'b0;
                disable_rdStatus;
                do_read_unique = 1'b0;
                col_valid = 1'b0;
            ...
            // ********************************************************
            // Command (D5h) : SET FEATURES by LUN for L06 Chip; 

            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hD5) && FEATURE_SET[CMD_FEATURES]) begin
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin
                cmnd_85h = 1'b0;
                ...
                abort_en = 1'b0;
                clear_queued_planes;
                disable_md_stat = 1'b0; //78h Command allowed after tFEAT with set feature by LUN command
                lastCmd = 8'hD5;
		die_select = 1'b0;
                set_get_feature_lun_1addr = 1'b1;
		set_get_feature_lun_addr_ok = 1'b0; 
	    ...
	    // ********************************************************
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hD9)) begin  
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		die_select = 1'b0;
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hE0) begin
		...
		addr_cnt_en = 1; // enable counter b/c it was possibly disabled by previous 05h cmd during multi-lun sharing same p_ce config
            ...
            // Command (E1h) : VOLUME SELECT
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hE1) begin
		// We have seperate block to detect E1h command and address because we must look at raw Ce_n instead of qualified Ce_n_vol
		// we put dummy code here to prevent the "Unsupported command" error
            end
	    
            // ********************************************************
            // Command (E2h) : ODT CONFIGURE
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hE2) begin
		cmnd_85h = 1'b0;
		saw_cmnd_00h = 1'b0;
		saw_cmnd_00h_stat = 1'b0;
		saw_cmnd_60h = 1'b0;
		...
		col_counter = 0;
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hEC) & (FEATURE_SET[CMD_ONFI] || FEATURE_SET2[CMD_JEDEC]) & id_cmd_lun) begin	    
	        if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else if (partial_pg_rd_en) begin
		    $sformat(msg, "Partial Page read is prohibited with ONFI READ PARAMETER PAGE !!!!!!!!!!!!!"); ERROR(ERR_CMD, msg);		
		end else begin
				
                cmnd_85h = 1'b0;
		if (read_retry_flag) begin
		    $sformat(msg, "READ RETRY OPTION ENABLED,READ PARAMETER PAGE NOT VALID !!!!!!!!!!!!!"); ERROR(ERR_CACHE, msg);
		end		
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io_mirror[7:0] === 8'hED) & FEATURE_SET[CMD_UNIQUE] & FEATURE_SET[CMD_ONFI] & id_cmd_lun) begin
	    	if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else if (partial_pg_rd_en) begin
		    $sformat(msg, "Partial Page read is prohibited with ONFI READ UNIQUE ID !!!!!!!!!!!!!"); ERROR(ERR_CMD, msg);		
		end else begin

	        // Clear up other commands  
                cmnd_85h = 1'b0;
		if (read_retry_flag) begin
		    $sformat(msg, "READ RETRY OPTION ENABLED,READ UNIQUE ID NOT VALID !!!!!!!!!!!!!"); ERROR(ERR_CACHE, msg);
		end		
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		col_counter = 0; 
            ...
            // Only 1 LUN returns data, to avoid collisions
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hEE) & FEATURE_SET[CMD_FEATURES] & id_cmd_lun) begin
	       if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin	        
                cmnd_85h = 1'b0;               
                ...
                abort_en = 1'b0;
                clear_queued_planes;
                disable_md_stat = 1'b1; //78h DISABLED during EEh
                lastCmd = 8'hEE;
                disable_rdStatus;
                col_valid = 1'b0;
                row_valid = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hEF) && FEATURE_SET[CMD_FEATURES]) begin
		 if(op_pgm_cache) begin
		    $sformat(msg, "ARRAY is still busy (program cache is not completed) and has received new %0hh command. Command will be ignored until status become E0h.", Io_mirror); ERROR(ERR_CMD, msg);				
		end else begin	        
		cmnd_85h = 1'b0;
                ...
                abort_en = 1'b0;
                clear_queued_planes;
                disable_md_stat = 1'b1; //78h DISABLED during EFh
                lastCmd <= 8'hEF;
                disable_rdStatus;
                col_valid = 1'b0;
                row_valid = 1'b0;
	    ...
	    // ********************************************************
            // Command (D9h) : ZQ Calibration Long
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hF9)) begin  
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		die_select = 1'b0;
            ...
            // ********************************************************
            else if ((Io_mirror [7 : 0] === 8'hFA) & FEATURE_SET2[CMD_RESETLUN]) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		disable_md_stat = 1'b0;
            ...
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hFC) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                saw_cmnd_60h = 1'b0;
		...
		if (sync_mode || sync_enh_mode || nv_ddr3) begin
                    lastCmd = 8'hFC;
                    if (~ResetComplete)
                        nand_reset (1'b0);
                    else
            ...
            // ********************************************************
            // Command (FDh) : HARD RESET
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hFD) begin
            // SMK : ADD nand mode[0] SUPPORT HERE
                //set lastCmd register 
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                ...
                saw_cmnd_60h_clear = 1'b0;
                abort_en = 1'b0;
		if(OTP_mode && die_select) begin	 
		    $sformat(msg, "HARD RESET are only supported in the default mode of operation."); ERROR(ERR_CMD, msg);  				 
	       	end else begin
                  if (die_select ) begin
            ...
            // Command (FFh) : RESET
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hFF) begin
            // SMK : ADD nand mode[0] SUPPORT HERE
                //set lastCmd register 
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
            ...
            // ********************************************************
            else if (die_select && Io_mirror[7:0] !== 8'h84 && Io_mirror[7:0] !== 8'h13) begin
                $sformat(msg, "Unsupported command = %00h", Io_mirror[7:0]);
		cmnd_ignore =1'b1;
                ERROR(ERR_CMD, msg);
            end
        end //  cLatch_unbusy
        ...
        //-----------------------------------------------
            ...
            if (DEBUG[3]) begin
                $sformat(msg, "Attempting to latch command %0h while busy at time %t...", Io_mirror, $time); 
                INFO(msg);
            end
            $display("Attempting to latch command %0h while busy at time %t...", Io_mirror, $time); 
            // ********************************************************
            // Command (70h) : READ STATUS (DURING BUSY)
            // ********************************************************
            if ((Io_mirror [7 : 0] === 8'h70) && (die_select)) begin
            // SMK : ADD nand mode[0] SUPPORT HERE
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                if (DEBUG[3]) begin $sformat (msg, "Command Latched = %2hh", Io_mirror[7:0]); INFO(msg); end
            ...
            // ********************************************************
            else if (~nand_mode[0] && (Io_mirror [7 : 0] === 8'h78)) begin
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                if (DEBUG[3]) begin $sformat(msg, "Command Latched = %2hh", Io_mirror[7:0]); INFO(msg); end
                // Status
		...
		disable_md_stat = 1'b0;
            ...
            // Command (FCh) : SYNCHRONOUS RESET
            // ********************************************************
            else if (~nand_mode[0] && (Io_mirror [7 : 0] === 8'hFC)) begin
                //set lastCmd register
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
		...
		if (sync_mode || sync_enh_mode || nv_ddr3) begin
            ...
            // ********************************************************
            // Command (FFh) : HARD RESET (DURING BUSY)
            // ********************************************************
            else if (Io_mirror [7 : 0] === 8'hFD) begin
                cmnd_85h = 1'b0;
                saw_cmnd_00h = 1'b0;
                saw_cmnd_00h_stat = 1'b0;
                abort_en = 1'b0;
                $sformat(msg, "HARD Reset issued in busy state, similar as normal reset");
                WARN(msg);		
                lastCmd = 8'hFF;
                nand_reset (1'b1);
            end
		...
		//  to look at the upcoming address cycles to de-select the die if needed
                if (die_select) begin
		    if (Io_mirror[7:0] == 8'h30 || Io_mirror[7:0] == 8'h32 || Io_mirror[7:0] == 8'h35 || Io_mirror[7:0] == 8'h10 || Io_mirror[7:0] == 8'h11 || Io_mirror[7:0] == 8'h15 ||
			Io_mirror[7:0] == 8'hD0 || Io_mirror[7:0] == 8'hD1) begin
			$sformat(msg, "LUN is busy, and has received new %0hh command. New command will be ignored.", Io_mirror); ERROR(ERR_CMD, msg);
		    end
		    if ({Io_mirror[7:1], 1'b0} !== 8'h60 && {Io_mirror[7:1], 1'b0} !== 8'hD0)
			col_valid = 1'b0; // Do not reset for case of interleaved read and erase, read did not output data
...
always @ (posedge Clk_We_n) begin : ColAddrDisBlk
    if (Cle && ~Ale && ~Ce_n && Wr_Re_n && Io_mirror [7 : 0] === 8'h60)
        col_addr_dis <= 1'b1;  
    else if(Cle && ~Ale && ~Ce_n && Wr_Re_n && (Io_mirror [7 : 0] === 8'hD1 || Io_mirror [7 : 0] === 8'hD0))
        col_addr_dis <= 1'b0;  
    else if(Cle && ~Ale && ~Ce_n && Wr_Re_n && col_addr_dis && ~(Io_mirror [7 : 0] === 8'hD1 || Io_mirror [7 : 0] === 8'hD0)) begin
        col_addr_dis <= 1'b0;  
...
//-----------------------------------------------------------------
// Flag for Enhanced Sync Data input & Data output
//-----------------------------------------------------------------

always @(negedge Dqs) begin
    // set these flags at start of data input
    // !datain_sync_enh is used to detect first nedgedge DQS b/c it does not go high immediately, qualify with (Dqs !== 1'bz) to avoid DQS 1->z transition at start of data output
    if (!datain_sync_enh && !any_dout_sync_enh && sync_enh_mode && !Ale && !Cle && !Ce_n_vol && Rb_n_int && Wr_Re_n  && (Dqs !== 1'bz)) begin // die_select not needed
	datain_sync_enh <= #(tWPRE_sync_enh_min -1) 'b1;
	num_warmup_remain = num_warmup_din;
    end
    if (!any_din_sync_enh && !any_dout_sync_enh && sync_enh_mode && !Ale && !Cle && !Ce_n && Wr_Re_n  && (Dqs !== 1'bz)) begin
	first_dqs <= #1 1; // only used in timing checks, #1 delay to avoid Dqs timing check block that will turn first_dqs off
	any_din_sync_enh <= #(tWPRE_sync_enh_min -1) 'b1;
    end
end

always @(negedge Wr_Re_n) begin
    // set these flags at start of data output
    // !data_out_enable_sync_enh is used to detect first nedgedge RE# b/c it does not go high immediately
    datain_sync_enh <= 0;
    any_din_sync_enh <= 0;   
    if (!data_out_enable_sync_enh && sync_enh_mode && !Ale && !Cle && !Ce_n_vol && We_n && die_select) begin
	data_out_enable_sync_enh <= # (tRPRE_sync_enh_min -1) 'b1;
	Dqs_buf <= #tDQSD_sync_enh_max 1'b0;
	//Dq must be driven to some value when the device the driver
	Io_buf  <= #tDQSD_sync_enh_max {DQ_BITS{1'b0}};
	dqs_en <= #tDQSD_sync_enh_max 1;
	num_warmup_remain = num_warmup_dout;
    end
    if (!any_dout_sync_enh && sync_enh_mode && !Ale && !Cle && !Ce_n && We_n) begin
	any_dout_sync_enh <= # (tRPRE_sync_enh_min -1) 'b1;
	first_dqs = 1; // only used in timing checks, reuse this signal instead of creating new first_re_n signal
    end
end

always @(posedge Ce_n or posedge Ale or posedge Cle) begin
    if (sync_enh_mode) begin
	datain_sync_enh <= 0;
	any_din_sync_enh <= 0; // non-blocking for tCALH, tWPST check
	data_out_enable_sync_enh = 0;
	any_dout_sync_enh = 0;
	Dqs_buf <= #tCLHZ_sync_enh_max 1'bz;
	//Dq must be driven to some value when the device the driver
	Io_buf  <= #tCLHZ_sync_enh_max {DQ_BITS{1'bz}};
	dqs_en <= #tCLHZ_sync_enh_max 0;
	IoX_enable <= #tCLHZ_sync_enh_max 1'b0;
	t_readtoz = $realtime + tCLHZ_sync_enh_max;
	IoX        <= #tCLHZ_sync_enh_max 1'b0;
	saw_posedge_dqs <= 0;
    end
end

//-----------------------------------------------------------------
// Flag for Nv_ddr3 Data input & Data output
//-----------------------------------------------------------------

always @(negedge Dqs) begin
    // set these flags at start of data input
    // !datain_nv_ddr3 is used to detect first nedgedge DQS b/c it does not go high immediately, qualify with (Dqs !== 1'bz) to avoid DQS 1->z transition at start of data output
    if (!datain_nv_ddr3 && !any_dout_nv_ddr3 && nv_ddr3 && !Ale && !Cle && !Ce_n_vol && Rb_n_int && Wr_Re_n && (Dqs !== 1'bz)) begin // die_select not needed
	datain_nv_ddr3 <= #(tWPRE_nv_ddr3_min -1) 'b1;
	num_warmup_remain = num_warmup_din;
    end
    if (!any_din_nv_ddr3 && !any_dout_nv_ddr3 && nv_ddr3 && !Ale && !Cle && !Ce_n && Wr_Re_n && (Dqs !== 1'bz)) begin
	first_dqs <= #1 1; // only used in timing checks, #1 delay to avoid Dqs timing check block that will turn first_dqs off
	any_din_nv_ddr3 <= #(tWPRE_nv_ddr3_min -1) 'b1;
    end
end
always @(negedge Wr_Re_n) begin
    // set these flags at start of data output
    // !data_out_enable_nv_ddr3 is used to detect first nedgedge RE# b/c it does not go high immediately
    datain_nv_ddr3 <= 0;
    any_din_nv_ddr3 <= 0;
    if (!data_out_enable_nv_ddr3 && nv_ddr3 && !Ale && !Cle && !Ce_n_vol && We_n && die_select) begin
	data_out_enable_nv_ddr3 <= # (tRPRE_nv_ddr3_min -1) 'b1;
	Dqs_buf <= #tDQSD_nv_ddr3_max 1'b0;
	//Dq must be driven to some value when the device the driver
	Io_buf  <= #tDQSD_nv_ddr3_max {DQ_BITS{1'b0}};
	dqs_en <= #tDQSD_nv_ddr3_max 1;
	num_warmup_remain = num_warmup_dout;
    end
    if (!any_dout_nv_ddr3 && nv_ddr3 && !Ale && !Cle && !Ce_n && We_n) begin
	any_dout_nv_ddr3 <= # (tRPRE_nv_ddr3_min -1) 'b1;
	first_dqs = 1; // only used in timing checks, reuse this signal instead of creating new first_re_n signal
    end
end
always @(posedge Ce_n or posedge Ale or posedge Cle) begin
    if (nv_ddr3) begin
	datain_nv_ddr3 <= 0;
	any_din_nv_ddr3 <= 0; // non-blocking for tCALH, tWPST check
	data_out_enable_nv_ddr3 = 0;
	any_dout_nv_ddr3 = 0;
	Dqs_buf <= #tCLHZ_nv_ddr3_max 1'bz;
	//Dq must be driven to some value when the device the driver
	Io_buf  <= #tCLHZ_nv_ddr3_max {DQ_BITS{1'bz}};
	dqs_en <= #tCLHZ_nv_ddr3_max 0;
	IoX_enable <= #tCLHZ_nv_ddr3_max 1'b0;
	t_readtoz = $realtime + tCLHZ_nv_ddr3_max;
	IoX        <= #tCLHZ_nv_ddr3_max 1'b0;
	saw_posedge_dqs <= 0;
    end
end
//-----------------------------------------------------------------
...
//-----------------------------------------------------------------
    ...
    assign datain_sync = Cle_del & Ale_del & ~Ce_n_vol & Re_n & Rb_n_int & sync_mode & ~sync_enh_mode & ~nv_ddr3; // ??? remove CE# for new parts.

//async mode data input
assign datain_async = ~Cle & ~Ale & ~Ce_n_vol & Re_n & Rb_n_int & ~sync_mode & ~sync_enh_mode & ~nv_ddr3;
always @(posedge We_n) begin
    if (datain_async)
	...
	if (die_select && ~sync_mode && ~sync_enh_mode && ~nv_ddr3) begin
            we_adl_active <= 1'b1;
            tm_we_data_r <= $realtime;
        end
	if (lastCmd === 8'hEF || (lastCmd === 8'hD5) && set_get_feature_lun_addr_ok) begin   // wp_N no effect; set feature by LUN. 
            ...
            case (col_counter)
                0:  begin
                        if((id_reg_addr == 8'h80) & DRIVESTR_EN[1]) 
                            onfi_features[8'h10][7:0]       = Io_mirror;  // for backwards compatability feature address 80h =10h
                        else                     
                            onfi_features[id_reg_addr][7:0] = Io_mirror;
			
			set_ftr_data[7:0] = Io_mirror;    
			    
                    end  //P1
                1:  begin
                        if ((id_reg_addr == 8'h02) || (id_reg_addr == 8'hF5)) begin
                            onfi_features[id_reg_addr][15:8] = Io_mirror;
                        end else begin
                            onfi_features[id_reg_addr][15:8]  = 8'h00;
                        end
			
			set_ftr_data[15:8] = Io_mirror;
			
                    end  //P2
                2:  begin
                        // This is the only feature address that uses sub-parameter bytes 2 and 3
                        if (id_reg_addr == 8'hF5) begin
                            onfi_features[id_reg_addr][23:16] = Io_mirror;
                        end else begin
                            onfi_features[id_reg_addr][23:16]  = 8'h00;
                        end
			
			set_ftr_data[23:16] = Io_mirror;
	...
	else if (lastCmd === 8'hE2) begin // ODT config matrix command; For simplicity, E2h can only turn on ODT enable signals
            case (col_counter)
                0:  set_ftr_data[7:0] = Io_mirror;
                1:  set_ftr_data[15:8] = Io_mirror;	     
                2:  begin
			if (Io_mirror[3:0] != 4'h0) begin 
			    din_ODT_en = 1;
			    $sformat(msg, "Enabling ODT during data input via E2h command");
			    INFO(msg);
			end
			if (Io_mirror[7:4] != 4'h0) begin
			    dout_ODT_en = 1;
			    $sformat(msg, "Enabling DQ/DQS ODT during data output via E2h command");
			    INFO(msg);
			end
			set_ftr_data[23:16] = Io_mirror;
		    end
			...
			if (Io_mirror[3:0] != 4'h0) begin
			    dout_ODT_en = 1;
			    $sformat(msg, "Enabling RE ODT during data output via E2h command");
			    INFO(msg);
			end
			set_ftr_data[31:24] = Io_mirror;
        ...
        // non-nand mode[0] data input
            ...
            if((col_addr + col_counter) <= (NUM_COL - 1))  begin
                if (DEBUG[2]) begin
                    $sformat (msg, "Latch Data (%0h : %0h : %0h + %0h) = %0h",
                        row_addr[active_plane][(ROW_BITS-1):(PAGE_BITS)], row_addr[active_plane][(PAGE_BITS-1):0], col_addr, col_counter, Io_mirror);
                    INFO(msg);
                end
                ...
                if (bypass_cache) begin 
                    data_reg[active_plane] = (data_reg[active_plane] & ~bit_mask) | ( Io_mirror << ((((col_counter+col_addr)*BPC_MAX)+sub_col_cnt)*DQ_BITS)) ;
                    `ifdef PACK
                        case (active_plane)
                        0 : data_reg_packed0 [col_addr + col_counter] = Io_mirror;
                        1 : data_reg_packed1 [col_addr + col_counter] = Io_mirror;
                        2 : data_reg_packed2 [col_addr + col_counter] = Io_mirror;
                        3 : data_reg_packed3 [col_addr + col_counter] = Io_mirror;
                        endcase             
                    `endif
                end else begin 
                    cache_reg[active_plane] = (cache_reg[active_plane] & ~bit_mask) | ( Io_mirror << ((((col_counter+col_addr)*BPC_MAX)+sub_col_cnt)*DQ_BITS));
                    `ifdef PACK
                        case (active_plane)
                        0 : cache_reg_packed0 [col_addr + col_counter] = Io_mirror;
                        1 : cache_reg_packed1 [col_addr + col_counter] = Io_mirror;
                        2 : cache_reg_packed2 [col_addr + col_counter] = Io_mirror;
                        3 : cache_reg_packed3 [col_addr + col_counter] = Io_mirror;
                        endcase             
                    `endif
                end
            ...
            DriveStrength = (Io_mirror & 8'b00001100);
        end
    ...
    always @(Dqs) begin
        if (datain_nv_ddr3 && (num_warmup_remain > 0)) begin // ignore warmup cycles
	    if (!Dqs) num_warmup_remain = num_warmup_remain - 1; // decrement counter on falling edge of DQS
	end else    
        if (datain_sync_enh && (num_warmup_remain > 0)) begin // ignore warmup cycles
	    if (!Dqs) num_warmup_remain = num_warmup_remain - 1; // decrement counter on falling edge of DQS
	end else
	if (datain_sync || datain_sync_enh || datain_nv_ddr3)
        begin : latch_data_sync
            //also need to recognize here that the SET FEATURES command will apply to all LUNs per chip-enable
  	    if (lastCmd === 8'hEF || (lastCmd === 8'hD5) && set_get_feature_lun_addr_ok) begin	    
                //Only allowed for ONFI devices
                case (col_counter)
                0: begin
                    if((id_reg_addr == 8'h80) & DRIVESTR_EN[1]) 
                        onfi_features[8'h10][7:0]       = Io_mirror;
                    else                     
                        onfi_features[id_reg_addr][7:0] = Io_mirror;
		    set_ftr_data[7:0] = Io_mirror;	
                end //P1
                1: begin
                    if ((id_reg_addr == 8'h02) || (id_reg_addr == 8'hF5))
                        onfi_features[id_reg_addr][15:8] = Io_mirror;
                    else 
                        onfi_features[id_reg_addr][15:8]  = 8'h00;
		    set_ftr_data[15:8] = Io_mirror;	
		    ...
		    tWB_check_en = 1'b1;
                    ...
                    Rb_n_int = 1'b0;
                    status_register[6:5]=2'b00;
 		    if ((id_reg_addr == 8'h01) && (sync_mode || sync_enh_mode || nv_ddr3))
                       wait_for_cen <= 1;
                    go_busy(tFEAT);
                    //now update the design based on the input features parameters
                    update_features(id_reg_addr);
		    ...
		    disable_md_stat <= 1'b0;
                ...
                if (~Dqs) //get/set features need same data on both Dqs edges
                    col_counter = col_counter + 1;
            end // end for the lastCmd = EF and D5 
		...
		case (col_counter)
                ...
                1:  set_ftr_data[15:8] = Io_mirror;		
                2:  begin
			if (Dqs && Io_mirror[3:0] != 4'h0) begin 
			    din_ODT_en = 1;
			    $sformat(msg, "Enabling ODT during data input via E2h command");
			    INFO(msg);
			end
			if (Dqs && Io_mirror[7:4] != 4'h0) begin
			    dout_ODT_en = 1;
			    $sformat(msg, "Enabling DQ/DQS ODT during data output via E2h command");
			    INFO(msg);
			end
		...
		endcase
		if (~Dqs) // same data on both Dqs edges
		    col_counter = col_counter + 1;
	    end
			...
			`ifdef PACK
                        ...
                        `ifdef PACK
                         case (active_plane)
                         0 : case (sub_col_cnt)
                            2'b00 : cache_reg_packed0 [col_addr + col_counter][(0*DQ_BITS)+(DQ_BITS-1): 0*DQ_BITS] = Io_mirror;
                            2'b01 : cache_reg_packed0 [col_addr + col_counter][(1*DQ_BITS)+(DQ_BITS-1): 1*DQ_BITS] = Io_mirror;
                            2'b10 : cache_reg_packed0 [col_addr + col_counter][(2*DQ_BITS)+(DQ_BITS-1): 2*DQ_BITS] = Io_mirror;
                            2'b11 : cache_reg_packed0 [col_addr + col_counter][(3*DQ_BITS)+(DQ_BITS-1): 3*DQ_BITS] = Io_mirror;
                            endcase
                         1 : case (sub_col_cnt)
                            2'b00 : cache_reg_packed1 [col_addr + col_counter][(0*DQ_BITS)+(DQ_BITS-1): 0*DQ_BITS] = Io_mirror;
                            2'b01 : cache_reg_packed1 [col_addr + col_counter][(1*DQ_BITS)+(DQ_BITS-1): 1*DQ_BITS] = Io_mirror;
                            2'b10 : cache_reg_packed1 [col_addr + col_counter][(2*DQ_BITS)+(DQ_BITS-1): 2*DQ_BITS] = Io_mirror;
                            2'b11 : cache_reg_packed1 [col_addr + col_counter][(3*DQ_BITS)+(DQ_BITS-1): 3*DQ_BITS] = Io_mirror;
                            endcase 
                         2 : case (sub_col_cnt)
                            2'b00 : cache_reg_packed2 [col_addr + col_counter][(0*DQ_BITS)+(DQ_BITS-1): 0*DQ_BITS] = Io_mirror;
                            2'b01 : cache_reg_packed2 [col_addr + col_counter][(1*DQ_BITS)+(DQ_BITS-1): 1*DQ_BITS] = Io_mirror;
                            2'b10 : cache_reg_packed2 [col_addr + col_counter][(2*DQ_BITS)+(DQ_BITS-1): 2*DQ_BITS] = Io_mirror;
                            2'b11 : cache_reg_packed2 [col_addr + col_counter][(3*DQ_BITS)+(DQ_BITS-1): 3*DQ_BITS] = Io_mirror;
                            endcase 
                         3 : case (sub_col_cnt)
                            2'b00 : cache_reg_packed3 [col_addr + col_counter][(0*DQ_BITS)+(DQ_BITS-1): 0*DQ_BITS] = Io_mirror;
                            2'b01 : cache_reg_packed3 [col_addr + col_counter][(1*DQ_BITS)+(DQ_BITS-1): 1*DQ_BITS] = Io_mirror;
                            2'b10 : cache_reg_packed3 [col_addr + col_counter][(2*DQ_BITS)+(DQ_BITS-1): 2*DQ_BITS] = Io_mirror;
                            2'b11 : cache_reg_packed3 [col_addr + col_counter][(3*DQ_BITS)+(DQ_BITS-1): 3*DQ_BITS] = Io_mirror;
                            endcase 
                         endcase             
                        `endif
                    end 
...
always @(Clk) begin
	...
	if (Clk)    sync_output_active = (sync_mode && Cle && Ale && ~Ce_n_vol && die_select && ~Wr_n);
...
always @(Re_n) begin
    //#############################
    // SYNC ENH MODE OUTPUT
    //#############################
    if (sync_enh_mode) begin
	if (data_out_enable_sync_enh && num_warmup_remain > 0) begin // output warmup cycle
	    sync_output_data(8'hxx);
	    if (!Re_n) num_warmup_remain = num_warmup_remain - 1; // decrement counter on falling edge of Re_n
	end else begin
	    data_out_sync_enh_and_sync;
	end 
    end
...
always @(Re_n) begin
    ...
    //#############################
    if (nv_ddr3) begin
	if (data_out_enable_nv_ddr3 && num_warmup_remain > 0) begin // output warmup cycle
	    sync_output_data(8'hxx);
	    if (!Re_n) num_warmup_remain = num_warmup_remain - 1; // decrement counter on falling edge of Re_n
	end else begin
	    data_out_sync_enh_and_sync;
...
task data_out_sync_enh_and_sync;
	...
	if ((sync_output_active || data_out_enable_sync_enh || data_out_enable_nv_ddr3) && status_cmnd) begin
            output_status;
        end else begin
	    if (((sync_output_active && (Clk || (~Clk && saw_posedge_dqs))) || data_out_enable_sync_enh || data_out_enable_nv_ddr3) && (Rb_n_int === 1'b1)) begin : not_busy_sync
                ...
                if (do_read_id_2) begin
                    ...
                    col_counter = col_counter + 1;
	       end else if (lastCmd === 8'hEE || (lastCmd === 8'hD4) && set_get_feature_lun_addr_ok) begin 
                    //-----------------
                    //Read Features
                    //-----------------
                    case (col_counter)
                        ...
                        default : data = 8'h00;
                    endcase
		    if(col_counter == 7) set_get_feature_lun_addr_ok = 1'b0;
		    sync_output_data(data);
                ...
                //-----------------
                    ...
                    if (sync_enh_mode) 
			rd_out <= #(tDQSRE_sync_enh_max) 1'b0;
		    else if (nv_ddr3)
		        rd_out <= #(tDQSRE_nv_ddr3_max) 1'b0;
                    else
			    ...
			    `endif
                        ...
                        case (col_counter)
                            ...
                            4,5     : data = 8'h46; //'F'
                            6,7     : data = 8'h49; //'I'
			    8,9     : if(nv_ddr3) data = 8'h01;
			              else        data = 8'h00;
			    10,11   : data = 8'h00;
                            default : begin
                               $sformat(msg, "ONFI read beyond 6 bytes is indeterminate.");
                               ERROR(ERR_MISC, msg);
                               data = {DQ_BITS{1'bx}};
                            end
                        endcase
		  ...
		  else if (sync_enh_mode) 
		    rd_out <= #tAC_sync_enh_max 1'b0;  
		  else if (nv_ddr3) 
		    rd_out <= #tAC_nv_ddr3_max 1'b0;  
		  else
		    rd_out <= #tAC_sync_max 1'b0;		    
...
assign data_out_enable_async = (~sync_mode && ~sync_enh_mode && ~nv_ddr3 && ~Cle && ~Ale && ~Ce_n_vol && We_n && die_select);
always @ (negedge Re_n) begin
    ...
    if (die_select && ~sync_mode && ~sync_enh_mode && ~nv_ddr3 ) begin : rd_die_select
		...
		if(col_counter == 3) set_get_feature_lun_addr_ok = 1'b0;
		...
		if (id_reg_addr === 8'h00) begin : regular_id_read
			...
			4:  Io_buf <= #(tREA_max) 8'h00; 
			5:  Io_buf <= #(tREA_max) 8'h00;
                    endcase
...
always @ (posedge We_n) begin 
    ...
    if (command_enable && ($realtime - tm_vol_sel < tVDLY_max)) begin $sformat(msg,"tVDLY violation by %t", tVDLY_max - ($realtime - tm_vol_sel)); ERROR(ERR_TIM, msg); end
    if (command_enable && ($realtime - tm_eni_r < tENi_max)) begin $sformat(msg,"tENi violation by %t", tENi_max - ($realtime - tm_eni_r)); ERROR(ERR_TIM, msg); end
end 

always @ (We_n) begin
  if (~sync_mode & ~sync_enh_mode & ~nv_ddr3 & PowerUp_Complete) begin
...
always @ (Ce_n) begin
    if (~Ce_n) begin
        tm_ce_n_f <= $realtime;
	ENo <= #(tENo_max) (volume_assignment_done ? 1'bz : 1'b0);
	first_cmd_after_ce <= 1;
    end else begin
	if (PowerUp_Complete && ~sync_mode && ~sync_enh_mode && ~nv_ddr3) begin
            if (cache_op === 1) begin
                // special cache mode timing checks
...
always @ (Io_buf) begin
  if (~sync_mode && ~sync_enh_mode && ~nv_ddr3 && ~Ce_n) begin
...
begin
    ...
    while (delay > 0) begin
	   ...
	   if ((Io_mirror [7 : 0] !== 8'h78) && (Io_mirror [7 : 0] !== 8'h70)) begin  // tWB check should be ok if we have status command or status enhanced command in between
	       $sformat(msg, "Do not issue a new command during tWB, even if R/B# or RDY is ready"); ERROR(ERR_CMD, msg);
	   end else begin
	      if (Io_mirror [7 : 0] == 8'h70) begin
	          cmnd_70h = 1'b1;
	      end else if (Io_mirror [7 : 0] == 8'h78) begin  
	          cmnd_78h = 1'b1;
	      end 
	   end       
        end
...
endtask
    ...
    always @ (Re_n) begin
        dqs_enable = data_out_enable_sync_enh;
        if (sync_enh_mode && dqs_enable) begin
            if (Re_n) begin
                saw_posedge_dqs <= 1;
                Dqs_buf <= #tDQSRE_sync_enh_max 1'b1;
            end else begin
                if (saw_posedge_dqs) begin
                    Dqs_buf <= #tDQSRE_sync_enh_max 1'b0;
                end
            end
        end
    end
    ...
    always @ (Re_n) begin
        dqs_enable = data_out_enable_nv_ddr3;
        if (nv_ddr3 && dqs_enable) begin
            if (Re_n) begin
                saw_posedge_dqs <= 1;
                Dqs_buf <= #tDQSRE_nv_ddr3_max 1'b1;
            end else begin
                if (saw_posedge_dqs) begin
                    Dqs_buf <= #tDQSRE_nv_ddr3_max 1'b0;
                end
            end
        end
    end
...
*/
    ...
    begin
        ...
        3'b000: begin
           ...
           // if (lastState == 3'b011) check_idle <= #((tCKWR_sync_min-1)*tCK_sync-1) 1'b1;
            if (lastState == 3'b011) check_idle <= 1'b1; // if we are pausing the data in middle of data transfer and we need to wait only for tCAD to restart data
            lastState <= 3'b000;
        ...
        3'b011 : begin
           // added this condition to get rid of tCAD voilation if the Data is paused during data output
       //    if (lastState == 3'b000)
       //       tm_cad_r <= $realtime - (((tCKWR_sync_min-1)*tCK_sync-1));  
        ...
        3'b111 : begin
	    if (lastState !== 3'b100) // adding this to prevent a race between sampling lastState during data idle and data output 
              lastState <= {Wr_n,Cle,Ale}; //Data Write
            else begin 
              lastState <= #(tCK_sync / 2) {Wr_n,Cle,Ale};
            end
	  end
        endcase
    ...
    always @ (posedge Clk) begin
	   ...
	   if (($realtime - tm_ce_n_f) >= tCS_sync_min) begin
	   	tcs_sync_chk_en <= 1'b0;
	   end else if ((Cle || Ale) && (Io_mirror[7:0] !== 8'hFF)) begin 
    ...
    always @ (Clk) begin
      if (sync_mode && ~Ce_n ) begin
        if (Clk && die_select) begin //posedge clk
            ...
            if (Cle && Ale && Clk) begin //posedge Clk with data access
                ...
                if (($realtime - tm_clk_r < tWHR_sync_min) && (lastState == 3'b110) && ~Wr_n && die_select) begin $sformat(msg,"Sync mode : tWHR violation by %t ", tm_clk_r + tWHR_sync_min - $realtime); ERROR(ERR_TIM, msg); end
        ...
        if (~Ce_n && ~Clk) begin : negedge_Clk
            if (~new_clk && (tm_ce_n_f != $realtime) && die_select) begin
    ...
    always @(posedge Clk) begin
            if (sync_mode && ~Ce_n && Wr_n && die_select) begin
    ...
    always @ (Ce_n_i) begin // Ce_n_i is the direct input, while Ce_n is just aliased
        if (Ce_n_i && sync_enh_mode && timezero) begin
	    if (($realtime - tm_we_n_r) < tCH_sync_enh_min) begin $sformat(msg, "Sync_Enh mode : tCH violation on We_n by %0t", tCH_sync_enh_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end
            if (($realtime - tm_re_n_f) < tCH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tCH violation on Re_n by %0t", tCH_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tCH_sync_enh_min) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tCH violation on Dqs by %0t", tCH_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tWPST_sync_enh_min) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tWPST violation by %0t", tWPST_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_f) < tRPST_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tRPST violation by %0t", tRPST_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
        end
        if (~Ce_n_i && sync_enh_mode && timezero) begin
            if (($realtime - tm_dqs_r) < tDBS_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tDBS violation by %0t", tDBS_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_r) < tDBS_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tDBS violation by %0t", tDBS_sync_enh_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM,msg); end 
	end
	if (Ce_n_i && ($realtime - tm_vol_sel < tVDLY_max) && ($realtime > tVDLY_max)) begin $sformat(msg,"tCEVDLY violation by %t", tVDLY_max - ($realtime - tm_vol_sel)); ERROR(ERR_TIM, msg); end
        first_tDSC <= 1'b1;  //used to indicate second DQS clock edge for timing checks below
        first_tRC <= 1'b1;  //used to indicate second RE clock edge for timing checks below
    end

    always @ (We_n) begin
	if (sync_enh_mode & PowerUp_Complete) begin
	    if (~We_n) begin // negedge We_n
		if (~Ce_n) begin
		    if ($realtime - tm_we_n_f < tWC_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tWC violation by %t ", tm_we_n_f + tWC_sync_enh_min - $realtime); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_we_n_r < tWH_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tWH violation by %t ", tm_we_n_r + tWH_sync_enh_min - $realtime); ERROR(ERR_TIM, msg); end
		    tm_we_n_f <= $realtime;
		end
	    end else begin // posedge We_n
		if (~Ce_n) begin
		    if ($realtime - tm_we_n_f < tWP_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tWP violation by %t ", tWP_sync_enh_min - ($realtime - tm_we_n_f)); ERROR(ERR_TIM, msg); end
		    if (Ale) begin
			if (($realtime - tm_ale_r) < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Ale tCALS violation on We_n by %0t", tCALS_sync_enh_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_dq) < tCAS_sync_enh_min) begin $sformat(msg, "Sync_Enh mode : Ale tCAS violation by %0t", tCAS_sync_enh_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
		    end
		    if (Cle) begin
			if (($realtime - tm_cle_r) < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cle tCALS violation on We_n by %0t", tCALS_sync_enh_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_dq) < tCAS_sync_enh_min) begin $sformat(msg, "Sync_Enh mode : Cle tCAS violation by %0t", tCAS_sync_enh_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
		    end
		    if ($realtime - tm_ce_n_f < tCS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cmd/Addr tCS violation by %t ", tCS_sync_enh_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_wp_n < tWW_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tWW violation by %t ", tWW_sync_enh_min - ($realtime - tm_wp_n)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_re_n_r < tRHW_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tRHW violation by %t ", tRHW_sync_enh_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		end
		tm_we_n_r <= $realtime;
		if (Ale) tm_we_n_r_ale <= $realtime;
	    end
	...
	if (sync_enh_mode & PowerUp_Complete) begin
	    if (~Re_n) begin // negedge_Re_n
		if (~Ce_n_vol) begin
		    if ((lastCmd == 8'hE0) && ~status_cmnd && ~cmnd_78h && die_select) begin
			if ($realtime - tm_we_n_r < tCCS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tCCS violation by %t", tCCS_sync_enh_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end
		    end
		    if ($realtime - tm_ale_f < tAR_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tAR violation by %t ", tAR_sync_enh_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_cle_f < tCLR_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tCLR violation by %t ", tCLR_sync_enh_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_ce_n_f < tCR_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tCR violation by %t ", tCR_sync_enh_min - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
                    if (($realtime - tm_re_n_r < tREH_sync_enh_min - TS_RES_ADJUST) && !first_tRC) begin $sformat(msg,"Sync_Enh mode : tREH min violation by %t", tREH_sync_enh_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		    //if (($realtime - tm_re_n_r > tREH_sync_enh_max + TS_RES_ADJUST) && any_dout_sync_enh && !first_tRC)
			//begin $sformat(msg,"Sync_Enh Mode : tREH max violation by %t", ($realtime - tm_re_n_r) - tREH_sync_enh_max); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_we_n_r < tWHR_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tWHR violation by %t ", tWHR_sync_enh_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM, msg); end
                    if (($realtime - tm_rb_n_r < tRR_sync_enh_min) && ~status_cmnd && die_select) begin $sformat(msg,"Sync_Enh mode : tRR violation by %t ", tRR_sync_enh_min - ($realtime - tm_rb_n_r)); ERROR(ERR_TIM, msg); end
		end
		tm_re_n_f <= $realtime;
	    end else begin // posedge_Re_n
		if (~Ce_n) begin
		    if (first_dqs) begin
			if (($realtime - tm_re_n_f < tRPRE_sync_enh_min - TS_RES_ADJUST) && !dout_ODT_en) begin $sformat(msg,"Sync_Enh mode : tRPRE min violation by %t", tRPRE_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_re_n_f < tRPRE2_sync_enh_min - TS_RES_ADJUST) && dout_ODT_en) begin $sformat(msg,"Sync_Enh mode : tRPRE2 min violation by %t", tRPRE2_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
			if (dout_ODT_en) begin
			    if ($realtime - tm_ale_f < tCALS2_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Ale tCALS2 violation on Re_n by %0t", tCALS2_sync_enh_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			    if ($realtime - tm_cle_f < tCALS2_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cle tCALS2 violation on Re_n by %0t", tCALS2_sync_enh_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
			end else begin
			    if ($realtime - tm_ale_f < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Ale tCALS violation on Re_n by %0t", tCALS_sync_enh_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			    if ($realtime - tm_cle_f < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cle tCALS violation on Re_n by %0t", tCALS_sync_enh_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
			end
			if (($realtime - tm_ce_n_f < tCS1_sync_enh_min) && !dout_ODT_en) begin $sformat(msg,"Sync_Enh mode : tCS1 violation on Re_n by %t ", tCS1_sync_enh_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_ce_n_f < tCS2_sync_enh_min) && dout_ODT_en) begin $sformat(msg,"Sync_Enh mode : tCS2 violation on Re_n by %t ", tCS2_sync_enh_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    end else begin
			if (first_tRC && !Ale && !Cle && !Ce_n) begin // !Cle for the case where we only read 1 byte, in which case we would continue to wait for first tRC
			    first_tRC = 0; // blocking so that we do check first tRC/tRP on this clk edge
			    tRC_sync_enh = $realtime - tm_re_n_r;
			    update_clock_parameters;  //only need to do this update after the first tRC_sync_enh calculation after device becomes active
			    if (DEBUG[0]) $display("new RE clk : tRC_sync_enh_min=%0t, tRC_sync_enh=%0t", tRC_sync_enh_min, tRC_sync_enh);
			    if ((tm_re_n_f - tm_re_n_r < tREH_sync_enh_min - TS_RES_ADJUST) && !first_tRC) // Do first tREH checks now b/c we skipped it before
				begin $sformat(msg,"Sync_Enh mode : first tREH min violation by %t", tREH_sync_enh_min - (tm_re_n_f - tm_re_n_r)); ERROR(ERR_TIM, msg); end
			    //if ((tm_re_n_f - tm_re_n_r > tREH_sync_enh_max + TS_RES_ADJUST) && any_dout_sync_enh && !first_tRC)
			//	begin $sformat(msg,"Sync_Enh Mode : first tREH max violation by %t", (tm_re_n_f - tm_re_n_r) - tREH_sync_enh_max); ERROR(ERR_TIM, msg); end
			end
			//if (($realtime - tm_re_n_f > tRP_sync_enh_max + TS_RES_ADJUST) && any_dout_sync_enh && !first_tRC)
			 //   begin $sformat(msg,"Sync_Enh Mode : tRP max violation by %t", ($realtime - tm_re_n_f) - tRP_sync_enh_max); ERROR(ERR_TIM, msg); end
		    end
		    if (($realtime - tm_re_n_r < tRC_sync_enh_min) && !first_tRC) begin $sformat(msg,"Sync_Enh mode : tRC violation by %t ", tRC_sync_enh_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_re_n_f < tRP_sync_enh_min - TS_RES_ADJUST) && !first_tRC) begin $sformat(msg,"Sync_Enh mode : tRP min violation by %t", tRP_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_ale_r) < tRPSTH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tRPSTH violation by %0t", tRPSTH_sync_enh_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM,msg); end
		    if (($realtime - tm_cle_r) < tRPSTH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tRPSTH violation by %0t", tRPSTH_sync_enh_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM,msg); end
		end
		if (($realtime - tm_ce_n_r) < tRPSTH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tRPSTH violation by %0t", tRPSTH_sync_enh_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end
		first_dqs <= 0;
		tm_re_n_r <= $realtime;
	    end
	end
    ...
    always @(negedge Ale or negedge Cle) begin
        if(sync_enh_mode && ~Ce_n) begin
            if (($realtime - tm_we_n_r) < tCALH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tCALH violation by %0t", tCALH_sync_enh_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_dqs_r) < tDBS_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tDBS violation by %0t", tDBS_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_r) < tDBS_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tDBS violation by %0t", tDBS_sync_enh_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM,msg); end 
	end
        if(sync_enh_mode && ~Ce_n) begin 
            if (($realtime - tm_ce_n_r) < tCSD_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tCSD violation by %0t", tCSD_sync_enh_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end	    
	    else begin
	    #(TS_RES_ADJUST); // check again 0.001 ns later in case host brought all 3 signals high at same time
            if (($realtime - tm_ce_n_r) < tCSD_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tCSD violation by %0t", tCSD_sync_enh_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end	    
	    end
	end
    end

    always @(posedge Ale or posedge Cle) begin
        if(sync_enh_mode && ~Ce_n) begin
            if (($realtime - tm_re_n_f) < tCALH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tCALH violation on Re_n by %0t", tCALH_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tCALH_sync_enh_min) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tCALH violation on Dqs by %0t", tCALH_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tWPST_sync_enh_min) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tWPST violation by %0t", tWPST_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_f) < tRPST_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tRPST violation by %0t", tRPST_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
        end
        if(sync_enh_mode && ~Ce_n) begin 
    ...
    // Need the extra 0.001 in some of these cases to get around the simulator rounding errors
    always @(Dqs) begin
        if (sync_enh_mode && ~Ce_n && any_din_sync_enh) begin  //data & dqs input timing checks
            if (($realtime - tm_dq) < (tDS_sync_enh_min - TS_RES_ADJUST)) begin $sformat(msg, "Sync_Enh mode : tDS violation by %0t", tDS_sync_enh_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
            if (Dqs) begin
                if (first_dqs) begin // posedge Dqs
                    if (din_ODT_en) begin
			if (($realtime - tm_dqs_f) < tWPRE2_sync_enh_min) begin $sformat(msg,"Sync_Enh Mode : tWPRE2 violation by %0t", tWPRE2_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end
			if (($realtime - tm_dqs_r) < (tWPRE2_sync_enh_min + tCDQSS_sync_enh_min)) begin $sformat(msg,"Sync_Enh Mode : tCDQSS violation by %0t", tWPRE2_sync_enh_min + tCDQSS_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end else begin
			if (($realtime - tm_dqs_f) < tWPRE_sync_enh_min) begin $sformat(msg,"Sync_Enh Mode : tWPRE violation by %0t", tWPRE_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end
			if (($realtime - tm_dqs_r) < (tWPRE_sync_enh_min + tCDQSS_sync_enh_min)) begin $sformat(msg,"Sync_Enh Mode : tCDQSS violation by %0t", tWPRE_sync_enh_min + tCDQSS_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end
		    if (din_ODT_en) begin
			if (($realtime - tm_ale_f) < tCALS2_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Ale tCALS2 violation on Dqs by %0t", tCALS2_sync_enh_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_cle_f) < tCALS2_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cle tCALS2 violation on Dqs by %0t", tCALS2_sync_enh_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    end else begin
			if (($realtime - tm_ale_f) < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Ale tCALS violation on Dqs by %0t", tCALS_sync_enh_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_cle_f) < tCALS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : Cle tCALS violation on Dqs by %0t", tCALS_sync_enh_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    end
		    if ((($realtime - tm_ce_n_f) < tCS1_sync_enh_min) && !din_ODT_en) begin $sformat(msg,"Sync_Enh mode : tCS1 violation on Dqs by %t ", tCS1_sync_enh_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if ((($realtime - tm_ce_n_f) < tCS2_sync_enh_min) && din_ODT_en) begin $sformat(msg,"Sync_Enh mode : tCS2 violation on Dqs by %t ", tCS2_sync_enh_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_we_n_r_ale) < tADL_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tADL violation by %t ", tADL_sync_enh_min - ($realtime - tm_we_n_r_ale)); ERROR(ERR_TIM, msg); end
		    if (cmnd_85h && ~status_cmnd && ~cmnd_78h && die_select) begin
			if ($realtime - tm_we_n_r_ale < tCCS_sync_enh_min) begin $sformat(msg,"Sync_Enh mode : tCCS violation on We_n by %t", tm_we_n_r_ale + tCCS_sync_enh_min - $realtime); ERROR(ERR_TIM,msg); end
                    end
		end else begin
		    if (first_tDSC) begin // no need to qualify with !Cle as in first_tRC b/c we qualify with any_din_sync_enh above
			first_tDSC = 0; // blocking so that we do check first tDQSL/tDSC on this clk edge
			tDSC_sync_enh = $realtime - tm_dqs_r;
			
			update_clock_parameters;  //only need to do this update after the first tDSC_sync_enh calculation after device becomes active
	                if (DEBUG[0]) $display("new DQS clk : tDSC_sync_enh_min=%0t, tDSC_sync_enh=%0t", tDSC_sync_enh_min, tDSC_sync_enh);
			if ((tm_dqs_f - tm_dqs_r) < tDQSH_sync_enh_min - TS_RES_ADJUST) // Do first tDQSH check now b/c we skipped it before
			    begin $sformat(msg, "Sync_Enh Mode : first tDQSH min pulse width violation by %0t", tDQSH_sync_enh_min - (tm_dqs_f - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end
                    if ((($realtime - tm_dqs_f) < tDQSL_sync_enh_min - TS_RES_ADJUST) && !first_tDSC) begin $sformat(msg, "Sync_Enh Mode : tDQSL min pulse width violation by %0t", tDQSL_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg);end
                    if ((($realtime - tm_dqs_r) < tDSC_sync_enh_min) && !first_tDSC) begin $sformat(msg,"Sync_Enh Mode : tDSC violation by %0t", tDSC_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		end
		first_dqs <= 0;
            end else begin // negedge Dqs
		if (!first_dqs && !first_tDSC) begin
                    if (($realtime - tm_dqs_r) < tDQSH_sync_enh_min - TS_RES_ADJUST) begin $sformat(msg, "Sync_Enh Mode : tDQSH min pulse width violation by %0t", tDQSH_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg);end
                end
            end
	end
	if (sync_enh_mode && ~Ce_n) begin // update time values after the checks
            if (Dqs) begin
                tm_dqs_r <= $realtime;
            end else begin
		tm_dqs_f <= $realtime;
            end	
	    //check for postamble hold constraint 
            if (Dqs) begin
		if (($realtime - tm_ale_r) < tWPSTH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tWPSTH violation by %0t", tWPSTH_sync_enh_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM,msg); end
		if (($realtime - tm_cle_r) < tWPSTH_sync_enh_min) begin $sformat(msg, "Sync_Enh Mode : tWPSTH violation by %0t", tWPSTH_sync_enh_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM,msg); end
	    end
        end
	if (sync_enh_mode && Dqs && (($realtime - tm_ce_n_r) < tWPSTH_sync_enh_min)) begin $sformat(msg, "Sync_Enh Mode : tWPSTH violation by %0t", tWPSTH_sync_enh_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end
	if (sync_enh_mode && ~Ce_n && (Dqs === 1'bz) && first_dqs) begin // Host releases Dqs to zz at start of data output
	    if ((($realtime - tm_re_n_f) < tDQSRH_sync_enh_min - TS_RES_ADJUST)) begin $sformat(msg, "Sync_Enh Mode : tDQSRH violation by %0t", tDQSRH_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg);end
	    else begin
	    #(TS_RES_ADJUST); // check again 0.001 ns later in case host changed both signals at same time
	    if ((($realtime - tm_re_n_f) < tDQSRH_sync_enh_min - TS_RES_ADJUST)) begin $sformat(msg, "Sync_Enh Mode : tDQSRH violation by %0t", tDQSRH_sync_enh_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg);end
	    end
	end
    end
    ...
    always @(Io) begin
        if (sync_enh_mode && ~Ce_n) begin
            if (!Ale && !Cle) begin
                if (($realtime - tm_dqs_r) < (tDH_sync_enh_min - TS_RES_ADJUST) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tDH violation by %0t", tDH_sync_enh_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
                if (($realtime - tm_dqs_f) < (tDH_sync_enh_min - TS_RES_ADJUST) && any_din_sync_enh) begin $sformat(msg, "Sync_Enh Mode : tDH violation by %0t", tDH_sync_enh_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            end else if (Ale || Cle) begin
                if (($realtime - tm_we_n_r) < (tCAH_sync_enh_min - TS_RES_ADJUST)) begin $sformat(msg, "Sync_Enh Mode : tCAH violation by %0t", tCAH_sync_enh_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end 
            end
            tm_dq <= $realtime;
        end
    end
    ...
    always @ (Ce_n_i) begin // Ce_n_i is the direct input, while Ce_n is just aliased
        if (Ce_n_i && nv_ddr3 && timezero) begin
	    if (($realtime - tm_we_n_r) < tCH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 mode : tCH violation on We_n by %0t", tCH_nv_ddr3_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end
            if (($realtime - tm_re_n_f) < tCH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tCH violation on Re_n by %0t", tCH_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tCH_nv_ddr3_min) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tCH violation on Dqs by %0t", tCH_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tWPST_nv_ddr3_min) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tWPST violation by %0t", tWPST_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_f) < tRPST_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tRPST violation by %0t", tRPST_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
        end
        if (~Ce_n_i && nv_ddr3 && timezero) begin
            if (($realtime - tm_dqs_r) < tDBS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tDBS violation by %0t", tDBS_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_r) < tDBS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tDBS violation by %0t", tDBS_nv_ddr3_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM,msg); end 
	end
	if (Ce_n_i && ($realtime - tm_vol_sel < tVDLY_max) && ($realtime > tVDLY_max)) begin $sformat(msg,"tCEVDLY violation by %t", tVDLY_max - ($realtime - tm_vol_sel)); ERROR(ERR_TIM, msg); end
        first_tDSC <= 1'b1;  //used to indicate second DQS clock edge for timing checks below
        first_tRC <= 1'b1;  //used to indicate second RE clock edge for timing checks below
    ...
    always @ (We_n) begin
	if (nv_ddr3 & PowerUp_Complete) begin
	    if (~We_n) begin // negedge We_n
		if (~Ce_n) begin
		    if ($realtime - tm_we_n_f < tWC_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tWC violation by %t ", tm_we_n_f + tWC_nv_ddr3_min - $realtime); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_we_n_r < tWH_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tWH violation by %t ", tm_we_n_r + tWH_nv_ddr3_min - $realtime); ERROR(ERR_TIM, msg); end
		    tm_we_n_f <= $realtime;
		end
	    end else begin // posedge We_n
		if (~Ce_n) begin
		    if ($realtime - tm_we_n_f < tWP_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tWP violation by %t ", tWP_nv_ddr3_min - ($realtime - tm_we_n_f)); ERROR(ERR_TIM, msg); end
		    if (Ale) begin
			if (($realtime - tm_ale_r) < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Ale tCALS violation on We_n by %0t", tCALS_nv_ddr3_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_dq) < tCAS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 mode : Ale tCAS violation by %0t", tCAS_nv_ddr3_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
		    end
		    if (Cle) begin
			if (($realtime - tm_cle_r) < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Cle tCALS violation on We_n by %0t", tCALS_nv_ddr3_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_dq) < tCAS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 mode : Cle tCAS violation by %0t", tCAS_nv_ddr3_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
		    end
		    if ($realtime - tm_ce_n_f < tCS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Cmd/Addr tCS violation by %t ", tCS_nv_ddr3_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_wp_n < tWW_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tWW violation by %t ", tWW_nv_ddr3_min - ($realtime - tm_wp_n)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_re_n_r < tRHW_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tRHW violation by %t ", tRHW_nv_ddr3_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		end
		tm_we_n_r <= $realtime;
		if (Ale) tm_we_n_r_ale <= $realtime;
	    end
	...
	if (nv_ddr3 & PowerUp_Complete) begin
		...
		if (~Ce_n) begin
		    if ((lastCmd == 8'hE0) && ~status_cmnd && ~cmnd_78h && die_select) begin
			if ($realtime - tm_we_n_r < tCCS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tCCS violation by %t", tCCS_nv_ddr3_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end
		    end
		    if ($realtime - tm_ale_f < tAR_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tAR violation by %t ", tAR_nv_ddr3_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_cle_f < tCLR_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tCLR violation by %t ", tCLR_nv_ddr3_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_ce_n_f < tCR_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tCR violation by %t ", tCR_nv_ddr3_min - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
                    if (($realtime - tm_re_n_r < tREH_nv_ddr3_min - TS_RES_ADJUST) && !first_tRC) begin $sformat(msg,"Nv_ddr3 mode : tREH min violation by %t", tREH_nv_ddr3_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		    //if (($realtime - tm_re_n_r > tREH_nv_ddr3_max + TS_RES_ADJUST) && any_dout_nv_ddr3 && !first_tRC) begin $sformat(msg,"Nv_ddr3 Mode : tREH max violation by %t", ($realtime - tm_re_n_r) - tREH_nv_ddr3_max); ERROR(ERR_TIM, msg); end
		    if ($realtime - tm_we_n_r < tWHR_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : tWHR violation by %t ", tWHR_nv_ddr3_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM, msg); end
                    if (($realtime - tm_rb_n_r < tRR_nv_ddr3_min) && ~status_cmnd && die_select) begin $sformat(msg,"Nv_ddr3 mode : tRR violation by %t ", tRR_nv_ddr3_min - ($realtime - tm_rb_n_r)); ERROR(ERR_TIM, msg); end
		end
		tm_re_n_f <= $realtime;
	    end else begin // posedge_Re_n
		if (~Ce_n) begin
		    if (first_dqs) begin
			if (($realtime - tm_re_n_f < tRPRE_nv_ddr3_min - TS_RES_ADJUST) && !dout_ODT_en) begin $sformat(msg,"Nv_ddr3 mode : tRPRE min violation by %t", tRPRE_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_re_n_f < tRPRE2_nv_ddr3_min - TS_RES_ADJUST) && dout_ODT_en) begin $sformat(msg,"Nv_ddr3 mode : tRPRE2 min violation by %t", tRPRE2_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
			if (dout_ODT_en) begin
			    if ($realtime - tm_ale_f < tCALS2_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Ale tCALS2 violation on Re_n by %0t", tCALS2_nv_ddr3_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			    if ($realtime - tm_cle_f < tCALS2_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Cle tCALS2 violation on Re_n by %0t", tCALS2_nv_ddr3_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
			end else begin
			    if ($realtime - tm_ale_f < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Ale tCALS violation on Re_n by %0t", tCALS_nv_ddr3_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			    if ($realtime - tm_cle_f < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 mode : Cle tCALS violation on Re_n by %0t", tCALS_nv_ddr3_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
			end
			if (($realtime - tm_ce_n_f < tCS1_nv_ddr3_min) && !dout_ODT_en) begin $sformat(msg,"Nv_ddr3 mode : tCS1 violation on Re_n by %t ", tCS1_nv_ddr3_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
			if (($realtime - tm_ce_n_f < tCS2_nv_ddr3_min) && dout_ODT_en) begin $sformat(msg,"Nv_ddr3 mode : tCS2 violation on Re_n by %t ", tCS2_nv_ddr3_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    end else begin
			if (first_tRC && !Ale && !Cle && !Ce_n) begin // !Cle for the case where we only read 1 byte, in which case we would continue to wait for first tRC
			    first_tRC = 0; // blocking so that we do check first tRC/tRP on this clk edge
			    tRC_nv_ddr3 = $realtime - tm_re_n_r;
			    			    
			    update_clock_parameters;  //only need to do this update after the first tRC_sync_enh calculation after device becomes active
			    if (DEBUG[0]) $display("new RE clk : tRC_nv_ddr3_min=%0t, tRC_nv_ddr3=%0t", tRC_nv_ddr3_min, tRC_nv_ddr3);
			    if ((tm_re_n_f - tm_re_n_r < tREH_nv_ddr3_min - TS_RES_ADJUST) && !first_tRC) // Do first tREH checks now b/c we skipped it before
				begin $sformat(msg,"Nv_ddr3 mode : first tREH min violation by %t", tREH_nv_ddr3_min - (tm_re_n_f - tm_re_n_r)); ERROR(ERR_TIM, msg); end
			    //if ((tm_re_n_f - tm_re_n_r > tREH_nv_ddr3_max + TS_RES_ADJUST) && any_dout_nv_ddr3 && !first_tRC)
			//	begin $sformat(msg,"Nv_ddr3 Mode : first tREH max violation by %t", (tm_re_n_f - tm_re_n_r) - tREH_nv_ddr3_max); ERROR(ERR_TIM, msg); end
			end
			//if (($realtime - tm_re_n_f > tRP_nv_ddr3_max + TS_RES_ADJUST) && any_dout_nv_ddr3 && !first_tRC)
			  //  begin $sformat(msg,"Nv_ddr3 Mode : tRP max violation by %t", ($realtime - tm_re_n_f) - tRP_nv_ddr3_max); ERROR(ERR_TIM, msg); end
		    end
		    if (($realtime - tm_re_n_r < tRC_nv_ddr3_min) && !first_tRC) begin $sformat(msg,"Nv_ddr3 mode : tRC violation by %t ", tRC_nv_ddr3_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_re_n_f < tRP_nv_ddr3_min - TS_RES_ADJUST) && !first_tRC) begin $sformat(msg,"Nv_ddr3 mode : tRP min violation by %t", tRP_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_ale_r) < tRPSTH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tRPSTH violation by %0t", tRPSTH_nv_ddr3_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM,msg); end
		    if (($realtime - tm_cle_r) < tRPSTH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tRPSTH violation by %0t", tRPSTH_nv_ddr3_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM,msg); end
		end
		if (($realtime - tm_ce_n_r) < tRPSTH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tRPSTH violation by %0t", tRPSTH_nv_ddr3_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end
		first_dqs <= 0;
		tm_re_n_r <= $realtime;
	    end
	end
    ...
    always @(negedge Ale or negedge Cle) begin
        if(nv_ddr3 && ~Ce_n) begin
            if (($realtime - tm_we_n_r) < tCALH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tCALH violation by %0t", tCALH_nv_ddr3_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_dqs_r) < tDBS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tDBS violation by %0t", tDBS_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_r) < tDBS_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tDBS violation by %0t", tDBS_nv_ddr3_min - ($realtime - tm_re_n_r)); ERROR(ERR_TIM,msg); end 
	end
        
	if(nv_ddr3 & timezero) begin
            if (($realtime - tm_ce_n_r) < tCSD_nv_ddr3_min) begin $sformat(msg, "fffNv_ddr3 Mode : tCSD violation by %0t", tCSD_nv_ddr3_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end	    
	    else begin
	    #(TS_RES_ADJUST); // check again 0.001 ns later in case host brought all 3 signals high at same time
            if (($realtime - tm_ce_n_r) < tCSD_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tCSD violation by %0t", tCSD_nv_ddr3_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end	    
	    end
	end
    end
    ...
    always @(posedge Ale or posedge Cle) begin
        if(nv_ddr3 && ~Ce_n) begin
            if (($realtime - tm_re_n_f) < tCALH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tCALH violation on Re_n by %0t", tCALH_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tCALH_nv_ddr3_min) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tCALH violation on Dqs by %0t", tCALH_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if ((($realtime - tm_dqs_f) < tWPST_nv_ddr3_min) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tWPST violation by %0t", tWPST_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            if (($realtime - tm_re_n_f) < tRPST_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tRPST violation by %0t", tRPST_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg); end 
        end
        if(nv_ddr3) begin
    ...
    // Need the extra 0.001 in some of these cases to get around the simulator rounding errors
    always @(Dqs) begin
        if (nv_ddr3 && ~Ce_n && any_din_nv_ddr3) begin  //data & dqs input timing checks
            if (($realtime - tm_dq) < (tDS_nv_ddr3_min - TS_RES_ADJUST)) begin $sformat(msg, "Nv_ddr3 Mode : tDS violation by %0t", tDS_nv_ddr3_min - ($realtime - tm_dq)); ERROR(ERR_TIM,msg);end
            if (Dqs) begin
                if (first_dqs) begin // posedge Dqs
                    if (din_ODT_en) begin
			if (($realtime - tm_dqs_f) < tWPRE2_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : tWPRE2 violation by %0t", tWPRE2_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end
			if (($realtime - tm_dqs_r) < (tWPRE2_nv_ddr3_min + tCDQSS_nv_ddr3_min)) begin $sformat(msg,"Nv_ddr3 Mode : tCDQSS violation by %0t", tWPRE2_nv_ddr3_min + tCDQSS_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end else begin
			if (($realtime - tm_dqs_f) < tWPRE_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : tWPRE violation by %0t", tWPRE_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end
			if (($realtime - tm_dqs_r) < (tWPRE_nv_ddr3_min + tCDQSS_nv_ddr3_min)) begin $sformat(msg,"Nv_ddr3 Mode : tCDQSS violation by %0t", tWPRE_nv_ddr3_min + tCDQSS_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end
		    if (din_ODT_en) begin
			if (($realtime - tm_ale_f) < tCALS2_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : Ale tCALS2 violation on Dqs by %0t", tCALS2_nv_ddr3_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_cle_f) < tCALS2_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : Cle tCALS2 violation on Dqs by %0t", tCALS2_nv_ddr3_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    end else begin
			if (($realtime - tm_ale_f) < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : Ale tCALS violation on Dqs by %0t", tCALS_nv_ddr3_min - ($realtime - tm_ale_f)); ERROR(ERR_TIM, msg); end        
			if (($realtime - tm_cle_f) < tCALS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : Cle tCALS violation on Dqs by %0t", tCALS_nv_ddr3_min - ($realtime - tm_cle_f)); ERROR(ERR_TIM, msg); end
		    end
		    if ((($realtime - tm_ce_n_f) < tCS1_nv_ddr3_min) && !din_ODT_en) begin $sformat(msg,"Nv_ddr3 Mode : tCS1 violation on Dqs by %t ", tCS1_nv_ddr3_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if ((($realtime - tm_ce_n_f) < tCS2_nv_ddr3_min) && din_ODT_en) begin $sformat(msg,"Nv_ddr3 Mode : tCS2 violation on Dqs by %t ", tCS2_nv_ddr3_min  - ($realtime - tm_ce_n_f)); ERROR(ERR_TIM, msg); end
		    if (($realtime - tm_we_n_r_ale) < tADL_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : tADL violation by %t ", tADL_nv_ddr3_min - ($realtime - tm_we_n_r_ale)); ERROR(ERR_TIM, msg); end
		    if (cmnd_85h && ~status_cmnd && ~cmnd_78h && die_select) begin
			if ($realtime - tm_we_n_r_ale < tCCS_nv_ddr3_min) begin $sformat(msg,"Nv_ddr3 Mode : tCCS violation on We_n by %t", tm_we_n_r_ale + tCCS_nv_ddr3_min - $realtime); ERROR(ERR_TIM,msg); end
                    end
		end else begin
		    if (first_tDSC) begin // no need to qualify with !Cle as in first_tRC b/c we qualify with any_din_sync_enh above
			first_tDSC = 0; // blocking so that we do check first tDQSL/tDSC on this clk edge
			tDSC_nv_ddr3 = $realtime - tm_dqs_r;
			
			update_clock_parameters;  //only need to do this update after the first tDSC_nv_ddr3 calculation after device becomes active
	                if (DEBUG[0]) $display("new DQS clk : tDSC_nv_ddr3_min=%0t, tDSC_nv_ddr3=%0t", tDSC_nv_ddr3_min, tDSC_nv_ddr3);
			if ((tm_dqs_f - tm_dqs_r) < tDQSH_nv_ddr3_min - TS_RES_ADJUST) // Do first tDQSH check now b/c we skipped it before
			    begin $sformat(msg, "Nv_ddr3 Mode : first tDQSH min pulse width violation by %0t", tDQSH_nv_ddr3_min - (tm_dqs_f - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		    end
                    if ((($realtime - tm_dqs_f) < tDQSL_nv_ddr3_min - TS_RES_ADJUST) && !first_tDSC) begin $sformat(msg, "Nv_ddr3 Mode : tDQSL min pulse width violation by %0t", tDQSL_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg);end
                    if ((($realtime - tm_dqs_r) < tDSC_nv_ddr3_min) && !first_tDSC) begin $sformat(msg,"Nv_ddr3 Mode : tDSC violation by %0t", tDSC_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end
		end
		first_dqs <= 0;
            end else begin // negedge Dqs
		if (!first_dqs && !first_tDSC) begin
                    if (($realtime - tm_dqs_r) < tDQSH_nv_ddr3_min - TS_RES_ADJUST) begin $sformat(msg, "Nv_ddr3 Mode : tDQSH min pulse width violation by %0t", tDQSH_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg);end
                end
            end
	end
	if (nv_ddr3 && ~Ce_n) begin // update time values after the checks
            if (Dqs) begin
                tm_dqs_r <= $realtime;
            end else begin
		tm_dqs_f <= $realtime;
	    ...
	    //check for postamble hold constraint 
            if (Dqs) begin
		if (($realtime - tm_ale_r) < tWPSTH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tWPSTH violation by %0t", tWPSTH_nv_ddr3_min - ($realtime - tm_ale_r)); ERROR(ERR_TIM,msg); end
		if (($realtime - tm_cle_r) < tWPSTH_nv_ddr3_min) begin $sformat(msg, "Nv_ddr3 Mode : tWPSTH violation by %0t", tWPSTH_nv_ddr3_min - ($realtime - tm_cle_r)); ERROR(ERR_TIM,msg); end
	    end
        end
	if (nv_ddr3 && Dqs && (($realtime - tm_ce_n_r) < tWPSTH_nv_ddr3_min)) begin $sformat(msg, "Nv_ddr3 Mode : tWPSTH violation by %0t", tWPSTH_nv_ddr3_min - ($realtime - tm_ce_n_r)); ERROR(ERR_TIM,msg); end
	if (nv_ddr3 && ~Ce_n && (Dqs === 1'bz) && first_dqs) begin // Host releases Dqs to zz at start of data output
	    if ((($realtime - tm_re_n_f) < tDQSRH_nv_ddr3_min - TS_RES_ADJUST)) begin $sformat(msg, "Nv_ddr3 Mode : tDQSRH violation by %0t", tDQSRH_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg);end
	    else begin
	    #(TS_RES_ADJUST); // check again 0.001 ns later in case host changed both signals at same time
	    if ((($realtime - tm_re_n_f) < tDQSRH_nv_ddr3_min - TS_RES_ADJUST)) begin $sformat(msg, "Nv_ddr3 Mode : tDQSRH violation by %0t", tDQSRH_nv_ddr3_min - ($realtime - tm_re_n_f)); ERROR(ERR_TIM,msg);end
	    end
    ...
    always @(Io) begin
        if (nv_ddr3 && ~Ce_n) begin
            if (!Ale && !Cle) begin
                if (($realtime - tm_dqs_r) < (tDH_nv_ddr3_min - TS_RES_ADJUST) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tDH violation by %0t", tDH_nv_ddr3_min - ($realtime - tm_dqs_r)); ERROR(ERR_TIM,msg); end 
                if (($realtime - tm_dqs_f) < (tDH_nv_ddr3_min - TS_RES_ADJUST) && any_din_nv_ddr3) begin $sformat(msg, "Nv_ddr3 Mode : tDH violation by %0t", tDH_nv_ddr3_min - ($realtime - tm_dqs_f)); ERROR(ERR_TIM,msg); end 
            end else if (Ale || Cle) begin
                if (($realtime - tm_we_n_r) < (tCAH_nv_ddr3_min - TS_RES_ADJUST)) begin $sformat(msg, "Nv_ddr3 Mode : tCAH violation by %0t", tCAH_nv_ddr3_min - ($realtime - tm_we_n_r)); ERROR(ERR_TIM,msg); end 
            end
            tm_dq <= $realtime;
        end
    end
	...
	if (sync_enh_mode && !first_tDSC) begin // first_tDSC will transition 1->0 in this timestep, right before calling this task
	    tDQSH_sync_enh_min	= 0.43 * tDSC_sync_enh;
	    tDQSL_sync_enh_min	= 0.43 * tDSC_sync_enh;
	end 
	if (sync_enh_mode && !first_tRC) begin // first_tRC will transition 1->0 in this timestep, right before calling this task
	    tQH_sync_enh_min	= 0.37 * tRC_sync_enh;
	    tQSH_sync_enh_min	= 0.37 * tRC_sync_enh;
	    tQSL_sync_enh_min	= 0.37 * tRC_sync_enh;
	    tREH_sync_enh_min	= 0.45 * tRC_sync_enh;
	    tREH_sync_enh_max	= 0.55 * tRC_sync_enh;
	    tRP_sync_enh_min	= 0.45 * tRC_sync_enh;
	    tRP_sync_enh_max	= 0.55 * tRC_sync_enh;
	    tRPST_sync_enh_min	= tDQSRE_sync_enh_max + 0.5 * tRC_sync_enh;
	end

	if (nv_ddr3 && !first_tDSC) begin // first_tDSC will transition 1->0 in this timestep, right before calling this task
	    tDQSH_nv_ddr3_min	= 0.43 * tDSC_nv_ddr3;
	    tDQSL_nv_ddr3_min	= 0.43 * tDSC_nv_ddr3;
	end 
	if (nv_ddr3 && !first_tRC) begin // first_tRC will transition 1->0 in this timestep, right before calling this task
	    tQH_nv_ddr3_min	= 0.37 * tRC_nv_ddr3;
	    tQSH_nv_ddr3_min	= 0.37 * tRC_nv_ddr3;
	    tQSL_nv_ddr3_min	= 0.37 * tRC_nv_ddr3;
	    tREH_nv_ddr3_min	= 0.45 * tRC_nv_ddr3;
	    tREH_nv_ddr3_max	= 0.55 * tRC_nv_ddr3;
	    tRP_nv_ddr3_min	= 0.45 * tRC_nv_ddr3;
	    tRP_nv_ddr3_max	= 0.55 * tRC_nv_ddr3;
	    tRPST_nv_ddr3_min	= tDQSRE_nv_ddr3_max + 0.5 * tRC_nv_ddr3;
	end	
